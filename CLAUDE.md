...
│   │   ├── part-1/
│   │   └── ...
│   ├── aws/             # AWS 雲端系列（大綱已備，章節待寫）
│   │   ├── 課程大綱.md   # ← AWS 這本書的大綱
│   │   ├── part-1/
│   │   │   ├── aws-1-1-xxx.md
│   │   │   └── ...
│   │   └── ...
│   ├── infra/           # Infra 自架基礎建設系列（大綱已備，章節待寫；檔名 infra-X-Y-xxx.md）
│   │   ├── 課程大綱.md
│   │   └── ...
...
│   │   ├── 課程大綱.md
│   │   └── ...
│   ├── comfyui/         # ComfyUI 生成式美術系列（大綱已備，章節待寫；檔名 comfy-X-Y-xxx.md）
│   │   ├── 課程大綱.md
│   │   └── ...
│   └── ...              # 未來可再新增其他主題
├── 課外讀物/             # 跨課程共用的延伸閱讀（E-1 ~ E-14）
│   ├── 課程大綱.md       # ← 課外讀物的「共用總目錄」（所有書共用，不屬於 basic）
│   ├── E-1-terminal/
│   ├── E-2-npm/
│   ├── E-3-network/
│   ├── E-4-database/
...
>
> **每本書都有自己的 `課程大綱.md`**，放在該書資料夾根目錄（如 `lessons/basic/課程大綱.md`、`lessons/aws/課程大綱.md`）。**要撰寫或續寫某本書的章節時，先讀該書的 `課程大綱.md`，再依大綱的章節編號與順序撰寫。** 大綱列出規劃中的章節（即使檔案還沒建立）。
