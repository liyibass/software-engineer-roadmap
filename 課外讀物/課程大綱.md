...
| E-15 | 韌性模式（Resilience Patterns） | 新增系列 |
...
- `E-3-8` NAT：為什麼一堆裝置能共用同一個對外 IP
- `E-3-9` 網域搬家：DNS 委派鏈到底改了哪一行（NS／SOA、否定快取、DNSSEC 陷阱）
...
- `E-4-5` 資料庫鎖與並發控制：悲觀鎖、樂觀鎖、死鎖與如何避免
...
- `E-8-10` 一台電腦，兩個 GitHub 帳號：SSH 金鑰分流
- `E-8-11` Symlink 進 git 嗎？跨專案共用一份程式碼的坑（Windows 的 junction）
...
- `E-13-16` Broker：居中轉介的「中間人」——從 Message Broker 到 MQTT（呼應 E-13-5）
...
> 當你依賴的東西（別的服務、資料庫、第三方 API）壞掉時，怎麼讓自己的服務「不跟著一起死」。這個系列收錄讓系統更耐操的實戰模式，呼應 SRE Part 8「為失敗設計」。
