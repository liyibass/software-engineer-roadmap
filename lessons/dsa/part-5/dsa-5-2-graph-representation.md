...
先把邊列清楚：A—B、A—C、C—D，無向（所以每條邊兩端都要記）。
...
**2.**
頂點順序 A, B, C, D 對應索引 0, 1, 2, 3。格子 `[i][j] = 1` 表示 i 連到 j。無向圖的矩陣會**對稱**（沿對角線鏡射），對角線全是 0（沒有連自己）：
...
驗證一下：邊有 A—B、A—C、C—D 三條，矩陣裡對稱的 1 應該有 3 × 2 = 6 個——數一數剛好 6 個，正確。

**3.**
這是**稀疏圖**，該用**鄰接串列**。

推導：十億個頂點，如果用鄰接矩陣，空間是 O(n²) = (10⁹)² = 10¹⁸ 個格子——這數字大到任何電腦都存不下，而且其中絕大多數都是 0（因為每個人只連幾百個朋友，遠遠少於十億）。這正是「稀疏圖」的定義：實際的邊數（約 10⁹ × 幾百 ≈ 幾千億條）遠遠少於 n²。

改用鄰接串列，空間是 O(頂點 + 邊)，只存「真實存在的朋友關係」，就務實可行得多。而且社群網路最常做的操作是「列出某人的所有朋友」，這正好是鄰接串列的強項（直接拿那個人的串列即可）。所以真實的大型社群網路一律用鄰接串列（或其變體）。
