...
**第 1 題：用「排隊」解釋 FIFO，並說出和堆疊的核心差別**

排隊買票：先來的人排在前面、先買到票從「前端」離開；後來的人從「後端」排進去。於是**先進的先出**——這就是「先進先出（FIFO，First In First Out）」，公平地依序處理。

和堆疊的核心差別在「**從哪端進出**」：

```
堆疊 LIFO：同一端（頂端）進出   → 後進先出（像一疊盤子）
佇列 FIFO：一端進、另一端出     → 先進先出（像排隊）
```

一句話：堆疊「同進同出一端」，佇列「一端進、另一端出」，導致一個後進先出、一個先進先出。

**第 2 題：為什麼「陣列 + shift」的佇列有效率問題？**

問題出在 `dequeue` 用的 `arr.shift()`——它**從陣列開頭移除**：

```
arr.push(x)    // 從結尾加入（enqueue）→ O(1) ✓
arr.shift()    // 從開頭移除（dequeue）→ O(n) ⚠️
```

回憶 [dsa-2-1]：陣列要保持「連續」，把開頭那格移掉後，**後面所有元素都得往前補一格**（索引 1 補到 0、2 補到 1……），挪動量和元素個數成正比，所以 `shift` 是 **O(n)**。

於是每次 dequeue 都 O(n)，資料一多就明顯變慢。這正是「懂底層複雜度才能避開效能坑」的例子（[dsa-1-1]）。改用鏈結串列、環狀佇列或內建雙端佇列，才能讓 dequeue 也是 O(1)。

**第 3 題（思考題）：環狀佇列怎麼用「移動指標 + 繞圈」達成兩端 O(1)？**

關鍵想法：**不真的搬動元素，只移動兩個標記位置的指標**——`front`（前端，下次 dequeue 的位置）和 `rear`（後端，下次 enqueue 的位置）。

```
enqueue：把新元素放到 rear 的位置，然後 rear 往後移一格 → O(1)（沒挪別人）
dequeue：讀出 front 位置的元素，然後 front 往後移一格 → O(1)（沒挪別人）
```

因為 dequeue 只是「把 front 往後移」，而不是把後面全部往前補，所以避開了 shift 的 O(n)。

那空間怎麼重複利用？當指標移到陣列**末端**時，讓它**「繞回」開頭**（像時鐘從 12 繞回 1），去用前面被 dequeue 空出來的格子：

```
指標前進的公式：index = (index + 1) % capacity
   例如容量 8，指標在第 7 格 → (7 + 1) % 8 = 0 → 繞回開頭
```

這樣陣列被「首尾相接」當成一個環來用，既保住兩端 O(1)、又享受陣列的快取友善（[dsa-2-4]）。代價是容量固定（滿了要自己判斷、或另外擴容），且通常要另存一個 `size` 或留一格空位來區分「空」和「滿」。
