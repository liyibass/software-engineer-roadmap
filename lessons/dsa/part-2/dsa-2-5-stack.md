...
**第 1 題：用「一疊盤子」解釋 LIFO，並說出兩個核心操作**

洗好的盤子一個個往上疊，要用時從最上面拿。於是**最後疊上去的盤子，最先被拿走**——你不會從中間抽盤子。這就是「後進先出（LIFO，Last In First Out）」。

兩個核心操作：

- **push**：把一個東西放到頂端（疊上去）。
- **pop**：把頂端的東西拿走並回傳（拿最上面那個）。

（常見的還有 **peek**：只看一眼頂端是什麼，但不拿走。）這些都只動頂端、不牽動其他元素，所以都是 O(1)。

**第 2 題：舉一個「本質是後進先出」的例子（文中之外）**

示範幾個都可以：

- **文字編輯器的「復原（Undo）堆疊」**：每做一個動作就 push，按 Ctrl+Z 時 pop 最後那個動作先被撤銷。
- **程式語言的括號/標籤配對**：遇到 `(`、`{`、`<div>` 就 push，遇到對應的結尾就 pop——最後打開的要最先關閉，正是 LIFO。（HTML/JSX 標籤巢狀也是同理。）
- **走迷宮的回溯**：每往前一步就把位置 push，走到死路時 pop 退回上一步，最後走的先退。

判斷標準：只要問題的本質是「**最後發生的，要最先被處理／撤銷／返回**」，就是後進先出，適合用堆疊。

**第 3 題（思考題）：為什麼無窮遞迴造成 stack overflow？**

程式每呼叫一個函式，就會把該函式的「呼叫框（stack frame，存它的參數、區域變數、返回位置）」**push 到呼叫堆疊上**；函式執行完返回時才 pop 掉。

無窮遞迴的問題是：函式**沒有終止條件（或永遠碰不到）**，於是它一直呼叫自己、一直呼叫自己……每一層都 push 一個新的呼叫框上去，**卻永遠沒有任何一層執行完、永遠不 pop**：

```
f() 呼叫 f() 呼叫 f() 呼叫 f() ...
堆疊：[f][f][f][f][f][f]......  ← 只進不出，越堆越高
```

但呼叫堆疊的記憶體空間是**有限**的。當堆疊被推爆、超出這塊空間的上限，系統就報錯 **stack overflow（堆疊溢位）**、程式崩潰。

所以正確的遞迴一定要有「**基底條件（base case）**」讓它在某層停止呼叫、開始返回（pop），堆疊才不會無限長高。（詳見 [dsa-6-1] 遞迴。）
