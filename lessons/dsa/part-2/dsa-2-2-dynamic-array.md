...
**第 1 題：動態陣列「滿了」時做了什麼？這次是 O(1) 還 O(n)？**

當 `size == capacity`（現有格子全用完）時，push 會觸發「擴容」，三個步驟：

1. **開一塊更大的新陣列**（通常是原容量的兩倍）。
2. **把舊陣列的元素全部複製到新陣列**。
3. **放入新元素，並丟棄舊陣列**（讓動態陣列改用新陣列）。

這次操作是 **O(n)**——關鍵在步驟 2，要複製「現有的全部 n 個元素」，成本和 n 成正比。（沒滿的那些 push 才是 O(1)，直接放進去就好。）

**第 2 題：為什麼擴容要「翻倍」而不是「每次加一格」？**

差別在「多久擴容一次」：

- **每次加一格**：陣列一滿就只多一格，於是**幾乎每次 push 都會再次滿、再次擴容、再次複製全部**。push n 個元素，複製成本約 `1 + 2 + 3 + ... + n ≈ n²/2`，總體 **O(n²)**——超慢。
- **容量翻倍**：擴容後多出「一整倍的空位」，接下來可以塞很多次都不用再擴容；而且擴容越來越稀疏（容量 1→2→4→8→16…）。push n 個元素，總複製成本約 `1 + 2 + 4 + ... + n ≈ 2n`，是**線性**的。

所以「每次加一格」的問題是：把擴容變成幾乎每次都發生的常態，成本累積成平方級。翻倍則讓昂貴的擴容變得稀有。

**第 3 題（思考題）：為什麼說 push 是「攤銷 O(1)」？**

「攤銷（amortized）」是把「偶爾的昂貴操作」平均分攤到「大量的便宜操作」上，看**平均每次**的成本。

雖然「剛好觸發擴容的那一次 push」是 O(n)，但因為翻倍策略讓擴容極少發生：push n 個元素，總複製成本只有約 2n（見第 2 題），把它分攤到 n 次 push：

```
總成本 ≈ 2n（複製）+ n（每次放入）= 3n
平均每次 = 3n / n = 3 = 常數 → O(1) 攤銷
```

所以雖然單看某一次可能很貴，但**長期平均下來每次 push 就是常數時間**，這就是「攤銷 O(1)」。比喻就像健身房年費——偶爾一筆大支出，分攤到每天去，平均很便宜（[dsa-1-4]）。
