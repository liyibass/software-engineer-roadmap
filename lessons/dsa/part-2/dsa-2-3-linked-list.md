...
- **陣列 = 一排黏死在桌上、緊緊相鄰的格子。** 每格挨著下一格、位置連續。好處是「看編號就能直接指到哪一格」（算位址跳過去）；壞處是要在中間塞一格，得把後面全部往旁邊推。
- **鏈結串列 = 一串用線串起來的珠子。** 每顆珠子（節點）自己存「值」和「一條指向下一顆的線（指標）」，珠子本身可以散落在抽屜各處，只靠線連著。好處是要插一顆珠子，只要「剪線、重接兩條線」就好，不用動別的珠子；壞處是想拿第 5 顆，只能「從頭順著線一顆顆數過去」，沒辦法一步指到。

本質差異一句話：**陣列靠「連續 + 算位址」，鏈結串列靠「指標 + 順著走」。** 前者存取快、增刪慢；後者增刪快、存取慢。

**第 2 題：為什麼鏈結串列取第 5 個是 O(n)、陣列是 O(1)？**

- **陣列**：元素連續排列，位址能直接算出來（`起始位址 + 5 × 元素大小`），一步跳到，和索引大小無關 → **O(1)**。
- **鏈結串列**：節點散落各處、不連續，沒有公式能算出「第 5 個節點在哪」。唯一方法是**從 head 出發，順著 `next` 箭頭一步步走**：

```
current = head          // 第 0 個
current = current.next  // 第 1 個
current = current.next  // 第 2 個
current = current.next  // 第 3 個
current = current.next  // 第 4 個
current = current.next  // 第 5 個 ← 走了 5 步
```

要拿第 i 個就得走 i 步，最壞要走過幾乎整串 → 步數和 n 成正比 → **O(n)**。

**第 3 題：為什麼鏈結串列中間插入 O(1)、陣列 O(n)？（改箭頭 vs 挪元素）**

- **鏈結串列＝改箭頭。** 假設要在 A 和 C 之間插入 B：只要讓「B 指向 C」、再讓「A 指向 B」，改兩條箭頭就完成，其他節點原地不動。改箭頭的動作是固定兩步，和串多長無關 → **O(1)**。

  ```
  原本： A → C
  插入： A → B → C   （只重接了 A→B 和 B→C 兩個箭頭）
  ```

  ⚠️ 前提是你「已經站在插入點」（手上有前一個節點的參考）。如果還得先從 head 找到那個位置，找的過程是 O(n)——但「插入這個動作本身」是 O(1)。

- **陣列＝挪元素。** 陣列要保持連續，中間空出一格，就得把插入點之後的元素**全部往後挪一格**，才放得進去。挪的數量和元素個數成正比 → **O(n)**。

一句話：**鏈結串列只需「重接幾條線」，陣列卻要「搬動一整排」——這就是它增刪快、陣列增刪慢的根本原因。**
