...
分析：這個函式額外開了一個新陣列來裝結果，輸入有 n 個元素，就會 push 進 n 個平方值，所以這個新陣列會長到跟輸入一樣大——額外記憶體隨 n 線性成長，就是 **O(n) 空間**。

參考實作：

```typescript
function squareAll(arr: number[]): number[] {
  const result: number[] = []; // 這個新陣列會裝 n 個元素 → O(n) 額外空間
  for (const x of arr) {
    result.push(x * x);
  }
  return result;
...
（小提醒：我們算的是「額外」空間，不含輸入 `arr` 本身佔的空間。真正讓它變 O(n) 的是那個新開的 `result` 陣列；如果只用幾個固定變數，就會是 O(1)。）

**第 2 題**

`twoSum_B` 用一個 `Set` 記住「**目前為止看過的每一個數字**」。這樣一來，每掃到一個新數字 `x`，它只要問一句「我之前有沒有看過 `target - x`（湊成 target 所需的另一半）？」——查 Set 是 O(1)，馬上就知道答案。

它省下的是：解法 A 那個「對每個元素再跑一次內層迴圈，去比對其他所有元素」的重複比對。原本 O(n²) 的兩兩比對，因為「看過的都記在 Set 裡、隨時能 O(1) 查」，被壓成只要掃一遍的 **O(n)**。代價就是那個 Set 要佔 **O(n) 額外空間**——這就是「多花記憶體記住看過的東西，換掉重複計算」的用空間換時間。

**第 3 題**

是同一個思想。共同點一句話：**兩者都是「先花記憶體把算過／查過的結果存下來，之後直接拿現成的，避免重複計算」——用空間成本換取時間上的節省。**

（差別只在應用場景：演算法裡叫「用空間換時間」，系統設計裡把存下來的結果放在更快的地方就叫「快取」，但骨子裡的取捨完全一樣。）
