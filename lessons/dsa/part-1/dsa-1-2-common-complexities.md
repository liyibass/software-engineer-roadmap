...
（口訣：常數最快，接著對數，然後線性，再來線性對數，平方最慢。這就是本章排行榜圖從左到右的順序。）

**第 2 題**

各舉一例（有其他合理答案也可以）：

- **O(1)**：用索引取陣列元素 `arr[5]`；用 key 查 HashMap / Map。不管資料多大都是固定幾步。
- **O(log n)**：二分搜尋（在已排序陣列裡每次砍一半）。「砍一半」類的操作都是這級。
- **O(n)**：把陣列每個元素掃一遍，例如求和、找最大值。每個元素都要看一次。
- **O(n²)**：兩層巢狀迴圈，例如本書 dsa-1-1 的 `hasDuplicate`（兩兩比對）、簡單的氣泡/選擇排序。

**第 3 題**

**快了大約 5 萬倍。**

推導：一兆是 `1,000,000,000,000`（10¹²），兩千萬是 `20,000,000`（2×10⁷）。相除：

```
1,000,000,000,000 ÷ 20,000,000 = 50,000
```

也就是原本要跑「一兆步」的工作，現在只要「兩千萬步」，快了約 5 萬倍。

**為什麼這麼有價值？** 因為這種差距往往是「能不能用」的分水嶺，而不只是「快一點」。假設電腦一秒跑十億步：一兆步要約 1000 秒（超過 16 分鐘），使用者早就關掉頁面了；而兩千萬步只要 0.02 秒，幾乎是秒回。從 O(n²) 改成 O(n log n)，常常就是把「跑不完 / 卡死」變成「順暢即時」——而且資料越大，這個差距只會越誇張。這就是為什麼「挑對演算法等級」比「把程式碼微調快個幾成」重要得多。
