...
- **最好情況（best case）：O(1)**——目標剛好是第一個元素，一步就找到、直接回傳。
- **最壞情況（worst case）：O(n)**——目標在最後一個，或根本不存在，得把整個陣列 n 個元素都掃完。
- **平均情況（average case）：O(n)**——假設目標隨機出現在任一位置，平均掃到一半（約 n/2 步）才找到；n/2 丟掉常數係數 1/2 後，還是 **O(n)**。

（重點：平均雖然只跑一半，但「一半」仍隨 n 線性成長，所以等級一樣是 O(n)，只有最好情況才是 O(1)。）

**第 2 題**

因為最壞情況給出一個「**保證**」：不管輸入多刁鑽、運氣多差，這個演算法**最慢也就這麼慢，不會再更糟**。

這對重要系統特別關鍵——很多系統不能賭運氣。如果你只看「平均」或「最好」情況，萬一真的遇到最壞的輸入（惡意使用者故意構造、或剛好碰上極端資料），系統可能就突然卡死出事。最壞情況是「上限保證」，讓你能安心地說「再糟也就這樣」，這對需要穩定、不能突然崩掉的關鍵系統來說非常重要。所以沒特別說明時，Big-O 預設指的就是最壞情況。

**第 3 題**

健身房年費是「一次付一大筆」（貴），但如果你這一年去了很多次，把那筆年費**分攤**到每一次，其實每次去的成本很低。你不會因為「繳費那天付了一大筆」就說「每次去健身房都很貴」——因為那筆大錢被平均分攤掉了。

動態陣列的 push 完全一樣：

- **絕大多數的 push** 都是把元素直接放進預留空間 → O(1)，很便宜（像「平常去健身房不用再付錢」）。
- **偶爾空間滿了**，要開一塊兩倍大的新空間、把舊元素全搬過去 → 那一次是 O(n)，很貴（像「繳年費那天」）。

但擴容非常少發生（容量要翻倍才觸發一次），所以把那次昂貴的搬移成本，**攤銷**到中間許許多多次便宜的 push 上，平均下來每次 push 其實還是 **O(1)**。因此我們說它是「攤銷 O(1)」——別被偶爾那次擴容嚇到，平均很便宜；而不是被單次最壞情況誤導成 O(n)。
