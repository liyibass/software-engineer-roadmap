...
因為「跑幾秒」量到的不只是演算法本身，還混進了一堆和演算法無關的雜訊：你的電腦快不快、用什麼程式語言、當下系統忙不忙、甚至同一台機器跑兩次都可能不一樣。換一台更快的電腦，一個很爛的演算法也可能「跑得比較快」，那這樣根本沒辦法公平比較「演算法本身的好壞」。

而 Big-O 量的是「資料量 n 變大時，工作量成長得多快」——這是演算法的**本質特性**，跟硬體無關。不管你用哪台電腦，一個 O(n²) 的演算法在資料變大時就是會比 O(n) 的吃力得多。所以 Big-O 才是客觀、可攜、能反映演算法本質的尺。

（用本章的比喻：不問「現在時速幾公里」（受路況影響），而問「油門踩到底能加速到多快的潛力」（看本質）。）

**第 2 題**

答案是 **O(n)**。

推導：那個跑 n 次的迴圈是 n 步，三行固定計算是 3 步（和 n 無關的常數），加起來是 `n + 3` 步。依照 Big-O 「只看成長最快的主導項、丟掉常數與低次項」的規則，`n + 3` → **O(n)**。那三行固定計算不管 n 多大都只跑一次，在 n 很大時完全微不足道。

**第 3 題**

Big-O 關心的是「當 n 變得**非常大**時」的成長趨勢，而不是某個特定 n 的精確步數。

- 丟掉 **常數 5**：不管 n 多大，這個 5 永遠是 5，不會跟著 n 成長。當 n = 1,000,000 時，`3n = 3,000,000`，那個 +5 幾乎看不見，對「成長等級」毫無影響。
- 丟掉 **係數 3**：`3n` 和 `n` 都是「隨 n 線性成長」——n 翻倍，兩者都跟著翻倍。這個 3 只是把線放大三倍，並沒有改變「線性成長」這個本質等級。Big-O 分的是「成長的等級（是線性？平方？）」，不是「精確倍數」。

所以 `3n + 5` 的成長本質就是「線性」，寫成 **O(n)**。
