...
**第 1 題：畫存了 "to"、"tea"、"ted"、"in" 的 Trie**

從根沿字元往下鋪路，✓ 代表「有一個完整單字在此結束」：
...
- "to"、"tea"、"ted" 都以 t 開頭，**共用了 t 這個節點**；"tea" 和 "ted" 更進一步**共用了 t→e 這段路徑**，只在最後一個字元 a / d 才分岔。
- 標 ✓ 的是「單字結束」的節點：`o`（to）、`a`（tea）、`d`（ted）、`n`（in）。注意 `t` 和 `te` 這些節點**沒有** ✓，因為 "t"、"te" 不是我們存進去的完整單字，只是路過的中繼點。
- "in" 因為開頭字元 i 和 t 不同，從根就走另一條分支。

**第 2 題：為什麼 Trie 能高效找出「所有以某前綴開頭的詞」**

因為 Trie 的結構天生就把「**同前綴的詞放在同一條路徑下**」。要找所有以某前綴（例如 "te"）開頭的詞，只要：先沿著 t→e 走到「te 那個節點」（O(L)，L 是前綴長度），這個節點**底下的整棵子樹**就是所有以 "te" 開頭的詞（tea、ted…）。一次定位、再把子樹底下標 ✓ 的收集起來即可，完全不用去掃描字典裡其他不相關的詞。

**第 3 題：什麼時候用 Trie、什麼時候用雜湊表**

- **用 Trie**：需要「前綴相關」操作時——自動補全、前綴搜尋、列出所有共同開頭的詞、按字典序遍歷。這些正是 Trie 的專長，複雜度 O(L)（只跟字串長度有關，和存了幾個詞無關）。
- **用雜湊表**：只需要查「一個完整的詞在不在」、不在乎前綴時。雜湊表查完整詞是 O(1)、更省空間；而 Trie 每個字元都要一個節點、比較耗空間。

一句話：**要不要做前綴操作**就是分界線——要就 Trie，不要就雜湊表（用空間換前綴效率，呼應 dsa-1-3 的空間換時間）。
