...
先把這棵樹畫出來（根 8，左子 3、右子 10；3 的左子 1、右子 6；10 沒有子節點）：
...
- **前序（自己 → 左 → 右）**：先處理 8，再往左子樹（3，接著 3 的左 1、右 6），最後右子樹 10。
  → `8, 3, 1, 6, 10`
- **中序（左 → 自己 → 右）**：先走完 3 的左子樹（1）、再 3、再 3 的右（6），然後 8，最後 10。
  → `1, 3, 6, 8, 10`
- **後序（左 → 右 → 自己）**：先 3 的左右子節點 1、6，再 3，接著 10，最後才是根 8。
  → `1, 6, 3, 10, 8`

（順帶一提：這棵樹剛好是一棵 BST，所以中序結果 `1,3,6,8,10` 剛好由小到大排序，呼應 [dsa-4-3]。）

**第 2 題：深度優先 vs 廣度優先（層序）的差別**

- **深度優先（DFS）**：到一個節點就「一路往深處鑽到底、再回頭」。就像走迷宮時沿著一條路走到死路才折返。
- **廣度優先／層序（BFS）**：不鑽深，而是「一層一層、由上到下、由左到右」拜訪，把同一層的全部走完才進到下一層。就像水波一圈一圈往外擴散。

差別的關鍵在「拜訪順序」：DFS 先深後廣，BFS 先廣後深。實作上 DFS 常用遞迴（或堆疊），BFS 則要用佇列（[dsa-2-6]）來記住「還沒處理的下一層節點」。

**第 3 題：為什麼三種 DFS 只差一行的位置**

因為三種走訪的**骨架完全相同**——都是「處理自己、遞迴走左子樹、遞迴走右子樹」這三件事，唯一的差別只是「**處理自己這件事排在第幾順位**」：放最前是前序、放中間是中序、放最後是後序。左右子樹的遞迴呼叫順序都沒變。

這說明了兩件事：一是**遞迴處理樹非常優雅**——把「怎麼走一個節點」講清楚，剩下的左右子樹用同樣規則自動展開；二是**「拜訪的時機」和「拜訪的結構」是可以分離的**：結構（怎麼遍歷整棵樹）固定，只要調整「什麼時候處理資料」就能得到不同用途的走訪。
