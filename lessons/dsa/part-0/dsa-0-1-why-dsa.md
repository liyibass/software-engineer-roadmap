...
方法 B（二分）快這麼多，是因為它「每看一次，就把還要找的範圍砍掉一半」。方法 A（逐一）每看一次只排除掉一筆，所以一百萬筆最壞要看一百萬次；方法 B 一百萬筆只要砍約 20 次就把範圍縮到剩一筆（因為 2²⁰ ≈ 一百萬）。資料越大，這種「砍一半」的威力越驚人——十億筆也只要約 30 次。

它的**前提**是：**資料必須先照順序排好**。因為二分每次都要靠「目標比中間值大還是小」來決定往左半還往右半找；如果資料是亂的，「中間值比目標大」這個判斷就無法告訴你目標在哪一邊，二分就失效了。

**第 2 題**（示範答案，你的例子只要符合「同一件事有快慢不同做法」即可）

在一疊沒分類的文件裡找某一份：
- 慢方法：從最上面一張一張往下翻，直到找到。
- 快方法：先把文件照日期或姓氏排好、分成幾個區塊（例如 A–F、G–M…），要找時直接跳到對應區塊再找。

「先花力氣整理（建立好的結構）」讓之後每次「找」都快很多——這正呼應二分搜尋「先排序、再快速查」的精神。

**第 3 題**

因為「**能用什麼演算法、跑多快**」往往由「**資料怎麼放**」決定，兩者是綁在一起的。以二分搜尋為例：這個又快又聰明的演算法，只有在資料放進「**已排序的結構**」時才能用；如果資料是一堆亂放的，二分搜尋根本無從施展，你只能退回逐一尋找。

換句話說，演算法（怎麼處理）和資料結構（怎麼組織）是一對搭檔：**選對的結構，才解鎖高效的演算法**。所以分開學會少掉一半的理解——這也是為什麼這門課叫「資料結構**與**演算法」。
