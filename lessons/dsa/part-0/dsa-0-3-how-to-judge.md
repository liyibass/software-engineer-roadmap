...
1. **正確（Correctness）** —— 有沒有算出對的答案。
2. **夠快（時間複雜度）** —— 執行要花多少時間。
3. **不太耗記憶體（空間複雜度）** —— 執行要用掉多少額外記憶體。
4. **好讀好維護（可讀性）** —— 別人或未來的你看不看得懂、好不好改。

「**正確**」排第一，是因為一個跑得再快、再省記憶體、程式碼再漂亮的解法，只要答案是錯的，就**毫無價值**——甚至比慢一點但正確的解法更糟（錯的答案會誤導決策）。所以順序永遠是「先做對 → 再做清楚 → 真的需要時才做快」，速度與記憶體都是在「已經正確」的前提下才有意義。

**第 2 題**

「用空間換時間」是指：**多花一些記憶體去「記住」中間結果，來換取之後查詢或計算變快**。

以本章「判斷一個數字之前出現過沒」為例：不想每次都重頭掃一遍全部資料（省記憶體但慢），就用一個「集合（Set）」把看過的數字都存起來；之後要判斷某個數字出現過沒，直接問這個集合就好，一次 O(1) 就知道答案——代價是要額外花記憶體存這個集合。

它和「**快取（cache）**」是同一個思想：快取就是「把算過／拿過的結果先存起來（花空間），下次要用時直接拿，不用重算或重新去遠端拿（省時間）」。所以快取本質上就是一種大規模、系統層級的「用空間換時間」。

**第 3 題**

當「**資料量很小**」而且「**這段程式要好懂好維護**」的時候，簡單的 O(n²) 反而更好。原因：

- **Big-O 講的是「資料變大時」的成長趨勢**，資料量小的時候差距根本感覺不出來。假設 n 永遠不超過 100，O(n²) 也才約 1 萬次運算，現代電腦幾微秒就跑完，使用者完全無感。
- 而硬要換成複雜的 O(n log n)「最佳解」，往往程式碼更難懂、更容易寫出 bug，維護成本上升——**換來的效能提升卻趨近於零**（因為 n 本來就小）。

所以在「資料量小、且重視可讀性」的情境，「**夠好且簡單**」勝過「**最佳但複雜**」。這是一種工程判斷：先了解實際需求（資料多大、跑多頻繁），再決定值不值得為效能犧牲簡潔——也呼應「別過早最佳化」的原則。
