...
俄羅斯娃娃：打開一個娃娃，裡面是「更小的同款娃娃」，你用「同一套動作」去打開它；一直打開下去，直到最小、打不開的那個。遞迴一模一樣——解一個問題，就是解「更小的同款問題」再加一點處理，一路縮小，直到小到能直接回答。

遞迴必備的兩個要素，缺一不可：

- **基本情況（base case）**：最小、不用再遞迴、能直接回答的情況（對應「最小、打不開的娃娃」）。它是遞迴的煞車。
- **遞迴情況（recursive case）**：把問題縮小一點，然後呼叫自己去解那個更小的（對應「打開後裡面還有更小的娃娃」）。

只有遞迴情況、沒有基本情況，就永遠停不下來；只有基本情況、不縮小問題，就根本沒在遞迴。

**第 2 題（`sum(n)`）**

觀察 `sum(n) = 1 + 2 + ... + n = n + (1 + 2 + ... + (n-1)) = n + sum(n-1)`，基本情況是 `sum(0) = 0`：

```typescript
function sum(n: number): number {
  if (n <= 0) return 0;        // 基本情況：煞車
  return n + sum(n - 1);        // 遞迴情況：縮小成 sum(n-1)
}

console.log(sum(5));            // 15（= 1+2+3+4+5）
```

追一下：`sum(3)` = `3 + sum(2)` = `3 + (2 + sum(1))` = `3 + (2 + (1 + sum(0)))` = `3 + 2 + 1 + 0` = 6。

**第 3 題（忘了基本情況）**

如果忘了寫基本情況（或永遠到不了），函式會**無窮遞迴**——一直呼叫自己、永遠不返回。每一次遞迴呼叫都會在「呼叫堆疊（call stack）」上疊一層，堆疊卻永遠不彈出，最後把堆疊空間用光，程式崩潰。

這正是 [dsa-2-5] 講的**呼叫堆疊爆掉、堆疊溢位（stack overflow）**。遞迴是呼叫堆疊的極致應用：往下拆解時一層層疊上去，到基本情況才開始一層層彈出回傳。少了基本情況這個煞車，堆疊就只疊不彈，直到溢位。
