...
**第 1 題（為什麼必須先排序、沒排序硬用會怎樣）**

二分搜尋的每一步，都靠「看中間那個值，判斷目標在左半還是右半」來排除掉一半資料。這個判斷能成立的前提是：**中間值左邊的都比它小、右邊的都比它大**——也就是資料必須已排序。

如果資料沒排序：你看到中間值，卻無法保證「比它小的都在左邊」，於是根本無法判斷目標該往哪半找。這時二分搜尋會**排除錯半邊、找錯方向**，導致找不到明明存在的元素（回傳 -1），結果完全不可靠。沒排序的資料只能用線性搜尋。

**第 2 題（何時值得先排序再二分、何時直接線性）**

關鍵在於「你要在同一份資料上搜尋幾次」：

- **只搜尋一次（或很少次）**：直接用線性搜尋 O(n) 就好。不要為了一次搜尋特地排序——排序本身要花 O(n log n)，比直接線性掃一遍還貴，划不來。
- **要搜尋很多次**：先花一次 O(n log n) 把資料排好，之後每次搜尋都是 O(log n)。排序的一次性成本，被「很多次快速搜尋」攤平，非常划算。

一句話：排序成本要用「未來省下的搜尋成本」來攤還，搜尋次數越多越值得先排序。

**第 3 題（三種快速查找的前提／代價／是否保持順序）**

| 方式 | 複雜度 | 前提／代價 | 是否保持順序 |
|------|--------|-----------|-------------|
| 二分搜尋 | O(log n) | 資料必須「已排序」；若原本沒排序，得先付 O(n log n) 排序成本 | 是（建立在有序陣列上） |
| 二元搜尋樹（BST） | O(log n) | 要維護樹結構（插入/刪除時要保持排序性質，甚至需平衡） | 是（中序走訪即得有序） |
| 雜湊表 | O(1) 平均 | 要建雜湊表、額外記憶體、要處理碰撞；查找依賴雜湊函數 | 否（元素散開放，無順序） |

選擇取決於需求：要最快且不在乎順序 → 雜湊表；要快又要保持有序（能做範圍查詢、找前後鄰居）→ BST；資料已是排序陣列 → 二分搜尋；資料小或沒排序 → 線性搜尋。它們的共同思想是「**用某種預先組織（排序／建樹／雜湊）換取之後的快速查找**」。
