...
- **① 分（Divide）**：把大問題拆成幾個更小的同類子問題。→ 把一大疊鈔票分成幾小疊。
- **② 治（Conquer）**：遞迴地解決每個子問題，小到能直接解就直接解。→ 找幾個人，每人各數自己那一小疊。
- **③ 合（Combine）**：把子問題的答案合併成原問題的答案。→ 把每個人數出的數字加總起來，得到總張數。

核心精神就是「把大任務拆小、分頭處理、再把結果合併」。

**第 2 題（為什麼常達到 O(n log n)）**

關鍵在「對半分」帶來的兩件事：

- **層數是 log n**：每次把問題切成兩半，n → n/2 → n/4 → … → 1，砍 `log₂ n` 次就到底（呼應二分搜尋每次砍半）。所以整個遞迴樹只有大約 `log n` 層。
- **每層工作量是 O(n)**：雖然某一層被切成很多小塊，但同一層所有小塊「合併」的工作量加起來，總共還是處理 n 個元素，也就是每層 O(n)。

兩者相乘：`log n 層 × 每層 O(n) = O(n log n)`。這遠比樸素的 O(n²) 好，也是高效排序（[dsa-6-4]）都採用分治的原因。

**第 3 題（為什麼適合用遞迴）**

因為分治的「治」那一步，做的事情是「**對更小的同類子問題，再套用一次分治**」——這正是遞迴的定義：把問題化成「規模更小的同款問題」並呼叫自己。子問題和原問題長得一模一樣（只是變小），所以用同一個函式（呼叫自己）處理最自然。切到最小、能直接解的子問題時，就是遞迴的「基本情況」。「同類子問題 + 縮小 + 直到能直接解」這套結構天生就是遞迴，所以分治幾乎都用遞迴實作。
