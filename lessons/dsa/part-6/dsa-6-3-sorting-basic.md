...
想像你在整理手上的一副撲克牌。你把牌分成兩區：左邊是「已經排好順序的牌」，右邊是「還沒整理的牌」。你每次從右邊拿起第一張新牌，然後在左邊那疊已排好的牌裡「從右往左比」，找到它該待的位置，把比它大的牌往右挪一格騰出空位，再把這張新牌插進去。這樣左邊已排好的區塊就多了一張、仍保持有序。一張張插完，整副牌就排好了。這就是插入排序——每次把一個元素「插入」到前面已排好的部分的正確位置。

**第 2 題（為什麼都是 O(n²)——從迴圈結構）**

三個排序的共同點是都有**巢狀迴圈**（一個迴圈裡面再套一個迴圈），而且內外兩層都在掃同一份資料：

- **氣泡排序**：外層跑約 n 輪，每輪內層比較相鄰元素約 n 次 → n × n。
- **選擇排序**：外層要選 n 次最小值，每次內層掃剩下的元素找最小約 n 個 → n × n。
- **插入排序**：外層 n 個元素每個都要插入，每次內層最壞要往前比對／挪動約 n 個 → n × n。

三者都是「外層約 n 次 × 內層約 n 次」，相乘就是 O(n²)。看到「雙層迴圈掃同一份資料」，就該預期它是 O(n²)（呼應 [dsa-1-1]）。

**第 3 題（為什麼插入排序對近乎有序的資料特別快）**

因為插入排序每次插入的成本，取決於「這個元素要往前挪動幾格」。如果資料幾乎已經排好，那麼每個元素本來就離它的正確位置很近——內層 `while` 只要比對一兩次、發現前面的元素沒有比它大，就馬上停下、不用挪動。

極端情況：資料已完全排好，內層每次都「比一次就停」，總共只做約 n 次比較，退化成 **O(n)**（線性！）。這就是為什麼插入排序在「小資料」或「近乎有序」的場景其實很實用，有些高效排序在處理小區段時也會切換成插入排序。相對地，資料完全逆序時它才會退回最壞的 O(n²)。
