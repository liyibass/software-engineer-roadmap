...
把整條流程想成一條工廠自動化生產線：原料（程式碼）從一端進去，成品（上線的服務）從另一端出來，中間各站全自動、不用人工搬運。

- **CI（Continuous Integration，持續整合）** 是生產線前段的「品檢站」：每次有人 push 程式碼，自動幫你跑測試、build。它負責確認「這批新料沒把東西弄壞、能正常組裝起來」——沒通過測試就在這站被攔下來，壞的東西不會往下流。
- **CD（Continuous Deployment/Delivery，持續部署/交付）** 是生產線後段的「出貨站」：build 通過後，自動把成品送到環境上線，不用人再手動搬。

它解決手動部署的三個痛點：

1. **慢又煩**：手動每次上線都要跑一長串步驟；自動化後你只 `git push`，剩下全自動幾分鐘完成。
2. **容易出錯**：手動步驟會漏、會打錯（部署是事故大來源）；自動化每次都跑一模一樣的步驟，不會手滑。
3. **不一致**：每個人手動做的可能不一樣；自動化保證每次流程都一致。
...
從 push 到上線，典型有這幾站（對照本章的 workflow 範例）：

1. **觸發**：`git push` 到 main 分支 → 自動觸發 GitHub Actions（`on: push`）。
2. **取出程式碼**：`actions/checkout` 把 repo 的程式碼拉到 runner 上。
3. **跑測試（CI）**：`npm test`（或你的測試指令）。沒通過就停，壞的程式碼不會上線。
4. **build image、推到 ECR**：`docker build` 出 image，登入 ECR、tag、push 上去（對應 aws-7-2）。
5. **部署到 ECS/EKS**：更新 ECS 服務用新 image 做滾動部署（對應 aws-7-4），可設成金絲雀先導一小部分流量。
6. **上線完成**：新版上線，整個過程有記錄可追溯。

一句話串起來：**push → 測試 → build image → 推 ECR → 更新 ECS → 上線**，你只做了第一步，其餘全自動。
...
**為什麼把 AWS 金鑰存進 GitHub 是壞做法**：

- 那是一把**長期有效**的鑰匙，一旦外洩（不小心 commit 進程式碼、GitHub 設定失誤、第三方 Action 偷讀…），有心人就能拿去操作你的 AWS 帳號——挖礦、開一堆機器，變成 aws-1-3 講的天價帳單。
- 長期金鑰放著不動，等於一直曝露風險；就算沒外洩，也很難落實「最小權限、定期輪替」。

**比較好的方式：用 OIDC（OpenID Connect）**：

- 讓 GitHub Actions 在**執行當下**，向 AWS 出示身分，AWS 才發給它一組**臨時的、有時效的**憑證，去扮演一個預先設定好的 **IAM Role**（aws-2-1 的 Role）。
- 好處：**不用儲存任何長期金鑰**（GitHub 裡沒有可外洩的鑰匙）、憑證用完就過期、還能給這個 Role 綁**最小權限**（只給推 ECR、更新 ECS 需要的權限，aws-2-2）。

一句話：**「用 Role、臨時借權限」取代「存一把長期鑰匙」**——這正是 aws-2-1「用 Role、不用金鑰」原則在 CI/CD 上的延伸。
