...
**state 檔記錄什麼**：它記錄「Terraform 建過哪些資源、以及它們目前的狀態」——等於現實世界的一份快照（哪個 VPC、哪個子網路、它們的 id 和屬性…）。預設 `apply` 後存在本機的 `terraform.tfstate`。

**為什麼需要它**：Terraform 每次 `plan`／`apply` 的核心動作，是**比對兩邊算差異**：

- 一邊是「程式碼描述的架構」（你要的、期望狀態）。
- 一邊是「state 記錄的現實」（目前實際有什麼）。

比對後才知道「該新增／修改／刪除什麼」。state 就是 Terraform 的「**記憶**」——沒有它，Terraform 不知道現實已經有哪些資源，就無法正確算出這次該做的改動：可能重複建已存在的資源，或誤判某些東西該刪。所以 state 是它能正確運作的前提。
...
1. **不同步**：小明 `apply` 建了資源，只有他的本機 state 更新；小華的 state 還是舊的，plan 時會以為那些資源不存在，可能**重複建**或**誤判要刪除**。大家的「現實認知」對不上。
2. **互相覆蓋／衝突**：兩人同時 `apply`，各自寫自己那份 state，現實和記錄兜不攏，整個亂掉。
3. **遺失風險**：state 只存某人電腦，那台壞了或人離職，state 就沒了——Terraform 失去「記憶」，之後無法正確管理既有資源，後果嚴重。
4. **敏感資訊外流**：state 裡可能含機密（如資料庫密碼），散在各人電腦、或不小心進 Git，都不安全。
...
1. **把 state 存 S3 解決了什麼**：主要解決「共享」相關的問題——

   - **共享／同步**：團隊和 CI/CD 都讀寫「同一份」state，大家的現實認知一致，不會各拿各的舊 state。
   - **不遺失**：S3 非常耐用（11 個 9），還能開 versioning（aws-5-3）保留歷史，不再怕「某人電腦壞了 state 就沒了」。
   - **安全**：存在受控的私有、加密 S3，不散落各電腦、也不進 Git。

   但它**還沒解決「同時操作」的衝突**——這要靠 locking。

2. **State Locking（用 DynamoDB）解決什麼**：解決「兩人**同時** apply、互相覆蓋 state」的衝突。當有人在 apply，Terraform 就在 DynamoDB 記一把「鎖」，其他人要 apply 會被擋住、得等前一個人做完鎖釋放。確保「**同一時間只有一個人在改 state**」。

   **廁所門鎖比喻**：一份 state 就像一間單人廁所。有人進去用（apply）就把門反鎖，別人來只能在外面等；等他出來（apply 完成、鎖釋放），下一個人才能進去。這樣就不會兩個人同時擠進去、把事情弄得一團亂。DynamoDB 扮演的就是那個「門鎖」的角色。
