...
1. **無法重現**：正式環境是手動一步步點出來的，沒人完整記得每個設定。要在另一個 Region、另一個環境建一模一樣的，得手動重點幾百次，還可能漏、可能錯。
2. **設定漂移／不一致**：開發、測試、正式三個環境各自手動建，遲早設定不一致，出現「測試環境好好的、正式就壞」——也就是 infra Part 6-3 講的「雪花伺服器」。
3. **無法版本控制／追蹤**：誰改了哪個設定、什麼時候改、為什麼改，手動點 Console 完全沒有記錄，出事無法回溯、無法回滾。
4. **災難復原困難**：整個環境掛了要重建，手動重點幾百個設定是惡夢，又慢又容易漏。

這些其實就是 infra Part 6-3 的「設定漂移」問題在雲端的版本，而且因為雲端資源又多又複雜，問題被放得更大。
...
**IaC 怎麼解**：把整個架構（VPC、子網路、EC2、RDS…）寫成**一份程式碼**，然後：

- 開發環境：跑這份程式碼（帶 dev 參數）。
- 測試環境：跑**同一份**（帶 test 參數）。
- 正式環境：跑**同一份**（帶 prod 參數）。

因為三個環境都源自同一份程式碼，**結構完全一致**，不會漂移。要再加第四個環境？改幾個參數、跑一次就好。

**比手動好在哪**：

- **一致**：同一份程式碼建出來的環境結構一模一樣，消滅「測試好好的、正式就壞」。
- **快又可重現**：跑一次就建好幾百個資源，不用手動點幾百下、也不會漏。
- **可追蹤**：程式碼放 Git，誰改了什麼、何時、為何全有記錄，還能 review、回滾。
- **災難復原**：環境掛了，重跑一次程式碼就能分鐘級重建。
- **程式碼就是文件**：架構長什麼樣，看程式碼最準，不會像手動那樣「只在某人腦袋裡」。
...
- **設定漂移**：infra Part 6-3 講的是實體／虛擬機器手動改設定造成的漂移（雪花伺服器）；這章是同樣問題，只是主角換成 AWS 資源（手動點 Console 建的三個環境會漂移）。IaC 用「同一份程式碼」消滅漂移。
- **Ansible 與 Terraform**：兩者都是「基礎設施即代碼」的工具，精神一致——用程式碼描述基礎設施、放 Git、可重現。差別在**分工**：Ansible 偏「設定機器內部」（裝套件、改設定檔），Terraform 偏「建雲端資源」（VPC、EC2、RDS）。
- **宣告式**：兩門課都強調「描述要的最終狀態，工具自己算怎麼做」——infra Part 6-3 的「給司機目的地、而非逐步指示」，也呼應 K8s 的「期望狀態」。
- **冪等**（infra Part 6-4）：同一份程式碼跑幾次結果都一樣（已達目標狀態就不動）。所以你能放心「改程式碼、重跑」，工具只套用差異。

一句話：這章是把 infra Part 6 學的 IaC 核心概念，用在**雲端規模**上——因為雲端資源又多又複雜，這些好處威力更大。
