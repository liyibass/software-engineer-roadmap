...
- **起因**：2017 年 2 月，一位 AWS 工程師在維護 S3 系統時要下一個指令，本來只想移除「少數幾台」處理計費的伺服器，但**打錯了一個參數**，結果指令移除了「遠比預期多」的伺服器，一次砍掉 S3 一個重要子系統的大量容量。
- **怎麼擴大**：容量被砍太多，導致 S3（美東 Region）大規模故障。因為**太多服務都依賴 S3**（網站、App、甚至 AWS 自己的其他服務），S3 一掛就引發**連鎖反應**，一堆依賴它的服務跟著垮。更諷刺的是，連 AWS 自己的「服務健康狀態頁面」也依賴 S3，所以一開始連「S3 出問題了」這個狀態都顯示不出來。
- **影響多大**：大半個網際網路受影響，無數知名網站、App 出問題，持續約 **4 小時**。

三個關鍵詞串起來就是：**一個打錯的參數 → S3 大量容量被砍 → 連鎖拖垮半個網路約 4 小時**。
...
1. **人會犯錯，系統要防呆（SRE Part 5-3 無咎文化）**：真正的問題不是「那個工程師」，而是「系統竟允許一個打錯的指令造成這麼大破壞」。事後 AWS 的改善正是加上「移除容量的安全檢查、限制單次移除量」——對事不對人、修系統而非怪人。
2. **連鎖故障的威力（SRE Part 8-1）**：一個服務（S3）掛掉拖垮無數依賴它的服務。教訓是設計時要問「**我依賴的服務掛了怎麼辦**」——用降級、快取、斷路器來隔離（SRE Part 8）。
3. **監控/緊急工具不該依賴會出事的系統**：AWS 狀態頁依賴 S3，結果一起掛。監控、告警、應變工具要有**獨立性**，不能綁在「正在出事的那個系統」上。
4. **過度依賴單一點也是風險**：太多服務只用單一 Region 的 S3，所以一起遭殃。關鍵服務要考慮**跨 AZ/Region、甚至跨雲**備援（SRE Part 8-3 Multi-Region）。

（能對應到「連鎖故障 / 無咎文化 / 防呆 / 監控獨立性 / 冗餘備援」任兩個並解釋，就算答對。）
...
**對「上雲就萬無一失」的看法**：這是個危險的迷思。連 AWS 這種最強的雲端商、最厲害的工程師，都會因為一個打錯的參數釀成半個網路癱瘓 4 小時。雲端是很強的**工具**，但不是**免死金牌**——它一樣會掛。盲目相信「上雲就高枕無憂」，反而會讓你疏於為失敗做準備，真出事時毫無招架之力。

**專業工程師該有的態度**：
- **假設一切都會壞**——包含「連 AWS 都會出事」，並為此設計（SRE Part 8「為失敗而設計」、Part 1-3「擁抱風險」）。
- 可靠性不是「靠某個東西永遠不壞」，而是「**接受一切都會壞，並設計成壞了也能撐住、能快速恢復、能從中學習**」——用冗餘、降級、斷路器、Multi-AZ/Region、無咎 postmortem 這些手段。
- 對人**無咎**，對系統**嚴格**：不怪犯錯的人，而是去修「讓錯誤能造成大破壞」的系統。

一句話收尾：**雲端是工具，不是保險。怎麼用它設計出「壞了也撐得住」的系統，才是真本事。**
