...
**容器解決什麼問題**：它把「應用程式 + 它需要的整套環境」打包成一個標準盒子（image），讓應用「在哪都能跑、跑起來都一樣」——根治那句經典的「在我電腦上明明可以跑」。環境不再跟某台機器綁死，而是跟著 image 走。

**比直接跑 EC2 好在哪**（任選兩點以上）：

- **環境一致性**：環境打包進 image，開發、測試、正式到哪都一樣，不會有「設定漂移」（infra Part 6-3）；直接跑 EC2 則常常這台裝的版本跟那台不同。
- **部署更快更簡單**：拉 image 跑起來就好，不用像 EC2 那樣進機器重新裝環境、改設定。
- **一台機器能跑多個應用且互相隔離**：容器彼此隔離，不會像直接跑那樣互相干擾。
- **擴展容易**：要更多份，就用同一個 image 開更多容器；EC2 則要每台新機器重新設定。
- **啟動快**：容器秒級啟動（共用主機核心），不像開一台 EC2 要開機 + 設定。
...
`docker compose` 只顧「這一台機器」，一旦規模變成「跨多台機器」，就有一堆它管不了的問題：

- **調度（要跑在哪台）**：我有幾十台機器，這個容器該排到哪一台？哪台還有空間？compose 不知道別台的存在。
- **自我修復（掛了誰重啟）**：某個容器掛了，誰負責在（可能是另一台）機器上把它重新拉起來？
- **跨機器擴縮**：流量大了要自動多開容器，還要把它們分散到多台機器上分攤負載。
- **跨機器網路**：分散在不同機器上的容器，怎麼穩定地找到彼此、互相溝通。
- **不中斷更新**：怎麼一批一批換成新版容器，過程中服務不中斷。

這些「跨多台機器、大規模管理容器」的事，需要一個**容器編排平台（container orchestration）**，也就是 ECS / EKS，而不是單機的 `docker compose`。
...
- **單機 docker compose**：本機開發、小專案、示範環境——在「一台機器」上把 app + db + nginx 幾個容器跑起來，簡單直接。
- **容器平台 ECS/EKS**：正式環境、需要高可用、需要規模——由平台跨多台機器調度容器、掛了自動重啟、流量大自動多開、跨 AZ 高可用。

**是取代還是不同層次**：比較像**不同層次**而不是單純取代。它們解決的是不同規模的問題：compose 顧「這一台」，平台顧「一整群機器上的所有容器」。實務上你甚至常常兩者都用——用 compose 在本機開發，用 ECS/EKS 跑正式環境；同一份容器化的成果（image），本機用 compose 跑，上雲交給平台跑。所以是「隨規模需求往上換工具」，而不是誰淘汰誰。
