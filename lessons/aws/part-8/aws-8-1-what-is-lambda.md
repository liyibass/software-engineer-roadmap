...
不是，「無伺服器（Serverless）」這個詞其實有點誤導。伺服器還是存在的（那些機器都在 AWS 的機房裡實實在在跑著），只是**你不用去管它們**——AWS 全部幫你打理好了。

所以更精準的說法是「你不用管伺服器」，而不是「沒有伺服器」。

用 Lambda 時：

- **你只管**：一段「函式程式碼」——也就是「事件來了要做什麼」的業務邏輯。
- **你不用管**：機器（OS、修補更新）、容器、擴縮（有多少請求就自動起多少實例）、什麼時候啟動、什麼時候關掉……這些通通 AWS 處理。

對照本章開頭那條「你管的東西越來越少」的演進：EC2 你管整台機器，Fargate 你管容器不管機器，到 Lambda 你連容器都不用管，只剩「一個函式」。這就是這條路的終點。
...
Lambda 是按「**執行次數**」加上「**執行時間 × 記憶體**」計費——白話說就是「跑幾次、每次跑多久、配多少記憶體」，沒被觸發就完全不計費。

「閒置零成本」之所以是大優勢，是因為：沒有事件觸發時，Lambda 根本不存在、不佔任何資源，所以一毛錢都不用付。

對比 EC2 就很明顯——EC2 是「一直開著就一直計費」，就算半夜完全沒人用，機器還開著、還在跑錶。而同樣的服務放 Lambda，半夜沒人呼叫就是零成本。再加上 Lambda 每月還有前 100 萬次請求的永久免費額度（呼應 aws-1-3），對小流量服務幾乎等於免費。

**2. Lambda 反而可能比 EC2 貴的情況**

當流量「**超大、持續高頻**」時。因為 Lambda 是按次數與執行時間累加計費，如果它一整天幾乎每一刻都在被觸發、一直在跑，這些費用加起來，可能比「一台固定價格、一直開著」的 EC2 還貴。

所以判斷原則是：Lambda 適合「**流量不定、有時閒、突發性**」的工作（閒的時候不花錢賺很大）；而「**持續高頻、長時間運行**」的工作反而用 EC2 / ECS 比較划算（8-3 的選型會詳談）。
...
當一個 Lambda 很久沒被觸發（或同時湧入很多請求、需要起新的執行實例）時，AWS 得「**從頭準備執行環境**」——載入你的程式、初始化執行環境。這個準備動作會多花一點時間（大約幾百毫秒到數秒），這就是冷啟動。

相對地，如果之後持續有請求進來，AWS 會把環境「保溫」重複使用，這時就不用重新準備、直接跑，速度就快了——這叫熱啟動（warm start）。

**對兩種服務的影響**

- **偶爾才被呼叫的服務**：因為很少被觸發，環境常常已經被回收掉了，所以幾乎每次呼叫都撞上冷啟動。使用者第一次呼叫時可能會感受到「慢一下」的延遲（呼應 SRE Part 2-2 的延遲概念）。
- **延遲超敏感的服務**（例如即時互動、要求極低回應時間的）：冷啟動那幾百毫秒到數秒的額外延遲可能就是個真問題，會影響體驗甚至不可接受。

**緩解方法**：保持「預熱」（定時戳它讓環境不被回收）、使用 Provisioned Concurrency（預留並發，讓一定數量的環境常駐、隨時待命，但要額外付費）、或選啟動較快的執行環境。

一句話總結這個取捨：Lambda 用「偶爾的冷啟動延遲」這個代價，換來「不用管伺服器 + 閒置零成本」。
