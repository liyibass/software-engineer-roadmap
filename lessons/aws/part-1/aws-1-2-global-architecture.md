...
- **Region（區域）**：一個**地理區域**（像「東京」「法蘭克福」），是你選來放資源的地點——選離使用者近的比較快。
- **Availability Zone（可用區，AZ）**：一個 Region 裡面**一群獨立的機房**（有各自的電力、網路、冷卻，彼此實體隔離、又靠高速網路相連），把資源分散到多個 AZ 可以達成容錯/高可用。
- **Edge Location（邊緣節點）**：**遍布全球、離使用者超近的前哨站**，主要用來快取內容、加速存取（CloudFront CDN 就靠它）。

一個類比串起來：Region 是「城市」，AZ 是「城市裡相隔一段距離的好幾棟大樓」，Edge Location 是「遍布各地的便利商店」。
...
**風險（單點故障）**：一個 AZ 就是一群實體集中的機房。如果你把所有資源都放在同一個 AZ，那個 AZ 一出事（斷電、火災、網路中斷、天災），你的服務就**全掛**——沒有任何備援可以頂上。這就是「把所有雞蛋放在同一個籃子」。

**為什麼分散到多個 AZ（Multi-AZ）**：AWS 特意把同一 Region 的多個 AZ 做成**實體隔離、各自獨立供電與網路**，但又用高速網路互連。把資源分散到多個 AZ 後，一個 AZ 掛了，其他 AZ 的副本還在、服務繼續運作——這就是雲端「高可用」的基礎。用大樓的類比：把雞蛋分散到不同棟大樓，一棟失火，不會全毀。

這正是 SRE Part 8-3、infra Part 9-2 講的「消除單點故障、用冗餘換取可用性」在 AWS 上的落地方式，而且 AWS 讓它變得很容易——你只要把資源設定成跨多個 AZ 就好。
...
1. **選歐洲的 Region**，例如**法蘭克福（eu-central-1）**、愛爾蘭（eu-west-1）或倫敦（eu-west-2）。理由：Region 離使用者越近，連線延遲越低、越快（光速有限，資料不用繞半個地球）。如果有歐盟資料落地（GDPR）的考量，選歐盟境內的 Region 還能同時滿足法規要求。

2. **把伺服器、資料庫分散到該 Region 的多個 AZ（例如 2~3 個）**，做成 Multi-AZ。這樣任一個 AZ 機房出事，其他 AZ 還能繼續服務，達成高可用；不要把資源全塞在同一個 AZ。

3. **用 CloudFront（AWS 的 CDN）**，讓圖片、影片、CSS/JS 等靜態內容從離使用者最近的 **Edge Location** 提供。這樣即使使用者偶爾散布到其他洲，也能就近拿快取好的副本，不必大老遠回到歐洲的 Region，存取速度快很多。

一句話：**選對 Region（近＋合規）＋跨多 AZ（穩）＋用 Edge/CloudFront（全球都快）**。
