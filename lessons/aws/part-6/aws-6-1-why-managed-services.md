...
- **自己架 = 自己開伙**：食材（機器費）比較便宜、想煮什麼都行（完全掌控），但買菜、下廚、洗碗、顧火全部自己來——對應到資料庫的安裝、設定、備份、修補、高可用、故障處理，全是你的事。人力維運成本很高。
- **受管服務 = 上餐廳**：一餐比較貴（服務費較高），選項也受菜單限制（掌控度較低），但有人幫你處理一切，你只要專心吃（專心做產品）。

一句話：**受管服務用「較高的服務費 + 較低的掌控彈性」，換來「省下大量維運人力 + 更可靠的高可用」**。價值是省心、可靠、上手快；代價是貴一點、客製受限。
...
對照本章那張「AWS 幫你做 vs infra 課自己做」的表，RDS 至少幫你扛走這些：

- **自動備份**（infra 要自己寫備份腳本、還要驗證能不能還原）。
- **自動安全修補**（infra 要自己盯資料庫版本、手動更新）。
- **高可用 / 故障轉移**（infra 要自己做主從複製、冗餘架構）。
- **自動擴展**（infra 要手動加機器、擴容）。
- **監控**（infra 要自己架 Prometheus 之類的監控）。

換句話說，**RDS 把 infra 課那一大坨「維運 toil」自動化掉了**——你只要「建立資料庫、拿連線位址、連上去用」。
...
**結論：3 人團隊幾乎一定該用 RDS。** 逐項套框架：

1. **核心價值**：「維護資料庫」通常不是 3 人團隊的產品重點，把時間花在產品上更值得 → 傾向受管。
2. **人力**：3 個人根本撐不起「24 小時有人顧資料庫、半夜掛了要有人爬起來救」的維運。自己架的隱藏成本會壓垮小團隊 → 傾向受管。
3. **可靠性**：要做到真正的 Multi-AZ 高可用、可靠備份，自己土法煉鋼又慢又容易出錯，RDS 點一下就有 → 傾向受管。
4. **成本**：RDS 服務費看起來貴，但把「省下的人力 + 避免的出事損失」算進去，對小團隊通常反而更划算 → 傾向受管。

四項全指向 RDS。**唯一會考慮自己架的例外**：需要 RDS 不支援的特殊客製，或規模大到自己架明顯更省——但這都不是 3 人團隊的處境。

（這題是開放題，只要能用四個框架逐項推導、得到「用 RDS」並說明理由即可。）
