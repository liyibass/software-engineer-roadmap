...
- **通訊錄 = 資料庫**：資料完整、可靠，但每次要查都得整本翻，比較慢（要下查詢、可能還要讀硬碟）。
- **便條紙 = 快取**：把常查的幾個電話抄在便條紙上，就放桌上，一眼就看到。

**為什麼加速**：常用的資料放在「拿取超快」的記憶體（便條紙），從快取拿比從資料庫（翻通訊錄）快好幾個數量級。

**為什麼減輕資料庫負擔**：大量「重複的讀取」都被便條紙擋下來了，根本不用去翻通訊錄——資料庫收到的查詢次數大減，壓力自然變小（呼應 RDS 讀取擴展、SRE 容量）。
...
1. 應用要資料時，**先問快取**（不是直接問資料庫）。
2. **快取有**（cache hit）→ 直接把快取裡的資料回傳，超快，完全不碰資料庫。
3. **快取沒有**（cache miss）→ 才去**問資料庫**，拿到資料後**順手把它存進快取**（通常會設一個 TTL 過期時間）。
4. **下一次**要同樣的資料 → 快取就有了，走第 2 步。

一句話：**先問快取，沒有才問資料庫、並回頭把結果塞進快取**。效果是——同一份熱門資料，只有第一次會真的打到資料庫，之後都從快取拿。
...
1. **為什麼重要資料不該只存快取**：快取本質是「記憶體裡的暫存」，是「可以丟的」——它可能因為節點重啟、記憶體被淘汰（eviction）而清空。所以**永久、重要、不能遺失的資料一定要存在資料庫（RDS），快取只放「副本 / 暫存」**。這也呼應 SRE「為失敗而設計」——就算快取整個掛了，應用也要能回頭去問資料庫，而不是資料就此消失。

2. **不一致怎麼緩解**：資料庫更新了、但快取還是舊的，使用者會讀到過期資料。緩解方式：
   - 設**過期時間（TTL）**，例如「快取 5 分鐘後自動失效」，過期就重新去資料庫拿。
   - 或在**更新資料庫時，一併更新 / 清除對應的快取**。

   這是在取捨「**效能**（快取越久越省資料庫、越快）」與「**資料新鮮度**（快取越久越可能讀到舊資料）」——TTL 越長越快但越不新鮮，越短越新鮮但資料庫壓力越大，要依業務對「新鮮度」的要求拿捏。
