...
- **總倉庫 = 你的來源（Origin，例如在東京的 S3 或 ALB）**：東西最齊全，但只有一個、位置固定。全世界的人都大老遠跑去總倉庫拿，又慢又擠。
- **便利商店 = 遍布全球的 Edge Location**：CloudFront 把你的內容**快取**到離使用者很近的節點。

台灣的使用者要一張圖，不用連到東京的總倉庫，而是去**最近的便利商店（台北 Edge）**拿——距離近、延遲低，所以快。第一個人來拿時便利商店沒貨，就去總倉庫調一份放著；之後同區的人來拿就直接從便利商店給。

效果：**全球使用者都「就近」拿內容**，不管人在哪都快；同時大量請求被便利商店（Edge）擋下，總倉庫（來源）的負擔也大減。
...
1. **來源（Origin）通常是什麼**（兩個常見的）：
   - **S3 bucket**——放靜態資源（HTML/CSS/JS/圖片），對應前端靜態網站。
   - **ALB**——動態內容的入口（後面接 EC2 / 容器）。

2. **Edge 沒快取（cache miss）時**：Edge Location 會**去「來源（Origin）」拿**那份內容，回給使用者的同時**順手把它快取在 Edge**。這樣下次同一區的使用者要同樣內容，Edge 就直接給、不用再回來源——跟 ElastiCache 的 cache-aside 是同一套邏輯，只是分散在全球各節點。
...
1. **設定快取時間（TTL）**：例如「快取 24 小時」，時間到 Edge 自動回來源重新拿最新的。優點簡單；缺點是「過期前」使用者仍可能拿到舊版，且改不了「已經在 Edge 的舊快取」立刻失效。

2. **手動失效（invalidation）**：部署新版後，對指定路徑（如 `/index.html`、`/*`）發一個「失效」指令，強制 Edge 丟掉舊快取、下次請求重新回來源拿。優點是能「立刻」讓新版生效；缺點是失效請求有配額 / 費用，不適合每次都對全站 `/*` 狂刷。

3. **檔名加版本 / hash**（最推薦）：前端打包工具會把檔名做成 `app.[hash].js`，**內容一變、檔名就變**。HTML 引用的是新檔名，自然對應到一份「Edge 上還沒有的新檔案」，完全繞過舊快取；舊檔案沒人要了就讓它自然過期。這是最乾淨的做法——這也是為什麼 HTML 本身常設短 TTL 或主動失效，而帶 hash 的靜態資源可以設很長的 TTL。

實務上常見「**帶 hash 的靜態資源長快取 + HTML 短快取或部署後失效**」的組合。
