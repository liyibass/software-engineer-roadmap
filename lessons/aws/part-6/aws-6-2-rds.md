...
- **自動備份**：每天自動備份，還支援「時間點還原」——能還原到過去任意時刻，而不只是回到某個備份點。
- **自動修補**：資料庫軟體的安全更新自動幫你套用，不用自己盯版本。
- **高可用**：開 Multi-AZ 就有跨 AZ 的故障轉移。
- **監控**：內建效能指標，不用自己架監控。

你只剩下「建立資料庫、拿到連線位址、連上去用」這件事——而且記得把資料庫放在**私有子網路**，絕不對外公開。
...
1. **各解決什麼問題**：
   - **Multi-AZ** 解決「**高可用**」——怕主庫所在的 AZ 掛掉，在另一個 AZ 放一個同步的待命副本，掛了自動切過去。
   - **Read Replica** 解決「**讀取擴展**」——怕讀取量太大把主庫壓垮，開幾個讀取副本把「讀」分攤掉。

2. **副本平常有沒有在服務**：
   - Multi-AZ 的**待命副本平常完全不服務請求**，純粹待命，只為主庫掛掉時頂上。
   - Read Replica 的副本**平常就實際處理讀取請求**。

3. **備胎 vs 幫手**：Multi-AZ 是「**備胎**」——平常放後車廂不上路，爆胎（主庫掛）時才換上去；Read Replica 是「**幫手**」——平常就跟你一起分擔工作（分讀取），但它不是為了「壞掉時頂上」而存在。

補充差異：Multi-AZ 是**同步**複製（資料一致），Read Replica 是**非同步**複製（可能有極短延遲、最終一致）。
...
- **開 Multi-AZ**：主庫在 AZ-a、待命副本在 AZ-b（同步複製）。負責「高可用」——AZ-a 掛了自動切到 AZ-b，連線位址不變，使用者幾乎無感。
- **加多個 Read Replica**：例如 2~3 個讀取副本。負責「讀取擴展」——把大量「讀」的請求分散到副本。

**應用端的分流**：
- **寫入**（發文、下單、更新）→ 一律送**主庫**。
- **讀取**（看清單、看詳情）→ 分散到 **Read Replica**。

這樣「寫」由主庫穩穩處理、「讀」被副本分攤、主庫所在 AZ 掛了還有 Multi-AZ 頂上——**既高可用又能扛大量讀取**。而備份、修補、故障轉移全是 RDS 自動處理。

（開放題，只要答出「Multi-AZ 顧高可用 + Read Replica 顧讀取擴展、讀寫分流」即可。）
