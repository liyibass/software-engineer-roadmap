...
**Internet Gateway 是「正門」——雙向**。面向馬路、進出都通：外界能主動連進來（使用者連你的網站），公開區的資源也能主動連出去。公開子網路之所以公開，就是因為路由通往 IGW。

**NAT Gateway 是「側門 + 代收/代寄櫃台」——單向（只出不進）**。私有住宅的人可以透過這個櫃台把東西寄出去、收回自己寄那筆的回信，但外人不能從這個側門闖進住宅。對應到雲端：私有子網路的資源能「主動出去」（下載更新、呼叫外部 API）並收回那筆的回應，但外界無法透過 NAT 主動連進私有資源。

**關鍵方向對比**：IGW 進出雙向；NAT 只能由內往外主動發起，外界進不來。
...
1. **例子**：下載軟體 / 安全更新（`apt update`）、呼叫外部 API（金流、寄信服務）、從外部拉取資料。這些都需要私有資源「主動連出去」。
2. **不能直接走 IGW**，因為 IGW 是雙向的——一旦讓資料庫走 IGW，等於給它一條雙向對外的路，外界也能反過來連進來，它就變「公開」的、暴露了，失去躲私有的安全性。
3. **NAT 的做法**：NAT Gateway 放在公開子網路，私有資源要出門時把流量先送到 NAT，由 NAT 用自己的公開 IP「代為」連出去，回應再沿原路轉回私有資源。因為連線一律是「私有資源主動發起」的，外界無法反過來主動透過 NAT 連進私有資源——所以做到「能出去、但不能被主動連進來」。
...
**NAT Gateway 要錢，IGW 免費。**

NAT Gateway 按「開著的時間（每小時）+ 通過的流量」計費——只要它一直開著，就一直在算錢，就算你沒在用它也一樣。新手要特別注意，是因為它常被忽略：你可能只是練習時建了它，練完忘了刪，它就默默持續計費，變成帳單上「咦，這是什麼費用」的常見來源（呼應 aws-1-3 的帳單意識）。所以學習用完一定要記得把 NAT Gateway 刪掉、並釋放它綁的 Elastic IP。
