...
1. **找邊界**：VPC 在哪、CIDR 多少（4-1、4-2）。
2. **找分區**：哪些是公開子網路、哪些是私有子網路（4-3）。
3. **找 AZ**：有沒有跨多個 AZ（圖是不是左右對稱兩半），判斷高不高可用（4-7）。
4. **找入口出口**：IGW（通常在頂部）、NAT 在哪（4-4）。
5. **找資源**：每個東西（ALB、EC2、RDS）放在哪個子網路（4-3）。
6. **追流量**：使用者的請求怎麼一路從上流進去到資料庫（4-6）。

口訣：邊界 → 分區 → AZ → 入口出口 → 資源 → 追流量。
...
**問題一（安全，第②⑤步看出來）：資料庫畫在公開子網路。** 這代表資料庫直接暴露在網際網路上、外界能連到它，攻擊面極大，違反「資料庫該躲在私有子網路、外界連不到」的核心原則。這是最危險也最常見的錯誤。

**問題二（可用性，第③步看出來）：只有一個 AZ。** 沒有跨 AZ 冗餘——那個機房一旦掛掉（斷電、火災、網路中斷），整個服務就全掛，這是單點故障，沒有高可用。

**修正方向**：把資料庫移到私有子網路（外界連不到，只讓內部應用連），並把架構擴成跨至少 2 個 AZ（子網路、應用、資料庫都分散到不同 AZ，資料庫用 RDS Multi-AZ）。
...
> 這是動手畫圖題，手繪即可、沒有唯一標準答案；能標出全部組件並畫對流量方向就算過關。下面給一個示範版本（照 aws-4-8 建的 VPC）：
...
```
     ...
     ╔═════════╪═════════════════════╗ VPC 10.0.0.0/16
     ║  ┌──────┴───────────────────┐ ║
     ║  │ 公開子網路 public-1a       │ ║  ← 上半＝公開區
     ║  │  10.0.1.0/24              │ ║
     ║  │  NAT Gateway              │ ║
     ║  └──────────┬────────────────┘ ║
     ║  ┌──────────┴────────────────┐ ║
     ║  │ 私有子網路 private-1a       │ ║  ← 下半＝私有區
     ║  │  10.0.2.0/24              │ ║
     ║  │  EC2（無公開 IP，躲起來）   │ ║
     ║  └───────────────────────────┘ ║
     ╚═══════════════════════════════════╝
...
流量箭頭：
  使用者 → IGW → 公開子網路（雙向）
  私有 EC2 → NAT Gateway → IGW → 網際網路（只出不進，下載更新用）
```

**驗收自檢**：有沒有標到 VPC 框與 CIDR、公開/私有兩個子網路（各自的 CIDR）、IGW 在頂部、NAT 在公開區、EC2 在私有區？流量箭頭有沒有畫出「使用者從上往下進」和「私有 EC2 靠 NAT 主動出去」？都有就代表你真的懂了。
