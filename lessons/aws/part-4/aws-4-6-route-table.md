...
1. 連 `10.0.2.30`：它在 VPC 範圍 `10.0.0.0/16` 內，符合第一條 → 走 **local**（VPC 內部網路，資源之間直接互通）。
2. 連 `1.1.1.1`：不在 VPC 範圍內，落到 `0.0.0.0/0`（其他所有地方）→ 走 **igw-12345**（Internet Gateway，出網際網路）。
3. 因為有一條 `0.0.0.0/0 → IGW`，讓它能雙向連網際網路，所以這是**公開子網路**的路由表。
...
由**它關聯的路由表怎麼設**決定，不是一個你勾選的開關。

- 路由表有一條 `0.0.0.0/0 → Internet Gateway`（能雙向連網際網路）→ 這個子網路就是**公開**的。
- 沒有這條、或把它改成指向 NAT Gateway（只能出、不能被進）→ 外界連不到 → 這個子網路就是**私有**的。

所以「公開/私有」是「路由表怎麼設」的**結果**：把路由指向 IGW 就公開，不指或指向 NAT 就私有。
...
`0.0.0.0/0` 應該指向 **NAT Gateway**。這樣裡面的資源能「主動出去」下載更新、呼叫外部 API，但外界無法主動連進來（只出不進）——既私有又能上網，正是要的效果。

如果指向 **IGW** 會壞事：IGW 是雙向的，這條路由等於讓子網路能雙向對外、外界也能主動連進來——這個子網路就變成「公開」的了，失去私有的安全性（資料庫就暴露了）。所以要私有能上網，一定是指 NAT，不是 IGW。
