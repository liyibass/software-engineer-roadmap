...
**Security Group 像「每戶的門鎖」**：

- 層級：作用在**單個資源**（EC2、資料庫等）上，同子網路裡不同資源可掛不同 SG。
- 拒絕：**只能設「允許」規則**，沒列到的自動拒絕。
- 狀態：**有狀態（stateful）**——允許進來的連線，它的回應自動放行，不用另外設。

**NACL 像「社區的圍牆」**：

- 層級：作用在**整個子網路**，套用到子網路裡所有資源。
- 拒絕：**允許和拒絕都能設**（可做黑名單，例如擋掉某段惡意 IP）。
- 狀態：**無狀態（stateless）**——不記得連線，允許「進來」還要另外允許「回應出去」，進出要各自設。

兩層都要通過，流量才到得了資源。日常主要用 SG，NACL 通常維持預設。
...
1. **「有狀態」**就是 SG 會「記得」連線：如果你允許一個請求進來，那這個請求的「回應」會自動被允許出去，你不用為回應另外設規則。像一個聰明的門衛，記得剛剛放某人進來辦事，那人辦完要出去時他自動放行、不用再檢查。
2. **不需要**。正因為 SG 有狀態，你設了「允許 80 進來」後，伺服器對這個請求的回應會被自動放行出去，不用再多設一條出站規則。（相對地，NACL 是無狀態的，就必須另外設「允許回應出去」。）
...
把資料庫 SG 的入站規則設成：**只允許「來源 = 應用伺服器的 Security Group」、目的 port = 資料庫的 port（例如 PostgreSQL 的 5432、MySQL 的 3306）**。

關鍵是「來源」用**應用伺服器的 SG**，而不是某個 IP、更不是 `0.0.0.0/0`（所有人）。這樣的效果：

- 只有掛著「應用伺服器 SG」的機器連得到資料庫。
- 連同一個私有子網路裡的其他資源，也不能亂連資料庫。
- 比用 IP 限定更精確（機器換 IP、自動擴縮都不受影響），是最小權限的極致（aws-2-2）。

因為 SG 有狀態，只要設好入站，回應會自動放行，不用另外設出站。
