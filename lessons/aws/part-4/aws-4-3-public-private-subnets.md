...
**公開子網路**像面向馬路的「店面」——外界能直接連到它、外面也進得來（雙向），所以放需要對外服務、需要被使用者連到的資源。

**私有子網路**像社區內部的「住宅」——外界無法直接連進來（躲起來），更安全，所以放不該被外界直接碰的資源。

**關鍵差別**：能不能被網際網路「直接連到」。公開可以，私有不行。技術上這個差別，來自子網路的路由表有沒有一條通往 Internet Gateway 的路由（4-6 會講）——有就公開、沒有就私有。
...
判斷原則：問「這個資源需不需要被外界直接連到？」

1. **負載平衡器 → 公開子網路**。它要接收使用者的請求，是對外的流量入口，當然得對外。
2. **資料庫 → 私有子網路**。存最寶貴的資料，絕對不該暴露在網路上，只讓內部應用連得到。
3. **後端應用伺服器 → 私有子網路**。它不直接對外，讓公開區的負載平衡器把請求轉發進來就好，躲起來更安全。
4. **跳板機（Bastion）→ 公開子網路**。它是「唯一對外開放 SSH 的入口」，必須讓你從外面連進來，才能再從它跳進私有子網路管理私有資源。

核心原則：能躲起來的就躲起來，只有「非得對外」的（負載平衡器、跳板機）才放公開區。
...
1. **不可以**。每個子網路只屬於「一個 AZ」，一個子網路不能跨 AZ。
2. 要在**每個 AZ 都建子網路**——例如 AZ-a 建一組公開+私有、AZ-b 也建一組公開+私有，然後把資源分散到不同 AZ 的子網路（例如資料庫主節點放 AZ-a 私有、副本放 AZ-b 私有）。這樣一個 AZ 掛了，另一個 AZ 還在。
3. 因為資料庫放私有子網路後，**外界根本連不到它**，攻擊面極小，駭客連「敲門」的機會都沒有；放公開則直接暴露在網路上，任何人都能嘗試連線、暴力破解、掃漏洞攻擊，一旦被攻破所有核心資料就外洩——這是最危險也最常見的錯誤。
