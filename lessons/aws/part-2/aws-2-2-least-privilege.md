...
**最小權限原則（Principle of Least Privilege）用自己的話講：** 每個 User、每個 Role，只給它「完成該做的工作所需的『最小』權限」，不多給一點。需要讀一個 bucket，就只給「讀那一個 bucket」，不給「讀寫所有 S3」，更不給「管理整個 AWS」。

**三者共通的精神——都在假設「壞事一定會發生」，並事先把「萬一發生時的損害」壓到最小：**

- **infra Part 2-6「別用 root、用一般使用者」**：平常用權限小的一般帳號做事，就算手滑或被入侵，也動不了整台系統的要害。這其實就是「在 Linux 上落實最小權限」。
- **SRE Part 8「為失敗而設計」**：不奢望「永遠不出事」，而是預設失敗會來，先設計好「出事時怎麼把影響關在小範圍」。
- **最小權限**：不奢望「金鑰永遠不外洩、程式永遠沒 bug」，而是預先把每個身份的權限縮到剛好，讓任何一次外洩／bug／手滑能造成的破壞都很有限。

一句話收斂：**它們都是在「縮小爆炸半徑」——承認防線一定會有破口，所以先把每個破口後面能碰到的東西減到最少。**
...
`AdministratorAccess` 是「整個 AWS 的全部權限」。給一個程式這種權限，兩種情況下：

**① 金鑰外洩時：** 這個程式的金鑰等於一把「打開整間公司所有門」的萬能鑰匙。一旦外洩，駭客就能為所欲為——開一大堆 EC2 挖礦、刪光你所有資料庫和 S3、改動或關掉服務、甚至動你的帳單設定。這就是 aws-1-3 那種天價帳單慘案的劇本。

**② 程式有 bug 時：** 程式本來也許只該「讀」資料，但因為它握有全部權限，一旦有 bug（例如邏輯寫錯、把刪除當更新），它「有能力」去刪掉、覆蓋不該碰的資源，AWS 也不會攔——因為權限是開的。

**最小權限怎麼限制損害：** 如果一開始就只給這程式「它真正需要的那一點權限」（例如只能讀某一個 bucket），那麼——

- 金鑰外洩：駭客撿到也只能讀那一個 bucket，刪不了、碰不了其他資源，損害被關在很小的範圍。
- 程式有 bug：就算程式想刪東西，因為「根本沒有刪的權限」，AWS 會直接擋下來——權限的邊界成了你的安全網。

核心：**權限越小，任何一次外洩或 bug 能造成的破壞就越小。**
...
**最符合最小權限的是第 3 個：「只能寫入那一個特定資料表」的自訂 policy。**

因為這個 Lambda 的實際需求就是「寫入那一張 DynamoDB 表」，第 3 個給的權限剛好等於需求、一點都不多——這正是最小權限的定義。萬一它的憑證外洩或程式有 bug，能造成的破壞頂多是動到那一張表，其他資源都碰不到。

**另外兩個為什麼不好（範圍層層過大）：**

- **1. AdministratorAccess**：整個 AWS 的全部權限，完全失控。這個 Lambda 只需要寫一張表，卻拿到能刪光所有資料、開機器、改帳單的萬能鑰匙——出事就是災難級。**範圍大到離譜。**
- **2. AmazonDynamoDBFullAccess**：雖然收窄到「只在 DynamoDB 裡」，但仍能「讀寫你所有的 DynamoDB 資料表」。需求只是「寫一張表」，卻能動所有表、還多了根本用不到的「讀」和「刪」等權限。一旦外洩或出 bug，你其他的 DynamoDB 資料都有風險。**還是太大。**

看得出範圍是「全部 AWS → 全部 DynamoDB → 一張表的寫入」層層收窄；每收窄一層，出事時的損害就小一截，第 3 個才是「剛好夠用」。
