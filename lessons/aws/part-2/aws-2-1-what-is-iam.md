...
- **User（使用者）＝ 員工**：一個固定的「身份」，代表某個特定的人或某個特定的程式。就像每位員工手上那張「屬於自己、不會給別人」的員工識別證。
- **Group（群組）＝ 部門**：一群 User 的集合，方便「統一管權限」。就像「工程部」這個部門——你不用一個個發權限，只要把人丟進部門，整個部門的門禁就套到他身上；要調整時改部門一次就好。
- **Role（角色）＝ 臨時訪客證 / 某個職務**：一個「可以被臨時取得」的身份。它不固定屬於誰，任何被授權的人或服務，需要時可以「暫時戴上」它、拿到它的權限，用完就脫下。像櫃台那疊訪客證——誰來訪、經核准就領一張，離開時歸還。
- **Policy（政策）＝ 門禁卡上的權限清單**：定義「能進哪些門、能碰哪些東西」的規則本身。它要「附加」到 User、Group 或 Role 上才生效——就像權限清單得寫進某張卡片裡才有作用。

一句話串起來：**Policy 是權限規則，貼到 User／Group（管人）或 Role（管服務）身上才會生效。**
...
- **User** 是「固定屬於某人／某程式」的身份——它是誰的就永遠是誰的，代表一個確定的主體。
- **Role** 是「可以被『臨時取得』的身份」——它不綁定特定某個人或服務，任何被授權的對象都能暫時「變成」這個 role、拿到它的權限，用完就放下。

一句話：User 是「你本人」，Role 是「一個可以被暫時戴上的職務／訪客證」。

**2. 為什麼 EC2 用 Role 比寫死金鑰安全：**

如果在 EC2 裡「寫死一組 IAM User 的金鑰」，這組長期有效的金鑰就靜靜躺在機器上，一旦機器被入侵、程式碼被上傳到公開的 repo、或設定檔外洩，金鑰就落到別人手上——這正是 aws-1-3 那種「金鑰外洩 → 駭客開一堆機器挖礦 → 天價帳單」慘案的根源。而且金鑰還得靠人手動輪換，很容易忘記。

改用 Role 後，你把 role「指派」給 EC2，EC2 會自動「戴上」它，AWS 在背後發放**短期、會自動輪換、且不落地成一組固定金鑰**的臨時憑證。整個過程完全不需要你手上握有、也不需要寫進機器的長期金鑰，等於把「金鑰外洩」這個最大的破口直接拿掉。所以官方最佳實踐是：**給服務一個 Role，而不是給它金鑰。**
...
Lambda 是一個「AWS 服務」，不是一個「人」。回到本章的大原則：**人用 User（＋ Group）、服務用 Role、權限都由 Policy 定義。** Lambda 屬於「服務」，所以權限要透過 Role 給。

具體做法是：建一個 Role（例如 `lambda-write-dynamodb-role`），在它上面附加一份「允許寫入那一個 DynamoDB 資料表」的 policy，然後把這個 Role 指派給 Lambda 函式（在 Lambda 的執行角色設定裡選它）。Lambda 執行時就會自動「戴上」這個 role，拿到臨時憑證去寫 DynamoDB。

**為什麼不用 User？** 用 User 就得產生一組長期金鑰、再想辦法塞進 Lambda——這又回到「寫死金鑰」的老問題：金鑰可能外洩、要手動輪換，是安全大忌。用 Role 則完全不需要金鑰，AWS 自動發放會過期、會輪換的臨時憑證，安全得多。
