...
EC2（Elastic Compute Cloud，彈性運算雲）一句話就是「**在雲端租一台電腦**」——你選好規格、開機，幾分鐘後就有一台可以 SSH 進去、跑任何東西的伺服器，而且能隨時開、關、改大小，用多少付多少。

它和 infra Part 1-3 學的「雲主機」的關係是：**EC2 就是「雲主機」的代表實作**。infra 課把伺服器分成實體機、VM、雲主機三種；EC2 本質上是 AWS 幫你開的一台虛擬機（VM），跑在 AWS 的實體機上，而你只要按用量付租金，不用管底下的硬體——這正是「雲主機」的定義。所以你在 infra 學的所有 Linux 操作、架服務的知識，在 EC2 上完全適用，因為它就是一台 Linux 機器。
...
1. **AMI（機器映像）**：決定機器「裝什麼」——作業系統（Ubuntu、Amazon Linux…）和預裝的軟體。像機器的出廠範本。
2. **Instance Type（執行個體類型）**：決定機器的「規格」——幾顆 CPU、多少記憶體（例如 `t3.micro` 小、`m7g.large` 中）。選錯會不夠力或浪費錢。
3. **Key Pair（金鑰對）**：決定你「怎麼 SSH 進去」——AWS 用金鑰登入，開機時要指定一把金鑰，之後靠它連線。
4. **網路與安全群組（Security Group）**：決定機器「放在哪個網路（VPC）」以及「開放哪些 port」。Security Group 就是 EC2 的防火牆。
5. **儲存（EBS）**：決定機器的「硬碟」——多大、什麼類型。

記法：**裝什麼（AMI）、多大力（Type）、怎麼進去（Key）、放哪＋開哪些門（網路/SG）、硬碟多大（EBS）**。
...
- **停止（stop）＝關機**：機器關掉，之後可以再開回來，開機時**硬碟（EBS）上的資料都還在**。停止時不再收「運算」的費用，但**硬碟仍會小額計費**（因為資料還存著）。
- **終止（terminate）＝徹底刪除**：機器整個銷毀，連同它的預設硬碟一起消失，資料拿不回來。terminate 之後就**完全不計費**了。

一句話：stop 是「先關起來，之後要用再開」；terminate 是「這台我不要了，永久刪掉」。

**2. 該用 stop。**

因為你「下週還想繼續用這台機器」，代表你要保留機器上的環境和資料（例如你裝好的軟體、設定）。stop 只是關機，硬碟資料保留，下週再 start 就是原本那台。若用 terminate，機器和硬碟都會被刪掉，下週得整個重開重裝。代價是 stop 期間硬碟仍有小額費用，但為了保留環境是值得的。（若確定完全不用了，才選 terminate 徹底止血。）
