...
- **區塊儲存（Block）**：像一顆「裸硬碟」，掛載到某台機器上，變成那台機器的硬碟來用（AWS 對應 EBS）。
- **檔案儲存（File）**：像「網路共享資料夾」，多台機器可以一起掛載、看到同一份檔案（AWS 對應 EFS）。
- **物件儲存（Object）**：透過 API 或網址存取的「檔案倉庫」，你不掛載它，而是上傳/下載檔案（AWS 對應 S3）。

**S3 是「物件儲存」。** 記法：你不會把 S3「掛到機器上」，而是拿一個網址或用 API 去存取它，就像把東西丟進一個無限大的雲端置物櫃，再憑「取物編號（key）」取回。
...
- **Bucket（儲存桶）**：最頂層的容器，名字**全球唯一**（不是你帳號內唯一，是全 AWS 都不能重複）。你所有的物件都放在某個 bucket 裡。
- **Object（物件）**：你存進去的每一個檔案本身，例如一張 `cat.jpg`、一份 `report.pdf`。
- **Key（鍵）**：物件的「完整路徑名」，是它在 bucket 裡的唯一識別，例如 `photos/cat.jpg`。要注意 S3 其實**沒有真正的資料夾**——`photos/` 只是 key 的前綴，S3 用它「模擬」出資料夾的感覺，實際上 bucket 裡是一堆扁平的「key → object」對應。

**2. 拆解那段 Bucket Policy：**
...
用 aws-2-3 學的「Effect / Action / Resource（＋ Bucket Policy 特有的 Principal）」拆：

- **Effect: Allow** → 這是一條「允許」規則。
- **Principal: "\*"** → 對象是「任何人」（Principal 是 Bucket Policy 比 IAM Policy 多的欄位，用來指定「是誰」，`*` 代表全世界，包含未登入的匿名使用者）。
- **Action: s3:GetObject** → 允許的動作是「讀取物件」（下載檔案內容）。
- **Resource: arn:aws:s3:::你的bucket/\*** → 適用範圍是「這個 bucket 底下的所有物件」（結尾的 `/*` 代表所有 key）。

合起來就是：**「允許任何人讀取這個 bucket 裡的所有物件」**——也就是把整個 bucket 的內容公開給全世界下載。這正是 aws-1-5 為了做公開靜態網站才需要的設定。
...
**1.** S3 bucket **預設是「私有」**——剛建好時誰都連不到，只有你（或你明確授權的對象）能存取。**私有比較安全**，這也是「最小權限原則」（aws-2-2）的體現：預設關死，需要開放才開放，而不是預設全開再來補漏。

**2. 為什麼是常見事故：**

- 很多人為了「讓某個檔案能被連到」，圖方便直接把整個 bucket 設成公開，結果連帶把不該公開的機密（客戶資料、備份、金鑰）也一起攤在網路上。
- 一旦公開，任何人都能透過網址存取，而且**網路上有專門的掃描工具在找「公開的 S3 bucket」**——所以外洩往往幾小時內就被人發現。很多公司的資料外洩新聞就是這樣來的。

**怎麼避免：**

- **維持預設私有**，不要輕易全 bucket 公開。
- 保留 **Block Public Access（封鎖公開存取）** 這個總開關開著，當作防呆——就算不小心寫了公開的 Bucket Policy，它也會擋下來。
- 真的需要「讓某人存取某個檔案」時，**用 Presigned URL（aws-5-3/5-4）** 做精確、限時的授權，而不是把整個 bucket 打開。
- 要公開靜態資源時，**透過 CloudFront（6-5）** 對外，bucket 本身仍維持受控。
