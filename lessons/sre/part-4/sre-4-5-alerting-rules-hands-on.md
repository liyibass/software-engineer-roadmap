...
逐一對照這條告警的每個欄位，會發現它把整個 Part 4 濃縮進去了：

- **對症狀（4-4）** → `expr` 算的是「錯誤率」（5xx 佔總請求的比例），這是**使用者感受到的症狀**，不是 CPU、記憶體那種內部原因。所以它只在使用者真的遇到錯誤時才響。
- **不疲勞（4-2）** → `for: 5m` 要求「持續 5 分鐘」才觸發，過濾掉瞬間尖峰的雜訊；`> 0.01`（1%）閾值對照 SLO，不會動不動就響。少而精，避免告警疲勞。
- **分級（4-1）** → `severity: page` 標成「叫醒人」級。搭配後面第四步的路由，`page` 級才會打電話把人挖起來；其他級別（如 ticket）只發訊息。只有真緊急的才吵醒人。
- **有手冊（4-3）** → `annotations.runbook` 附上 runbook 連結，被叫醒的人（就算是新人）能照著一步步判斷、止血、診斷、升級，不用在慌亂中重新發明輪子。

外加它同時滿足 4-1 好告警三條件：**緊急**（錯誤持續惡化）、**可行動**（有明確問題要去查、去止血）、**真實**（直接反映使用者體驗變差）。這一條就是「對症狀、有緩衝、分級、附手冊」的完整示範。
...
**類比：** 你每月有一筆零用錢（＝這個月的錯誤預算，Part 2-4）。單純看「當前錯誤率」就像只看「現在戶頭剩多少」——它不告訴你危不危險，因為它**無法區分小問題和大災難**：錯誤率 1.1% 和 50% 都會觸發同一個「> 1%」告警，但一個是小擦傷、一個是流血不止。

**燃燒率（burn rate）聰明在哪？** 它看的不是「現在花了多少」，而是「**燒錢的速度**」——也就是照現在這個速度，多久會把整月預算燒光。這才是真正決定「有多急」的資訊：

- **燒得快 🔥🔥🔥**（例如 1 小時就燒掉一大塊預算、再幾小時預算就見底）→ 這是**大災難**，要**緊急告警、立刻叫醒人**（對應範例裡 `severity: page`）。
- **燒得慢 🔥**（例如只是持續略高，照這速度要好幾天才會超支）→ 不是急事，**做成工單、上班時間處理**就好，不用半夜挖人。

這完美實現了 Part 4-1 的「分級」理念：**用「多快會違反 SLO」來決定緊急程度**，而不是「一超過閾值就用同一種方式狂響」的一刀切。同樣是超標，暴燒才叫人、慢燒開工單——這樣既不會漏掉真災難，也不會被小問題吵到崩潰。
...
這是開放設計題，關鍵是每一條都要**對症狀、有持續時間、有分級、有 runbook**，並通過「緊急、可行動、真實」三條件。以一個「線上商店」服務、假設 SLO 為「可用性 99.9%、p95 延遲 < 500ms」為例，示範三條：

**告警 1：高錯誤率（緊急級）**
- 對什麼症狀：錯誤率（5xx 請求 / 總請求）——使用者的操作有沒有大量失敗。
- 閾值：> 1%（對照 99.9% 可用性 SLO 的容忍度）。
- 持續多久：`for: 5m`，過濾瞬間尖峰。
- 級別：`page`（叫醒人）。
- runbook：有，連到「high-error-rate」處理手冊。
- 為什麼是好告警：緊急（持續惡化）、可行動（去查去止血）、真實（使用者正遇到錯誤）。

**告警 2：延遲超標（緊急級）**
- 對什麼症狀：p95 延遲——服務快不快。
- 閾值：p95 > 500ms（對照延遲 SLO 上限）。
- 持續多久：`for: 5m`。
- 級別：`page`。
- runbook：有，連到「high-latency」手冊。
- 為什麼是好告警：直接反映使用者感受到的「變慢」，三條件都中。

**告警 3：錯誤預算快速燃燒（緊急級，進階）**
- 對什麼症狀：錯誤預算燃燒率——是不是快到會在短時間內燒光整月預算。
- 閾值：短時間（如 1 小時）窗口的燃燒率超過「快速燃燒」門檻（如 14.4 倍）。
- 持續多久：`for: 2m`（暴燒要反應快）。
- 級別：`page`。
- runbook：有，連到「budget-burn」手冊。
- 為什麼是好告警：只在「真的暴燒、值得半夜叫人」時觸發，比單純錯誤率門檻更能區分小問題與大災難。

**（可選）容量預警：磁碟即將滿（工單級）**
- 症狀外的例外：有明確趨勢、修復需要時間的原因，做成 ticket 級，隔天處理，不半夜叫人。

要避免的反例：不要設「CPU > 80%」「記憶體 > 85%」這類原因告警當緊急級——那會製造告警疲勞，把它們留在儀表板診斷用即可。
