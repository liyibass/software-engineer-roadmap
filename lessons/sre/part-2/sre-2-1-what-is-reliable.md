...
**一個具體例子**：某購物網站的伺服器 CPU、記憶體、硬碟都在正常範圍，服務程序也都活著（機器視角一切健康、監控全綠燈）。但因為某次改版帶進一個 bug，「加入購物車」的按鈕點下去後永遠回傳失敗——結果沒有任何使用者能完成結帳。

從機器視角看：一切正常。從使用者視角看：這個服務對想買東西的人來說**完全壞了**。

其他常見的例子還有：

- 資料庫沒掛，但某支 API 回傳的價格算錯了（機器活著，但結果不正確）。
- 伺服器都正常，但每個請求都要等 8 秒才回應（機器健康，但慢到沒人受得了）。
- 機器沒事，但顯示的商品庫存是三天前的舊資料（服務在跑，但資料不新鮮）。

**為什麼 SRE 堅持用使用者視角？** 因為使用者不會因為「你的 CPU 很正常」而開心——他們只在乎「我想做的事有沒有成功、快不快、對不對」。機器的健康指標只是**手段**，使用者能不能順利完成任務才是**目的**。如果只盯著機器狀態，就會出現「監控全綠、客訴卻爆量」的荒謬情況。所以可靠性一定要從「使用者實際體驗到什麼」來定義。
...
關鍵是把「順」這個模糊形容詞，從**使用者實際體驗**的角度拆成幾條**可量測**的定義。示範答案（4 條）：

- **可用（能不能播）**：99.9% 的播放請求，能成功開始播放（不會轉圈圈轉到失敗）。
- **快（開始播放要等多久）**：95% 的影片，從按下播放到畫面出現，等待時間在 2 秒內。
- **順（會不會卡頓）**：99% 的觀看時間，不發生緩衝中斷（buffering / 重新緩衝）。
- **畫質（清不清楚）**：95% 的觀看時間，能維持在 720p 以上，不因網路而掉成馬賽克。

每一條都是**可以量測、可以設目標、可以追蹤**的——這才叫把「順」翻譯成工程能處理的東西。（你的數字設多少可以不同，重點是每條都對應一個使用者真正在乎的面向，而且是具體可量的。）
...
- **SLI（Indicator，指標）**：你**實際量到**的數字——「現在幾分」。例如「這個月可用率實際做到 99.95%」。
- **SLO（Objective，目標）**：你**想達到**的目標值——「目標幾分」。例如「我們的目標是 99.9%」。
- **SLA（Agreement，合約）**：你**對客戶承諾**、沒做到要負責（通常賠錢）的底線——「低於幾分要賠錢」。例如「沒做到 99.5% 就退費」。

一句話串起來：**SLI 是「現在幾分」，SLO 是「我們的目標幾分」，SLA 是「低於幾分要賠錢」。** 三者由嚴到鬆：實際 SLI ≥ 內部 SLO ≥ 對外 SLA。
