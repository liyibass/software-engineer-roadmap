...
**SLO 是「對內」的工程目標，沒達到是「該檢討改善」（觸發 error budget policy）；SLA 是「對外」給客戶的法律合約，沒達到要「負責賠償」（退費、賠款）。**

拆成兩個關鍵差別：

- **對誰（對象）**：SLO 對**內**——是團隊給自己訂的目標；SLA 對**外**——是對客戶的正式承諾。
- **沒達到的後果**：SLO 沒達到只是「工程上該改善」，不會有人罰你；SLA 沒達到是**違約**，要付出實質代價（退費、賠款、客戶流失）。

類比：SLO 像你給自己訂的目標體重（沒達到只會自我檢討），SLA 像你跟健身房簽的保證合約（沒減到就真的要賠錢）。
...
**風險是：你失去了「安全邊際（buffer）」。** 當 SLA 和 SLO 一樣（都 99.9%）時，你的**內部警報**和**對外違約線**壓在同一個點上——這代表等你內部 SLO 亮紅燈、發現「糟了沒達到目標」的那一刻，你其實**同時就已經違反 SLA、要開始賠錢了**。完全沒有反應與補救的時間，太遲了。

**為什麼讓 SLO 比 SLA 嚴格比較安全？** 因為理想的層次是：

```
SLA（對外承諾，最鬆） < SLO（對內目標，較嚴） < SLI（實際做到，最好）
例如：SLA 99.5% < SLO 99.9% < SLI 99.95%
```

讓 SLO（99.9%）比 SLA（99.5%）**嚴格**，你就多出一段「**內部已經緊張、但還沒對外違約**」的緩衝區。當 SLO 開始亮紅燈時，離「違約賠錢」還有很大空間，你有時間從容排查、補救，在真正傷到客戶、賠出錢之前先把問題解決掉。這就是經營可靠性的安全網——你應該在「違反對客戶的承諾」之前，就先發現「沒達到內部目標」並開始行動。
...
判斷準則：**SLA 只有在「對外、有合約關係、需要對客戶正式承諾」時才需要**；其他情況有 SLO 就夠了。

1. **個人作品集網站 → 不需要 SLA，有 SLO 就夠（甚至連正式 SLO 都不太需要）。** 沒有付費客戶、沒有對誰做合約承諾，掛掉也不欠誰。頂多自己心裡有個「盡量別掛」的目標即可。

2. **賣給企業、按月收費的雲端服務 → 需要 SLA。** 這是典型的「對外、有合約關係」情境——企業客戶付錢，會要求白紙黑字的可用率承諾與未達標的賠償（例如退費）。當然，內部同時也要有一個比 SLA 更嚴格的 SLO 來留安全邊際。

3. **公司內部給員工用的請假系統 → 不需要 SLA，有 SLO 就夠。** 使用者是內部員工，沒有對外的合約關係、沒有付費客戶要賠償。訂一個合理的 SLO 來自我要求就足夠了。

**核心觀念**：SLO 是工程的核心工具，**每個正式服務都該有**；SLA 則是商業合約的產物，**只有對外承諾時才需要**。很多團隊只有 SLO 沒有 SLA，這完全正常。
