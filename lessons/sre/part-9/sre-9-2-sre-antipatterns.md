...
「有救火英雄」聽起來很棒，實際上是**團隊失敗的徵兆**，原因有三：

1. **它掩蓋了系統的脆弱**：因為「反正總有人會救」，組織就不去解決「為什麼一直要救火」的根本問題。英雄的存在，讓系統的脆弱被容忍下來，根因永遠沒人修。
2. **它製造了「人的單點故障」**：所有救火知識都在一個人腦袋裡。他一請假、一離職，團隊就崩。這違反 SRE「把知識變成 runbook、變成自動化」的精神。
3. **它會把人燒掉**：長期熬夜救火，英雄遲早 burnout、過勞離職——連那個單點都保不住。

**該表揚什麼**：與其崇拜「救火」，不如崇拜「**讓火不再發生**」。真正該被表揚的，不是熬夜搶修的人，而是「寫了自動化、修好了根因，讓這類事故不再需要人處理」的人。把英雄行為當成「系統需要改善的訊號」，而不是值得慶祝的事。
...
判斷標準：**這個指標有反映「使用者的真實體驗」嗎？還是只是個好看的大數字？**

1. 「處理了 5 億個請求」→ **虛榮指標**。數字很大很威風，但它沒告訴你這些請求有多少是「成功且夠快」的。5 億個請求裡如果一堆失敗，這個數字毫無意義。
2. 「95% 的使用者結帳在 2 秒內成功完成」→ **有意義的指標**。它直接對應使用者體驗（成功 + 夠快），這正是一個好的 SLI。
3. 「CPU 平均使用率只有 30%」→ **虛榮指標**（以「反映使用者體驗」而言）。CPU 是機器視角，健康的 CPU 不代表使用者用得好；使用者可能因為 bug 一直失敗，而 CPU 看起來很漂亮。
4. 「使用者遇到錯誤的比例是 0.05%」→ **有意義的指標**。它直接量測使用者實際遭遇的錯誤，是典型的錯誤率 SLI。

**規律**：對應「使用者實際成功/延遲/錯誤」的（2、4）是有意義的 SLI；只是「大數字」或「機器狀態」的（1、3）是虛榮指標。這呼應 Part 2-1「機器視角 vs 使用者視角」。
...
任選兩個即可，這裡把四個都列出來對照，每個都是「把手段誤當成目的」：

| 反模式 | 被誤當成目的的「手段」 | 真正的目的 |
|--------|----------------------|-----------|
| 英雄主義 | 「救火」本身 | 讓系統**不需要**救火（修根因、自動化） |
| 過度監控告警 | 「監控/告警的量」 | 快速發現**真正的**問題 |
| 虛榮指標 | 「漂亮的大數字」 | 反映使用者的**真實體驗** |
| 為可靠而可靠 | 「更多的 9」 | 剛好滿足使用者與業務需求 |

示範作答（挑英雄主義 + 為可靠而可靠）：

- **英雄主義**：把「救火、搶修」這個手段當成了值得追求的目的（還去表揚它）。但救火只是手段，真正的目的是「讓系統穩到不用救火」。當你把救火當目的，就永遠不會去修根因。
- **為可靠而可靠**：把「更多的 9」這個手段當成了目的，不計成本地往上追。但可靠性只是服務使用者與業務的工具，真正的目的是「剛好滿足使用者與業務需求」。從 99.9% 追到 99.999% 成本暴增、使用者卻無感，還可能扼殺創新。

**心法**：避開所有反模式的同一句話——時時問自己「**我做這件事，真正的目的是什麼？這樣做有達成那個目的嗎？**」不要被「看起來很努力、很專業」的表象迷惑。
