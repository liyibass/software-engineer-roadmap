...
**監控像汽車儀表板上固定的幾個錶**（時速、油量、水溫）——它只顯示「設計時就決定要顯示的東西」。你事先知道要看什麼、設好門檻（例如水溫過高就亮燈），它回答的是「**我預先知道要看的那些已知問題，發生了嗎？**」

**觀測性像車上的萬能診斷接口**——車子出了任何你當初沒預料到的怪問題，技師都能接上去，一路深入追查到底發生了什麼、為什麼。它回答的是「**到底發生了什麼、為什麼？**」，包括你事先根本沒設想過的狀況。

**為什麼複雜系統光有監控不夠**：現代系統很複雜，故障常常是「你從沒預料到的組合」。監控只能抓你「事先知道要看」的東西——但你不可能事先預設所有可能的故障。當一個你沒設過告警的怪狀況發生時，監控會一片正常、幫不上忙，這時就得靠觀測性（Metrics + Logs + Traces）讓你「事後追問當初沒預料到的問題」，一路追到根因。
...
1. **Metrics（指標）。** 「過去一週的趨勢」是隨時間變化的數值，指標便宜、可長期保存、最適合畫趨勢圖——這正是它的強項（回答「發生了什麼」）。
2. **Traces（追蹤）。** 問題是「卡在哪個服務」，需要看這一個請求穿過各服務的完整旅程、每一段各花多久，才能定位是哪一段慢——這正是追蹤的專長（回答「慢在哪一段」）。
3. **Logs（日誌）。** 要的是「具體的錯誤訊息內容」，這種一筆筆事件的細節，只有日誌記得下來（回答「細節是什麼、為什麼」）。

順帶一提，這三題剛好對應典型除錯流程的三個階段：Metrics 發現異常 → Traces 定位環節 → Logs 找到根因。
...
這是一個示範流程，重點在「由粗到細、接力縮小範圍」：

1. **Metrics 先確認並發現異常。** 打開首頁 API 的儀表板，看延遲指標——例如發現「首頁的 p95 延遲從平常的 200ms 飆到 2.5 秒」。這一步確認了「問題是真的、發生在什麼時間、影響多大」，但還不知道是哪一段造成的。

2. **Traces 定位是哪一段。** 抓一個「慢的首頁請求」的 trace，看它的瀑布圖。假設首頁會呼叫「驗證服務、使用者資料服務、推薦服務」，trace 顯示驗證 30ms、使用者資料 40ms、但**推薦服務那段花了 2.3 秒**——範圍就從「整個首頁慢」縮小到「推薦服務這一段」。

3. **Logs 找到根因。** 去翻推薦服務在那個時間點的日誌，例如看到「呼叫 ML 模型 API 逾時、重試 3 次」或「某個查詢沒用索引、掃了全表」。到這裡才真正知道「為什麼」，可以動手修了。

一句話總結：**Metrics 告訴你「哪裡不對」，Traces 告訴你「在哪一段」，Logs 告訴你「為什麼」**——三者接力，一路追到底。
