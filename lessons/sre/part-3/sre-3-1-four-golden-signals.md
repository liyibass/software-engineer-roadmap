...
四個黃金訊號是**延遲、流量、錯誤、飽和度**，各量的東西：

- **延遲（Latency）**：處理一個請求要多久（要把成功與失敗的延遲分開看，並看 p95/p99 而非平均）。
- **流量（Traffic）**：系統正承受多少需求量，對 Web 服務通常是每秒請求數（RPS）。
- **錯誤（Errors）**：失敗請求的比例，包含明顯的 5xx，也包含「回 200 但內容其實錯了」的隱性錯誤。
- **飽和度（Saturation）**：系統資源有多接近極限（CPU、記憶體、磁碟 I/O、連線數），告訴你「還能撐多久」。

一個記憶法：**前三個（延遲、流量、錯誤）反映「使用者現在的體驗」，第四個（飽和度）預測「未來會不會出事」。**
...
因為**失敗的請求常常「快速失敗」——延遲很低**。例如一個瞬間回傳 500 的請求，可能只花 20ms 就結束了。如果把成功和失敗混在一起算，這些又快又「假」的低延遲會把整體延遲數字「美化」掉。

**盲點**：你可能看到「平均延遲很低、看起來很健康」就放心了，殊不知這個低是因為一大堆請求正在快速失敗。真正該服務使用者的「成功請求」其實可能很慢，卻被大量的失敗低延遲拉低了平均值——你等於被自己的數字騙了。

所以正確做法是分開量：**成功請求的延遲**（這才代表真實的使用者體驗）、**失敗請求的延遲**（另外看，通常還搭配錯誤率一起判斷）。這也呼應了「看 p95/p99、不看平均」的教訓——平均和混算都會掩蓋真相。
...
1. **現在還沒有直接的問題。** 延遲正常、錯誤率正常，代表使用者此刻的體驗是好的——沒有人正在受影響。

2. **因為 CPU 已經 92%，離極限只剩一點點。** 飽和度是預警訊號：它告訴你「還能撐多久」。CPU 快用滿代表**再來一點流量，系統就會開始排隊、變慢，接著延遲和錯誤會一起爆**。SRE 的價值就在於「在使用者受影響之前就處理」，所以他會現在就去查（是不是有慢查詢、要不要擴容、是不是有異常流量），而不是等崩了才救火。

3. **這體現了「飽和度（Saturation）」的價值。** 它是四個訊號裡最有預警性的一個——延遲與錯誤反映「現在痛不痛」，飽和度反映「未來會不會出事」。這題正是飽和度「在事情發生前先看見」的典型場景。
