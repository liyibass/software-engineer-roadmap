...
判斷法則：**只增不減、講「累計幾次」的是 Counter；可上可下、講「此刻的瞬時值」的是 Gauge。**

1. **總請求數 → Counter。** 「累計」的量，只會往上加，永遠不會減少。
2. **目前的 CPU 使用率 → Gauge。** 是此刻的瞬時值，會上上下下（現在 45%、等一下可能 70%）。
3. **累計發生的錯誤總數 → Counter。** 一樣是「累計」，只增不減。
4. **當前資料庫連線數 → Gauge。** 「當前」的瞬時值，連線會建立也會關閉，數字可上可下。

補充：兩個 Counter（1 和 3）你都不會直接看它的值，而是用 `rate()` 看它「每秒增加多少」。
...
因為 **Counter 的絕對值本身幾乎沒有意義**——它是「從伺服器啟動到現在的累計總數」。看到「總共處理了 1500 萬個請求」，你完全不知道系統現在忙不忙、健康不健康：這 1500 萬可能是跑了三個月慢慢累積的，也可能是這一小時暴衝進來的，光看數字分不出來。而且伺服器一重啟，這個累計值就歸零了。

你真正關心的是「**速率**」——現在每秒進來幾個請求、每秒發生幾個錯誤。這才反映系統「當下的忙碌程度與變化趨勢」。`rate(http_requests_total[5m])` 就是算「過去 5 分鐘內，每秒平均增加多少」，把一個一路往上爬、看不出所以然的累計曲線，變成一條「每秒速率」的曲線——這才是流量、錯誤率這些黃金訊號真正要的東西。

（`rate()` 還會自動處理 Counter 重啟歸零的情況，不會因為重置就算出亂七八糟的負值。）
...
```promql
sum(rate(http_requests_total{path="/api/login"}[5m]))
```

拆解怎麼組出來的：

- `http_requests_total{path="/api/login"}`：先用**標籤過濾**，只挑出 `path` 是 `/api/login` 的那些時間序列。
- `rate(...[5m])`：對這個 Counter 算「過去 5 分鐘每秒增加多少」，也就是每秒請求數。
- `sum(...)`：因為 `/api/login` 底下還可能被 `method`、`status` 等標籤拆成好幾條序列（例如 GET/POST、200/500 各一條），用 `sum()` 把它們加總成一個「這個路徑的總 RPS」。

如果你想更細，例如「只看登入成功的」，可以再加標籤：`sum(rate(http_requests_total{path="/api/login", status="200"}[5m]))`。

（若不加 `sum()` 也算「有寫出速率」，但會得到依標籤拆開的多條線；題目問的是「這個路徑的每秒請求數」這一個數字，所以包一層 `sum()` 最貼切。）
