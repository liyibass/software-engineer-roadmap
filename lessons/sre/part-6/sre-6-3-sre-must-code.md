...
- **傳統維運**遇到問題 → **手動處理**。問題這次解決了，下次還會回來，於是人一直重複做同樣的事。而且系統越大、問題越多，手動負擔就線性暴增（6-1 說的最危險特徵），最後人力追不上、被自己的成功壓垮。
- **SRE** 遇到問題 → **寫程式解決**。寫一次程式把問題自動化、甚至讓它自我修復，問題就被**永久消滅**，不會再回來，而且系統再怎麼成長，這個負擔幾乎不變。

所以「會不會寫程式」決定了你會停在「手動硬撐、被 toil 越纏越緊」，還是能「用工程把問題根除」。SRE 的整套做法——消滅 toil（6-1）、往自動化高層爬（6-2）、把事故行動項目寫成防護機制（5-5）——全部都建立在「會寫程式」這個能力上。沒有它，這些都做不到，只能退回傳統維運。因此會寫程式不是 SRE 的加分項，而是它的根本分野。
...
- **應用開發者**寫的程式是「**給使用者的功能**」——實作產品要賣、要用的功能（例如購物車、貼文、金流）。
- **SRE** 寫的程式是「**讓系統可靠、讓維運自動**」——它們不是給終端使用者的功能，而是為了讓基礎設施運轉得更穩、更自動，服務的對象往往是系統本身或公司內部的開發者。

一句話：應用開發者用程式**做產品**，SRE 用程式**顧好產品運轉的環境**。

**SRE 會寫的程式類型（舉兩個即可）：**

1. **自動化腳本與工具**——部署、備份、清理、批次操作。
2. **基礎設施即代碼（IaC）**——用 Ansible、Terraform 把基礎設施寫成程式。

（其他可接受的答案：監控與告警邏輯、自我修復機制（L5）、給內部開發者的自助服務平台與工具（L4）。）
...
把產品團隊平常關心的事，套用到「給內部開發者的平台與工具」上，SRE 該關心：

- **使用者體驗**——開發者用我們的工具順不順手？部署、監控、擴容做起來痛不痛苦？（就像產品團隊關心 UX）
- **自助化程度**——能不能讓開發者自己部署、自己查狀態，而不用每次都來找 SRE 幫忙？（把工作推到 6-2 的 L4 自助服務，減少雙方的 toil）
- **工具本身的可靠性**——我們的平台夠不夠穩？連工具自己都要訂 SLO、要監控，因為它掛了會擋住所有開發者。
- **持續改進與回饋**——收集開發者的痛點，像產品迭代一樣不斷把平台做得更好用。

這正是「**平台工程（Platform Engineering）**」的精神：把維運從「幫人擦屁股」提升為「打造一個讓大家自助的優質內部平台」，用經營產品的心態去經營基礎設施。
