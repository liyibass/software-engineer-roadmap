...
Toil 的五個特徵（說出任 4 個即可）：

1. **手動**——要人親自動手去執行某個步驟、點某個按鈕。
2. **重複**——同一件事一再地做，不是一次性的。
3. **可自動化**——機器其實做得來，不需要人的判斷力。
4. **無長期價值**——做完之後服務狀態沒變更好，只是維持原狀。
5. **隨規模線性成長**——使用者或機器多一倍，這工作也多一倍。

「燒腦但有價值的架構設計」**不是 toil**。原因：雖然它很累、很花腦力，但它同時（1）**有長期價值**——設計出來的架構會讓系統長久受益；（2）**需要創造力與判斷力**，機器做不來，不符合「可自動化」；（3）**不重複**——每次設計的問題都不一樣。判斷是不是 toil 不能只看「累不累、討不討厭」，而要看那五個特徵。累但有價值、需要判斷的工作，是真正的工程，不是 toil。
...
1. **每天手動檢查並重啟掛掉的服務 → 是 toil。** 手動、重複（每天）、可自動化（機器能檢查與重啟）、無長期價值（只是維持原狀）、隨服務數成長。這正是最典型的 toil。
2. **研究一個新框架該不該採用 → 不是 toil。** 需要判斷力與評估、有長期價值（決定會影響往後的技術方向）、不重複，這是工程/決策工作。
3. **每次有新客戶就手動建立一套環境 → 是 toil。** 手動、重複、可自動化，而且會隨客戶數線性成長——客戶越多、這個負擔越重，是很危險的 toil。
4. **寫一個自動建立客戶環境的腳本 → 不是 toil。** 這正是「消滅 toil」的工程工作，有長期價值——做一次，之後每個新客戶都自動受益。注意第 3 題與第 4 題的對比：手動做是 toil（越做越多），寫工具自動化是工程（做一次永久受益）。
...
這題沒有標準答案，重點是走對「量化 → 找最大 toil → 優先自動化」的流程。示範一份盤點（比照本章範例的作法）：

```
一週工作盤點：
  手動處理密碼重設請求      8h    ✅ toil
  每次部署手動跑 checklist   6h    ✅ toil
  手動清理測試資料          3h    ✅ toil
  設計新告警系統            10h   ❌ 工程（有長期價值）
  回覆團隊技術問題          3h    ❌ 需判斷、非重複
  處理一個新型態事故         5h    ❌ 需判斷
  ───────────────────────────────
  toil = 8 + 6 + 3 = 17h / 35h ≈ 49%
```

**會優先自動化哪一個？** 挑「密碼重設」（8h，最大宗）。理由：

- 它佔的 toil 時間最多，砍掉它效益最大（一口氣少掉 8h）。
- 它高頻、純機械、完全可自動化——把它做成「自助密碼重設」（L4 自助服務），這類請求的 toil 直接歸零，不需要人介入。

原則：**先量化，再從最大的 toil 下手**，投資報酬率最高。就算你的 toil 沒超過 50%，養成「盯著最大 toil、優先消滅」的習慣一樣重要。
