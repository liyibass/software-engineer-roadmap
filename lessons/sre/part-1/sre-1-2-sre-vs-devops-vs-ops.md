...
- **傳統維運（SysAdmin）**：手動照顧機器與系統的人，特色是大量手動操作，規模一大就撐不住。
- **DevOps**：一種「打破 Dev 與 Ops 高牆、讓雙方緊密協作共同負責」的**文化與方法**，不是一個職位。
- **SRE**：Google 提出的、把 DevOps 理念**落地的一套具體工程作法**（用 SLO、error budget、toil 管理等工具實踐可靠性）。
- **Infra 工程師**：專注「基礎建設本身」——機器、網路、平台、部署，讓系統**跑得起來**。

一句話串起來：Infra 把系統建起來、傳統維運手動顧著它、DevOps 是該有的協作文化、SRE 則用工程方法讓它跑得可靠。
...
1. 因為 DevOps 講的是「開發與維運要緊密協作、共同負責整個服務生命週期」——這是**整個團隊該有的理念與做事方式**，不是某個人的職務內容。它強調文化（合作、共同負責）和實踐（自動化、CI/CD、快速回饋），但只指出「理想方向」，沒規定「具體怎麼做」。所以嚴格說「DevOps 工程師」這個職稱有點怪：你沒辦法叫一個人「去做文化」，文化是全隊一起長出來的。
2. 借用程式的比喻：**DevOps 像一個「介面（interface）」**，定義了「該達成什麼」（打破高牆、自動化、快速回饋）；**SRE 像一個「實作（implementation）」**，給出「具體怎麼做」——用 SLO 定義可靠性、用 error budget 平衡開發與穩定、用 toil 上限逼自己自動化。所以「class SRE implements DevOps」的意思是：SRE 是實現 DevOps 理想的其中一種具體、可操作的方法。
...
判斷關鍵：問題是「系統怎麼**跑起來**」（Infra）還是「系統怎麼**跑得可靠**」（SRE）。

1. **Infra**——設定新伺服器、裝環境，是把系統「建起來」，屬於基礎建設本身。
2. **SRE**——決定該保證 99.9% 還是 99.99%，是在定義「可靠性目標（SLO）」，這是用數據取捨可靠性，正是 SRE 的核心。
3. **SRE**——事後檢討報告（postmortem）是從事故中學習、提升可靠性，屬於 SRE 的事故處理範疇。
4. **Infra**——設計網路架構是規劃基礎建設，讓系統能運作。

（提醒：實務上這些界線在不同公司很模糊，小公司常一個人全包。重點不是糾結頭銜，而是分辨這兩種思維——「建起來」還是「顧可靠」。）
