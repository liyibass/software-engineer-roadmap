...
1. **準備太少（被打爆）**：資源不足以應付流量。後果是流量一來系統就被打爆——延遲飆高、錯誤率上升，服務變慢甚至整個掛掉，直接**違反 SLO**、傷害使用者體驗，嚴重時就是一場事故。
2. **準備太多（過度配置）**：資源遠超實際需求。後果是大部分時間資源都在閒置，**白白燒錢**——你付了機器的錢，卻沒有得到對應的價值。

容量規劃的目標，就是在這兩端之間找到「剛剛好」的平衡點：足以撐住需求，又不浪費錢。這正呼應了 SRE「擁抱風險、剛剛好」的精神——不是資源越多越安全，而是要找到平衡。
...
**橋的類比**：一座橋標示「載重上限 10 噸」，但它實際能承受的重量遠超過 10 噸。這個「標示上限」和「實際極限」之間的差距，就是**安全邊際（headroom）**——為了應付突發超載、材料老化、計算誤差等意外。系統的容量規劃也一樣：預測值就像「你打算讓橋承載的重量」，但你準備的資源要能扛「更多」，留一段緩衝，而不是配到剛好等於預測值。

**要留緩衝的理由（任選兩個以上）**：

1. **預測會失準**：實際流量可能比預測高——行銷比預期成功、突發爆紅。如果只配到預測值，一旦估低就被打爆。
2. **要容許故障**：如果 N 台機器剛好扛滿預測流量，那掛掉一台，剩下的就扛不住、引發雪崩。要留餘裕，讓「掛幾台也還撐得住」（冗餘）。
3. **資源不是瞬間能加的**：就算用雲端自動擴容，開機器、啟動服務也需要時間。沒有緩衝，等你擴容完，使用者早就已經受影響了。

常見做法是讓平常的資源使用率維持在某個水位以下（例如 CPU 平均不超過 60-70%），剩下的空間就是你的安全邊際。
...
現況：4 台扛 800 RPS（CPU 約 50%）。目標尖峰 2,400 RPS。

先算流量成長倍數：2,400 ÷ 800 = **3 倍**。

線性外推：4 台 × 3 = **12 台**。

（也可以用「每台的產能」算：800 RPS ÷ 4 台 = 200 RPS/台；2,400 ÷ 200 = 12 台，結果相同。）

**2. 加上 25% 安全邊際後是幾台？**

12 台 × 1.25 = **15 台**。

所以要準備約 15 台，才能在撐住 2,400 RPS 之餘，留下應付「預測失準、容許故障、擴容空窗」的緩衝。

**3. 預先配置還是自動擴縮？**

這題是開放性的，重點在理由。示範答案：

- 如果這個 API 的流量**波動很大**（白天高、半夜低，或偶有活動尖峰），選 **自動擴縮**——平常用少少機器省錢，流量來了自動加到上限（可把 15 台設成擴容上限），退了自動減，最符合「剛剛好」。前提是應用要設計成無狀態。
- 如果流量**穩定可預測**、或身處**自架環境**沒有彈性擴容能力，選 **預先配置**——直接備好 15 台固定資源，簡單可靠。

多數雲端服務會選自動擴縮，但無論哪種，都仍需先做這個容量規劃來決定「上限要設幾台」。
