...
**橋樑的類比**：一座新橋在通車前，工程師會開一堆滿載的卡車上去壓，主動測試它到底能不能承重——這是在**安全、可控**的條件下驗證。沒有人會直接開放通車，然後「等真實車流來幫忙測試」承載力，因為萬一橋撐不住，代價是人命。

**對應到系統**：容量規劃只是紙上計算，系統實不實際扛得住是另一回事。你有兩種方式知道：

1. **等真實流量來測**（例如等雙 11 當天）——一旦估錯，就是一場真實的大事故，使用者直接受害，而且是在你最措手不及的時刻爆發。
2. **自己先模擬大量流量壓測**——在安全的測試環境主動把系統壓爆，提前發現問題、從容修正，還能反覆重測驗證。

SRE 當然選第二種：**不拿真實使用者的體驗（甚至公司的信譽）去賭一次沒驗證過的容量估算。** 主動壓測讓你在可控環境中先看到真相，而不是被動地讓事故來告訴你。
...
1. **「我們的系統極限到底在哪？」→ 壓力測試（Stress）**
   一直加壓直到系統崩潰，就是為了找出「天花板在哪、崩潰時怎麼崩」。

2. **「秒殺活動開始那一瞬間的暴衝撐得住嗎？」→ 尖峰測試（Spike）**
   模擬流量「瞬間暴衝」的情境，正是秒殺、搶票這類突發爆量的特徵。

3. **「連續跑一週會不會有記憶體洩漏？」→ 浸泡測試（Soak）**
   用中等流量「持續很久」，才能讓記憶體洩漏、連線沒釋放這種「跑很久才浮現」的問題現形。

4. **「預估的雙 11 尖峰扛得住嗎？」→ 負載測試（Load）**
   灌到「預期的尖峰流量」，驗證正常尖峰扛不扛得住。
...
瓶頸在**資料庫**。因為當延遲飆高時，監控顯示「應用伺服器 CPU 只有 40%（還很閒），但資料庫 CPU 達 98%（已經滿載）」。先到極限的那個資源就是瓶頸——這裡明顯是資料庫，不是應用伺服器。

**2.「再多加幾台應用伺服器」能解決問題嗎？為什麼？**

**不能。** 應用伺服器 CPU 才 40%，代表它根本沒滿、不是它撐不住。瓶頸卡在資料庫，你再加多少台應用伺服器，這些新機器最後還是全部去打**同一個**資料庫，只會讓已經 98% 的資料庫更慘。加錯地方的資源，不會解決真正的瓶頸（反而可能更快壓垮下游）。

**3. 你會往哪個方向優化？**

朝**資料庫**下手，例如：

- **優化查詢**：找出拖慢資料庫的慢查詢，加索引、改寫 SQL、減少不必要的查詢。
- **加讀取副本（read replica）**：把大量讀取分散到副本，減輕主庫壓力。
- **加快取**：用 Redis 等快取擋掉重複的資料庫讀取（見本章課外讀物）。

核心觀念：**優化要對準真正的瓶頸**，而不是盲目地加你「以為」有問題的那一層。這正是負載測試最大的價值——它讓你在事故發生前就看清「該優化哪裡」。
