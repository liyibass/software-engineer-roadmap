...
1. **偵測（Detect）**：先知道「出事了」。你連問題都還沒發現，什麼都免談，所以這是起點。理想上是告警先發現，而不是等使用者來罵——偵測得越早，後面每一步都越省時間、影響越小。
2. **止血（Mitigate）**：先用最快的手段讓使用者不再受影響（哪怕只是臨時手段）。它排在修復之前，是因為使用者的痛苦每一秒都在累積，得先把痛止住。
3. **修復（Fix）**：使用者已經沒事了，才從容地找出根本原因、真正解決。
4. **復原（Recover）**：確認服務完全恢復、清掉臨時手段、整理時間軸，為之後的 postmortem 做準備。

順序的邏輯是：**沒偵測就不知道要做什麼；使用者在受苦時，止血比查原因急；根因要在使用者已安全、沒有壓力時才慢慢查；最後收尾與記錄。** 簡單說就是「先知道 → 先止痛 → 再治本 → 再收尾」。
...
**類比**：病人大出血送進急診，醫生會先止血、穩住生命徵象，而不是先花時間研究「他到底為什麼會受傷」。因為命先保住最重要，受傷原因可以之後慢慢查。系統事故一樣——**先用最快的手段讓使用者恢復（回滾、重啟、擴容、關功能），再從容找根因。**

關鍵在於：常見的止血手段（例如回滾到上一版）**根本不需要知道確切的 bug 是什麼**，就能讓使用者先恢復。所以「查根因」這件事，完全可以延到使用者已經沒事之後再做，沒有理由讓它擋在前面。

**如果反過來（先找根因再處理）會怎樣**：你埋頭研究「為什麼壞掉」的那 30 分鐘裡，使用者一直在受苦、營收一直在流失、老闆和客服一直在炸。而且高壓之下查根因又慢又容易出錯。等於用「使用者持續受害」的代價，去換一個其實可以晚點再做的事——這就是本末倒置。正確順序永遠是：**先止血讓使用者恢復，再回頭慢慢查為什麼。**
...
1. **剛上線新版本，錯誤率暴增** → **回滾（rollback）**，退回上一個正常版本。時間點高度吻合「剛上線就出事」，退回去就能讓使用者立刻恢復，完全不需要先知道新版本裡確切是哪個 bug。
2. **流量暴增 10 倍撐不住** → **擴容**，加機器分攤流量。問題不是程式壞了，而是量太大撐不住，所以加資源是最直接的止血；必要時也可搭配限流保住核心功能。
3. **某個新功能一直出錯、其他功能正常** → **關閉問題功能（feature flag）**，先把那個出包的新功能關掉。這樣壞的部分停止影響使用者，其他正常功能不受牽連，之後再從容修那個功能。

共通點：這三招都**不需要先查出根因**就能讓使用者先恢復——這正是「止血優先」的精神。
