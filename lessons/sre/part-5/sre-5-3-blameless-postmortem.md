...
**核心信念**：**人永遠會犯錯，但系統可以被設計成「即使有人犯錯也不會釀災」。**

把焦點放在系統，是因為：如果一個人犯的錯，居然能造成這麼大的事故，那真正的問題是「**系統允許這個錯誤造成災難**」，而不是那個人手滑。以「工程師打錯指令刪掉正式資料庫」為例——與其問「是誰害的」，該問的是：為什麼一個手滑的指令就能刪掉正式庫？為什麼沒有二次確認？為什麼沒有權限隔離？為什麼備份沒驗證過還原？

處分那個人，下一個人照樣可能手滑，問題原封不動；但把系統補上防呆（確認、權限、備份驗證），就算再有人犯同樣的錯也不會釀災。**「人」是修不完、也改不掉會犯錯這件事的；「系統」才是真正能修、能一勞永逸的東西。** 這就是無咎檢討把焦點放在系統的原因。
...
因為懲罰犯錯的人，教會大家的**不是「別犯錯」（人本來就會犯錯，這學不會），而是「別讓人知道我犯錯」。** 於是形成一條惡性循環：

**怪罪文化 → 人開始隱瞞錯誤（「說出來會被罰，那我藏起來」）→ 組織拿不到真相、學不到教訓 → 同樣的問題反覆發生 → 系統越來越危險。**

具體會發生：出了事，第一反應是掩蓋、甩鍋，而不是趕快通報處理；沒有人敢誠實說「我做了什麼」，事故真相永遠拼不完整，也就修不到真正的根因；大家還會因為怕出錯被罰而不敢嘗試、不敢創新，組織停滯。

反過來，**無咎文化**讓人敢誠實說出「我下了這個指令、以為會 X 結果是 Y」。只有大家敢說真話，你才能拿到完整真相、才能真正把系統修好。所以——**安全的系統，是建立在「人敢於誠實面對錯誤」之上的**；懲罰會摧毀這份誠實，因此讓系統長期更不安全。
...
（答案不唯一，重點是一路往「系統層面」挖，而不是停在表面或指向某個人。）

```
使用者付了錢但訂單沒成立。
  為什麼？→ 金流已扣款，但建立訂單的那一步失敗了。
  為什麼建立訂單會失敗？→ 扣款成功後，系統呼叫訂單服務時逾時，就直接放棄了。
  為什麼逾時就直接放棄、沒有補救？→ 這段流程沒有重試機制，也沒有「扣款與建單」的一致性保障。
  為什麼沒有一致性保障？→ 當初把「扣款」和「建單」設計成兩個獨立步驟，中間沒有交易補償或對帳機制。
  為什麼沒有對帳機制？→ 沒有針對「付款成功但訂單未成立」這種不一致狀態做設計，也沒有事後偵測它的機制。
```

**根因（對事不對人）**：

> 付款與建立訂單被設計成兩個彼此獨立、缺乏一致性保障的步驟，當建單環節逾時失敗時，既沒有重試/補償機制，也沒有偵測「已扣款但無訂單」不一致狀態的對帳機制，使得使用者可能付了錢卻拿不到訂單。

注意這句話全程沒提「是誰寫的」「誰沒測到」，而是聚焦在**系統設計的缺陷**上——那才是真正能修的東西（例如：加重試、加對帳/補償、加不一致告警）。
