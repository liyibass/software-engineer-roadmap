# [1-1] 電腦只懂兩件事：資料與指令

> **本章目標**：理解所有程式的本質都是「資料」和「指令」的組合，建立看程式碼的基本框架。

## 你會學到

- 什麼是資料（Data）、什麼是指令（Instruction）
- 用食譜類比理解這兩個概念
- 在程式碼裡，資料和指令長什麼樣子
- 為什麼連 Instagram、Netflix 這種龐大系統，也只是這兩件事的組合

## 概念說明

### 先從一個問題開始

你有沒有想過，電腦到底在做什麼？

它看起來能做好多事：播音樂、顯示網頁、計算數字、傳訊息……但如果你剝掉所有外表，電腦其實只懂兩件事：

1. **記住東西**（資料）
2. **對東西做事**（指令）

就這樣。沒有別的了。

---

### 用食譜來理解

想像你手上有一份食譜：

```
【材料】
- 水 200ml
- 茶葉 5g
- 糖 10g

【步驟】
1. 把水燒開
2. 放入茶葉，等 3 分鐘
3. 過濾茶葉
4. 加入糖，攪拌均勻
```

這份食譜裡有兩種東西：

| 食譜中的東西 | 對應到電腦的概念 |
|------------|---------------|
| 材料列表（水、茶葉、糖） | **資料**（Data）— 電腦記住的東西 |
| 烹飪步驟（燒開、過濾、攪拌） | **指令**（Instruction）— 電腦要做的事 |

這個比喻不是亂講的——程式設計師真的就是這樣想事情的。

---

### 資料是什麼？

**資料（Data）**就是電腦存在記憶體裡、需要用到的東西。

它可以是：
- 數字（`42`、`3.14`、`-7`）
- 文字（`"你好"`、`"password123"`）
- 是非值（`true` 或 `false`）
- 圖片、影音……（本質上還是數字，只是很多很多個）

在程式碼裡，資料通常長這樣：

```
水量 = 200
茶葉重量 = 5
糖的重量 = 10
使用者姓名 = "小明"
是否已登入 = 是
```

---

### 指令是什麼？

**指令（Instruction）**就是你叫電腦去做的動作。

它可以是：
- 計算（`加`、`減`、`乘`、`除`）
- 比較（`是否相等`、`是否大於`）
- 顯示（`把這段文字印出來`）
- 傳送（`把這個請求送到伺服器`）
- 儲存（`把這份資料寫進資料庫`）

在程式碼裡，指令通常長這樣：

```
燒開水
把茶葉放進去
等待 3 分鐘
過濾掉茶葉
加入糖，攪拌
```

---

### 用 Pseudo Code 組合起來

**Pseudo Code**（虛擬碼）是一種用接近人類語言寫出來的「假程式碼」，讓你先把想法寫清楚，不用擔心語法。

泡茶的 pseudo code 長這樣：

```
// 資料（材料）
水量 = 200
茶葉重量 = 5
糖的重量 = 10

// 指令（步驟）
加熱(水量)
放入(茶葉重量)
等待(3 分鐘)
過濾()
加入(糖的重量)
攪拌()
輸出("茶泡好了！")
```

看出來了嗎？上面定義的資料，在指令裡被使用了。**資料和指令不是分開的——指令操作資料，資料被指令改變。**

---

### 視覺化：資料與指令的關係

```mermaid
graph LR
    A["資料<br/>（電腦記住的東西）"]
    B["指令<br/>（電腦要做的事）"]
    C["結果<br/>（新的資料或輸出）"]

    A -->|"指令讀取並操作資料"| B
    B -->|"產生"| C
    C -->|"可能成為下一個"| A
```

這張圖說明一件很重要的事：**指令處理資料，產生結果，結果又可能成為下一個輸入**。這個循環就是程式在做的事。

## 程式碼範例

### 用 JavaScript 寫出泡茶程式

下面這段程式碼把剛才的 pseudo code「翻譯」成真正的 JavaScript。先看看，感受一下資料和指令的分界線在哪裡。

```javascript
// === 資料區 ===
// 這些是我們需要「記住」的東西
const waterAmount = 200;       // 水量（毫升）
const teaAmount = 5;           // 茶葉重量（克）
const sugarAmount = 10;        // 糖的重量（克）
const brewingMinutes = 3;      // 泡茶時間（分鐘）

// === 指令區 ===
// 這些是我們叫電腦「去做」的事
console.log(`加熱 ${waterAmount}ml 的水`);
console.log(`放入 ${teaAmount}g 茶葉`);
console.log(`等待 ${brewingMinutes} 分鐘`);
console.log("過濾茶葉");
console.log(`加入 ${sugarAmount}g 糖，攪拌均勻`);
console.log("完成！一杯茶泡好了 ☕");
```

執行結果會長這樣：
```
加熱 200ml 的水
放入 5g 茶葉
等待 3 分鐘
過濾茶葉
加入 10g 糖，攪拌均勻
完成！一杯茶泡好了 ☕
```

注意看：`const waterAmount = 200` 是在定義**資料**；`console.log(...)` 是在下**指令**（叫電腦把文字印到螢幕上）。

---

### 在更複雜的例子裡也一樣

你可能覺得：「泡茶這麼簡單，當然只有資料和指令。Instagram 那麼複雜不一樣吧？」

不一樣嗎？看看 Instagram 的核心：

```
// 資料
使用者資訊 = { 名字: "小明", 追蹤者數量: 1234 }
照片列表 = [照片1, 照片2, 照片3, ...]
貼文內容 = "今天天氣好！"

// 指令
取得使用者的追蹤清單()
從追蹤清單中找出最新的貼文()
按照時間排序(貼文列表)
顯示在畫面上(排序後的貼文)
如果 使用者按下愛心：
    把愛心數量加 1
    通知貼文作者()
```

本質上還是一樣的東西——只是資料更複雜、指令更多，而且指令裡面還有很多層指令。

**這就是程式設計的奧祕**：把複雜的問題，拆解成資料和指令的組合，一層一層疊起來。

## 小練習

### 練習 1：分析 ATM 提款

想像你走到 ATM 前要提款，用以下框架分析這個過程：

1. **資料**：ATM 需要記住哪些東西？（提示：帳戶、餘額、密碼……）
2. **指令**：ATM 需要做哪些事？（提示：驗證密碼、扣款……）

試著用 pseudo code 的格式列出來。

<details>
<summary>參考解答</summary>

這題沒有標準答案，重點是你能把 ATM 的世界拆成「它要記住什麼（資料）」和「它要做什麼（指令）」。下面是一個示範：

```
// === 資料（ATM 需要記住的東西）===
插入的卡片帳號 = "1234-5678-9012"
使用者輸入的密碼 = "0000"
帳戶正確密碼 = "1234"
帳戶餘額 = 50000
使用者要提領的金額 = 2000
密碼錯誤次數 = 0

// === 指令（ATM 要做的動作）===
讀取卡片帳號()
要求使用者輸入密碼()
驗證(使用者輸入的密碼 是否等於 帳戶正確密碼)
如果 密碼正確：
    要求使用者輸入提領金額()
    檢查(帳戶餘額 是否 >= 要提領的金額)
    如果 餘額足夠：
        帳戶餘額 = 帳戶餘額 - 要提領的金額   // 扣款
        吐鈔(要提領的金額)
        列印收據()
    否則：
        顯示("餘額不足")
否則：
    密碼錯誤次數 = 密碼錯誤次數 + 1
    顯示("密碼錯誤")
```

判斷重點：像「餘額」「密碼」「要提領的金額」這種**被記住的名詞**通常是資料；像「驗證」「扣款」「吐鈔」這種**動作（動詞）**就是指令。注意「帳戶餘額 = 帳戶餘額 - 要提領的金額」這一行——指令（扣款動作）正在改變資料（餘額），這就是第 122 行說的「指令操作資料，資料被指令改變」。

</details>

---

### 練習 2：分析 Google 搜尋

當你在 Google 搜尋「貓咪影片」時，試著想想：

1. 你輸入的搜尋詞是**資料**還是**指令**？
2. Google 回傳給你的搜尋結果是**資料**還是**指令**？
3. 「把搜尋詞跟資料庫比對」是**資料**還是**指令**？

> 提示：這個問題沒有絕對的對錯，重點是你有沒有開始用「資料/指令」的眼光看事情。

<details>
<summary>參考解答</summary>

1. **你輸入的搜尋詞「貓咪影片」是資料。** 它是你交給 Google 記住、拿去處理的東西（一段文字），本身不是動作。

2. **回傳給你的搜尋結果是資料。** 那一串網頁標題、連結、摘要，都是 Google 產生給你看的東西。有趣的是：它是 Google「執行搜尋這個指令」之後產生的**結果資料**——正好呼應第 139 行那張圖：指令處理資料，產生新的資料。

3. **「把搜尋詞跟資料庫比對」是指令。** 「比對」是一個動作（動詞），是叫電腦去做的事。它會用到兩份資料（你的搜尋詞、資料庫的內容），但比對這個行為本身是指令。

一個好用的判斷法：**是名詞（被記住、被處理的東西）→ 偏資料；是動詞（要做的動作）→ 偏指令。** 這題的重點不是背答案，而是開始習慣用這副眼鏡看每一個你用的產品。

</details>

---

### 練習 3：動手跑程式碼

把上面泡茶的 JavaScript 程式碼複製下來，貼到一個叫 `tea.js` 的檔案裡，然後在終端機執行：

```bash
node tea.js
```

接著試試看：
- 把 `waterAmount` 改成 `500`，再跑一次，結果有什麼不同？
- 加一行 `console.log(`溫度設定為 100 度`);`，放在適當的位置

<details>
<summary>參考解答</summary>

這題要動手實作，需要你自己在電腦上跑跑看驗證。做法與驗收點：

**做法：**

1. 先確認電腦裝了 Node.js（在終端機打 `node -v`，有印出版本號就代表有裝）。
2. 新建一個檔案 `tea.js`，把章節裡那段泡茶程式碼貼進去。
3. 在該檔案所在的資料夾打開終端機，執行 `node tea.js`。

**驗收點 1（改水量）：** 把 `const waterAmount = 200;` 改成 `const waterAmount = 500;` 後再跑一次，第一行輸出會從 `加熱 200ml 的水` 變成 `加熱 500ml 的水`。原因：程式印出的那行用了 `${waterAmount}`，會即時代入這個資料現在的值——改了資料，輸出就跟著變。

**驗收點 2（加一行）：** 加上溫度那行時，記得放在「加熱水」的相關步驟附近才符合邏輯。範例：

```javascript
console.log(`加熱 ${waterAmount}ml 的水`);
console.log(`溫度設定為 100 度`);   // 新加的這行
console.log(`放入 ${teaAmount}g 茶葉`);
```

再跑一次，輸出應該會在「加熱」那行的下面多出一行 `溫度設定為 100 度`。

> 這題需要你自行在實機上執行確認，看到的輸出要和上面描述一致才算通過。

</details>

## 課外讀物

> 你有沒有好奇過，為什麼程式裡的陣列第一個位置是 `[0]` 而不是 `[1]`？這跟資料在記憶體裡的儲存方式有關 → [課外讀物 E-5-1：為什麼陣列從 0 開始？](../../../課外讀物/E-5-fun-facts/E-5-1-why-arrays-start-at-zero.md)
