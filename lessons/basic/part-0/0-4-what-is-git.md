...
這三題是思考題，答案因人而異，下面提供示範思路。

**第 1 題：你遇過的命名地獄**

大部分人都有過，示範一個常見情境：學校報告改到最後變成 `報告_final.docx`、`報告_final_真的最終.docx`、`報告_final_老師改.docx`⋯⋯當時的「解法」通常是——靠檔名後綴、修改日期、或自己的記憶來猜哪個是最新的。問題是這很容易出錯：一個不小心就覆蓋掉、或交出了舊版本。這正是 Git 想解決的痛點：不用再靠複製改名，Git 幫你記住每一個版本、每一次改了什麼。

**第 2 題：兩個人一起寫作業文件（不能用 Google Docs）**

常見分工是：各寫各的段落，最後再想辦法合併成一份。會遇到的問題：

- 兩個人同時改同一段，合併時一方的內容被另一方覆蓋掉。
- 拿到對方傳來的檔案，卻不知道他到底改了哪裡。
- 傳來傳去之後，沒人確定哪一份才是最新、最完整的版本。

Git 怎麼幫忙：兩人各自 `commit`（存快照），Git 會記錄**誰、在什麼時候、改了哪幾行**；合併時它能自動把兩人的改動兜在一起，只有在「改到同一行」時才會跳出衝突提示，讓你手動決定要保留哪個版本——而不是默默覆蓋掉。

**第 3 題：Git 怎麼做到「回到任何存檔點」？**

猜測方向（下一章會正式揭曉）：Git 每次 `commit` 時，其實是把**那個時間點所有檔案的完整狀態（快照）**記錄下來，連同「說明訊息、時間、上一個 commit 是誰」一起，存在專案裡一個隱藏的 `.git` 資料夾中。因為每個存檔點的內容都被完整保存著，要回到哪個點，Git 就把你的檔案還原成那個快照的樣子。換句話說，「回到過去」不是魔法，而是因為過去的每一版都被好好收在 `.git` 裡沒有丟掉。
