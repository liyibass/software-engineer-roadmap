...
瀏覽器不能直接執行 `.ts` 檔，是因為**瀏覽器只懂 JavaScript，完全不認得 TypeScript 的型別語法**（像 `const todos: Todo[] = []` 裡的 `: Todo[]`）。

中間少了的步驟是「**編譯（compile）**」——把 `.ts` 翻譯成 `.js`：拿掉所有型別註解，變成純 JavaScript，瀏覽器才看得懂。V1~V3 我們是用 `tsc` 手動做這一步，之後 Vite 會自動幫我們做。

**練習 2**（示範答案）

以「改一行程式碼要做的事」為例，最麻煩的通常是：**每次存檔後都得手動切到終端機跑編譯、再切回瀏覽器按重新整理**。

對照本章四個痛點，它屬於**痛點三：改一行就要手動重整**。解法是打包工具的「開發伺服器 + HMR（熱模組替換）」——存檔後瀏覽器自動更新，省掉手動那幾步。

（如果你選的是「多檔案 import 設定很麻煩」那就是痛點二；「切換開發／上線版本很麻煩」是痛點四。只要能對上其中一個並說出打包工具怎麼解，就算答對。）

**練習 3**（示範答案，需自行上網查證）

- **Webpack** 宣稱的優點：生態系成熟、外掛（plugin/loader）非常多，幾乎任何資源（圖片、CSS、字型）都能透過 loader 納入打包流程，客製化能力強、社群資源豐富。
- **Vite** 宣稱的優點：開發伺服器啟動極快（利用瀏覽器原生 ES modules，開發時不用先打包整包），且設定少、開箱即用、原生支援 TypeScript。

這題重點不是背這些優點，而是體會到「打包工具不只一種、各有取捨」，你將來能依專案需求選工具，而不是以為只有一個標準答案。
