...
1. 用指紋解鎖手機 → **認證**。指紋是在證明「你就是這支手機的主人」，回答的是「你是誰」。
2. 免費會員不能下載高畫質影片 → **授權**。你已經是登入的會員（身分確認過了），系統是在判斷「你這個等級能不能做這件事」，回答的是「你能做什麼」。
3. 輸入 Email 和密碼登入 → **認證**。用帳密證明「我就是這個帳號的本人」。
4. 一般員工看不到財務報表 → **授權**。員工身分沒問題（認證過了），但他的角色沒有看財務報表的權限。

小訣竅：凡是「證明我是誰」都是認證；凡是「我這個身分能不能做某事」都是授權。

**練習 2**

- **401 Unauthorized**：其實是「未認證」——你根本還沒證明身分（沒登入、或 token 無效／過期）。伺服器在說「我不知道你是誰」。
- **403 Forbidden**：「已認證，但沒授權」——你證明了身分沒錯，但你沒有做這件事的權限。伺服器在說「我知道你是誰，但這件事不准你做」。

生活例子：你去公司大樓，**刷卡進不了大門**（卡沒感應到、身分沒驗過）＝ 401；你**刷卡進了大門，但走到高階主管的辦公室門口刷不開**（身分有效，只是你的權限進不了那間）＝ 403。

**練習 3**

會變簡單一點，但也帶來新問題，屬於「拿一個麻煩換另一個麻煩」。

簡單的地方：如果後端天生就「記得」每個使用者，那登入一次之後，後端每次都認得你，就不必費心設計「發通行證、每次帶通行證」這整套機制了。

但新問題更棘手：
1. **記憶體壓力**：後端要幫「每一個在線使用者」都存一份狀態。使用者一多，伺服器記憶體就被登入狀態塞爆。
2. **多台伺服器就亂套**：正式服務通常有很多台後端伺服器分攤流量。如果「你的登入狀態」只記在第 1 台的記憶體裡，下一個請求被分到第 2 台時，第 2 台根本不認得你，又要你重登。要解決就得再搭一個共用的地方（例如集中式 session 儲存）來同步狀態，複雜度反而上升。

這正是為什麼「無狀態」雖然逼我們設計通行證機制，卻換來了「後端不用記狀態、天生好水平擴展」的大好處——也是這門課選 Token（JWT）的理由。
...
> 認證、授權都是 Web 安全的一環，想先建立整體的安全觀 → [課外讀物 E-10-1：Web 安全總覽 — OWASP Top 10](../../../課外讀物/E-10-security/E-10-1-web-security-overview.md)
