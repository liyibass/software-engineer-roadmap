...
做法：打開 Chrome（或任何瀏覽器）→ 按 F12 → 切到 Network 頁籤 → 重新整理頁面 → 點第一筆請求（通常就是那個網頁本身的 HTML 文件）。

驗收點：在右側 Headers 面板你會看到類似這樣的資訊——

- **方法**：載入網頁那筆通常是 `GET`（因為你只是「拿」網頁，沒有送資料上去）。
- **狀態碼**：正常情況是 `200 OK`。
- **開頭數字代表成功還失敗**：`2` 開頭 → 成功。所以 `200` 是「順利拿到了」。

（每個網站看到的實際數字會不同，這題重點是你能在畫面上「指出」方法和狀態碼在哪、並用開頭數字判斷成功或失敗。）

**練習 2**（需自行實機驗證）

依開頭數字分類的判斷方式：

- `301 Moved Permanently`（永久轉址）→ 開頭 `3`，屬於「重新導向」。它其實不算失敗，是後端跟你說「這個東西搬家了，去新網址拿」。（本章正文把大方向簡化成 2/4/5，實務上還有一個 `3xx` 家族專門管轉址，遇到不用慌。）
- `304 Not Modified`（沒有變更）→ 開頭 `3`，後端說「你要的東西跟你上次拿的一樣，直接用你快取裡的就好」，也不是錯誤。
- `404 Not Found`（找不到）→ 開頭 `4`，屬於「前端錯了」——你要了一個不存在的東西。

所以若你挑到的是 `404`，它屬於「前端錯」這一類：你請求了一個伺服器上沒有的資源。

**練習 3**（示範答案）

把「點一杯珍珠奶茶」寫成一次 HTTP 對話：

```
請求 Request（你 → 店員）：
    方法：POST         ← 你要「新增一筆訂單」，是在建立新東西，所以像 POST
    路徑：/orders      ← 對「訂單」這個資源動手（名詞，不是「點餐」這個動作）
    Body：{ "drink": "珍珠奶茶", "sugar": "半糖", "ice": "少冰" }

回應 Response（店員 → 你）：
    情況一（成功）：
        狀態碼：201 Created   ← 成功而且「建立了一筆新訂單」
        Body：{ "orderId": 42, "drink": "珍珠奶茶", "wait": "5 分鐘" }

    情況二（珍珠賣完了）：
        狀態碼：400 Bad Request  ← 你點的東西目前做不出來，算「請求方的問題」
        Body：{ "error": "珍珠已售完，請換其他配料" }
        （也有人會用 409 Conflict / 422，入門先用 400 表達「你的請求有問題」即可）

    情況三（已經打烊）：
        狀態碼：503 Service Unavailable  ← 開頭 5，店（伺服器）現在不營業、無法服務
        Body：{ "error": "本店已打烊，明天 10 點見" }
```

理由：路徑用名詞 `/orders` 指出「哪個資源」，動作交給方法 `POST` 表達（這是後面 4-B REST 會正式教的概念，這裡先感受一下）。狀態碼則讓你「一眼」分辨成功（2xx）、自己點錯（4xx）、店家出狀況（5xx）。
