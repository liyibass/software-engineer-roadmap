...
做法：到 jwt.io，把預設 token 右側 Payload 裡的某個值改掉（例如 `"name"` 改成你的名字）。你會看到左邊那串 token 文字**跟著變了**——因為 token 是「內容編碼後」串起來的，內容一改，字串就變。

觀察重點：在你**不知道 secret** 的情況下，jwt.io 會顯示簽章驗證**失敗**（通常是「Invalid Signature」之類的紅字提示）。這正說明關鍵：任何人都能改 Payload、也能看 Payload（它只是 Base64 編碼），但**沒有 secret 就補不出對得上的簽章**，所以竄改過的 token 拿去後端一驗就會被打回。請自行到 jwt.io 實機驗證。

**練習 2**

「JWT 防竄改，但不防偷看」意思是：
- **防竄改**：靠 Signature（簽章）。簽章是用「前兩段內容 + 只有後端知道的密鑰」算出來的，別人改了 Payload 卻算不出對應的新簽章（因為沒密鑰），後端一驗就發現對不上、拒絕。所以內容改不動。
- **不防偷看**：Payload 只是 **Base64 編碼**，不是加密。任何人拿到 token，貼進任意 JWT 解碼工具，就能看到 Payload 全部內容。

指導意義：Payload 只能放「不怕被看到」的識別資訊（例如 `userId`、`name`、`role`、過期時間 `exp`），**絕對不能放密碼、信用卡號、任何機密**——因為那等於把機密公開給所有拿到 token 的人。

**練習 3**

最壞情況：`JWT_SECRET` 就是那把「蓋防偽印章」的鑰匙。它一外洩，任何人都能拿它**簽出合法的假 token**——想冒充誰就把 Payload 的 `userId`（甚至 `role`）填成誰，簽章還完全正確、後端驗得過。等於整個認證系統被攻破，任何人都能假扮任何使用者（包括管理員）。

補救：**立刻換一把全新的 `JWT_SECRET`**（rotate secret）。副作用是——所有用舊 secret 簽發的 token（包括所有正常使用者手上那張）都會因為「用新密鑰驗不過」而**全部失效**，所以所有人都得重新登入。這雖然造成一次全體登入的不便，但這正是止血的必要手段：換了新鑰匙，攻擊者手上那把舊鑰匙就再也簽不出有效 token 了。

另外別忘了：光換 secret 還不夠，要一併把外洩的那份從 git 歷史裡清掉，並確保新的 secret 只放在後端環境變數、永不進版本控制。
