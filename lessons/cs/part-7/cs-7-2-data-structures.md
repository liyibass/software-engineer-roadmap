...
- **隨便堆成一疊**：找特定一本，得從最上面一本本翻下去，最慘要翻完整疊——很慢。
- **照書名排好上架**：因為有順序，你能快速定位到大概位置（甚至用二分的方式對折查找）——快很多。
- **另外建一張索引卡**：查卡片就直接知道書在第幾櫃第幾層——最快。

書一本都沒變、數量也一樣，變的只是「組織方式」，但找書的效率就完全不同。這正是「資料結構」在研究的事：**同一批資料，怎麼組織才能讓你最常做的操作變快**。組織方式本身，就是效率的關鍵。

**第 2 題**

因為不同資料結構各有「拿手」和「不拿手」的操作，一個結構不可能所有操作都最快。例如：

- **陣列（Array）**：用索引直接取用很快（`O(1)`），但在「中間插入一筆」要把後面的元素整批往後挪，很慢。
- **鏈結串列（Linked List）**：插入、刪除很靈活，但要「取第 500 個元素」得從頭一個個沿著指標走，很慢。
- **雜湊表（Hash Table）**：用 key 查找超快，但「照大小順序把資料列出來」它做不到。

所以選資料結構的原則是：**先看你最常做什麼操作，再選擅長那個操作的結構**——這就是為什麼說「沒有最好的，只有最適合某操作的」。

**第 3 題**

- **`Vec`（動態陣列）**：擅長「用索引直接取用」（`vec[3]`，`O(1)`）、以及在「尾端」push/pop 新增或移除元素。適合「一列照順序存放、常靠位置存取」的資料。但在中間插入或刪除就得搬移後面的元素，較慢。
- **`HashMap`（雜湊表）**：擅長「用 key 查找、插入、刪除」（都接近 `O(1)`）。適合「我有一個 key（例如使用者 ID），想直接拿到對應的值」的場景。但它不保證順序，沒辦法「照順序列出」或「取最小值」。

一句話總結：要「靠位置」找就用 `Vec`，要「靠 key」找就用 `HashMap`。
