...
演算法就是「解決某個問題的一套明確步驟」，像食譜之於做菜——輸入問題，照著步驟走，得到答案。

生活中「同一問題、不同解法、效率不同」的例子（任舉一個即可）：

- **在字典裡查一個單字**：解法 A 是從第一頁一頁頁翻；解法 B 是利用字典本來就照字母排序，直接翻到大概位置再左右微調（其實就是二分的精神）。解法 B 快非常多。
- **算 1 加到 100**：解法 A 是老老實實 1+2+3…一路加；解法 B 是用公式 `(1+100)×100/2 = 5050`，一步到位。
- **找一副洗好的撲克牌裡的黑桃 A**：一張張翻 vs. 先分堆（花色）再找。

重點在於：同一個問題，步驟不同，花的力氣可以差很多。

**第 2 題**

因為「跑幾秒」會受到「哪一台電腦、當下多忙、用什麼語言」影響——同一個演算法在快的電腦跑得快、慢的電腦跑得慢，拿秒數互比並不公平，也看不出演算法「本質」的好壞。

「成長速度（Big-O）」衡量的是更本質的東西：**當資料量變大時，工作量會怎麼成長**。這個趨勢跟電腦快慢無關，只跟演算法本身有關，所以能公平地比較兩個演算法誰更「耐得住資料變大」。而且真正會出事的場景，正是資料量很大的時候——這時成長速度才是決定性的因素。

**第 3 題**

- **資料量很小時**：差別不大。例如 100 筆資料，`O(n)` 約 100 步、`O(log n)` 約 7 步——都很快，使用者根本感覺不出來。
- **資料量很大時**：天壤之別。例如 10 億筆，`O(n)` 要約 10 億步，`O(log n)` 只要約 30 步。

這說明了：**Big-O 的差距要在「資料夠大」時才會爆發出來**。所以資料量小的時候不必為了效率鑽牛角尖（過早最佳化反而讓程式變複雜）；但只要系統的資料會長大，選對成長速度慢的演算法，價值就會隨資料量放大——這也是為什麼大資料量的系統對演算法效率錙銖必較。
