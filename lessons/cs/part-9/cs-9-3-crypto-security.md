...
**第 1 題：用「保險箱鑰匙」比喻對稱 vs 非對稱**

- **對稱加密＝一把鑰匙的保險箱**：鎖門和開門用「**同一把鑰匙**」。我用鑰匙 K 把東西鎖進保險箱，你也得拿到「同一把 K」才能打開。優點是快、效率高；麻煩在於「**我要怎麼把這把 K 安全地交到你手上？**」——鑰匙在路上被攔截，加密就破功了（這叫「金鑰交換」難題）。

- **非對稱加密＝一對鑰匙的特殊保險箱**：這種保險箱有「**兩把配對的鑰匙**」——一把**公鑰**專門負責「鎖」，另一把**私鑰**專門負責「開」，而且**用公鑰鎖上的，只有對應的私鑰能開**。所以你可以把「鎖用的公鑰」印在名片上到處發，任何人都能用它把秘密鎖給你；但只有你手上那把「開用的私鑰」能打開。沒有「怎麼把鑰匙安全送出去」的煩惱，因為公開出去的那把本來就只能鎖、不能開。代價是它比對稱加密**慢**。

**第 2 題：為什麼存「雜湊值」而不是明文密碼**

雜湊是「**單向**」的：同樣的密碼一定算出同樣的指紋，但**無法從指紋反推回原密碼**。網站的正確做法是：

- 註冊時，不存你的明文密碼，只存密碼算出來的「**雜湊值（指紋）**」。
- 登入時，把你輸入的密碼**也拿去雜湊**，再比對兩個指紋是否相同——相同就代表密碼對，全程網站都不需要知道你的原始密碼。

**資料庫被偷時怎麼保護使用者**：駭客拿到的只是一堆「指紋」，而雜湊是單向的、反推不回原密碼。所以他無法直接得到你的明文密碼去登入你的帳號（尤其是其他也用同組密碼的網站）。相對地，如果網站存的是明文密碼，資料庫一外洩，所有人的密碼就直接裸奔了。（實務上還要對每個密碼加不同的「鹽 salt」等強化，避免駭客用預先算好的表去猜——見課外讀物 E-10。）

**第 3 題（思考題）：為什麼非對稱加密能解掉「金鑰交換」難題**

關鍵在於**加密和解密用「不同」的鑰匙，而且用來加密的公鑰無法拿來解密**。

對稱加密的死結是：雙方得先共享「同一把鑰匙」，但這把鑰匙一旦在傳遞途中被攔，就全破了——你陷入「要安全傳資料，得先安全傳鑰匙」的雞生蛋困境。

非對稱加密拆開了這個死結：**公鑰只能鎖、不能開**。既然公鑰洩漏出去也只能拿來加密、開不了鎖，那它**根本不必保密**——你可以大方公開，被誰攔截都無所謂。別人要傳秘密給你，就用你公開的公鑰加密，而**只有你私藏、從不外傳的私鑰能解開**。這樣一來，「怎麼把鑰匙安全交給對方」的難題就消失了：需要公開的那把（公鑰）不怕被看到，需要保密的那把（私鑰）從頭到尾都不用送出去。

（實務上因為非對稱較慢，HTTPS 常「兩者並用」：先用非對稱加密安全地協商出一把對稱鑰匙，之後再用快速的對稱加密傳大量資料。）
