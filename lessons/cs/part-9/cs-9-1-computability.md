...
**第 1 題：「不可計算」和「電腦不夠快」的本質不同**

這兩件事常被搞混，但差很多：

- **「電腦不夠快」是「量」的問題**：問題有解、也寫得出程式，只是跑起來很慢。換更快的電腦、等更久，遲早算得出來。這是「時間夠不夠」的問題（下一章 P vs NP 就在談這個維度）。
- **「不可計算」是「質」的問題**：**根本不存在**一個能保證正確解出它的程式。這跟速度完全無關——就算給你一台快一兆倍、甚至無限快的電腦，也一樣做不到，因為卡住的是**邏輯上的不可能**，不是算力。

一句話：不夠快是「來不及算完」，不可計算是「**再多時間也算不出來，因為那個程式根本不可能存在**」。

**第 2 題：圖靈機為什麼重要**

因為它**替「計算」下了一個精確的數學定義**。在圖靈機之前，「計算」是個模糊的日常概念；圖靈機用「紙帶＋讀寫頭＋規則」這個極簡模型，把「一步步機械化地算」講得清清楚楚。

有了精確定義才能做兩件事：

1. **劃出計算的範圍**：搭配邱奇—圖靈論題（「凡是能機械化計算的，圖靈機都能算」），「圖靈機算得出來的」就約等於「任何電腦算得出來的」。你的手機、電腦本質上都是圖靈機的實現。
2. **證明某些問題不可計算**：正因為有了嚴謹定義，才能嚴謹地證明「有些問題連圖靈機都解不了」（例如停機問題）。沒有精確定義，就沒辦法談「邊界」。

**第 3 題（思考題）：停機問題與「完美 bug 偵測器」、與測試的關係**

推導鏈是這樣的：

1. 「判斷任意程式會不會無限迴圈（會不會停）」＝停機問題＝**已被證明不可計算**。
2. 一個「能完美抓出所有程式所有 bug」的工具，勢必得能判斷「這程式會不會卡在無窮迴圈」這類問題——而這正包含了停機問題。
3. 既然停機問題不可計算，那麼「對**任意**程式都完美正確的分析工具」就**不可能存在**。注意重點是「對任意程式都完美」；針對特定情況做近似分析是可以的，只是無法保證對所有程式都對。

**和測試的關係**：既然理論上「無法證明一支程式對所有情況都正確」，實務上我們就退而求其次——用**測試**去「檢驗它在許多重要情況下正確」。也就是說，計算理論的極限（不可能有完美的正確性證明工具）反過來塑造了實務做法（所以我們寫測試，涵蓋關鍵情境、提高信心，而不是奢求「證明百分之百正確」）。理論的天花板，正是工程上要靠測試的原因。
