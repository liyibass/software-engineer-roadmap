...
抽象就是「隱藏複雜的內部細節，只露出簡單好用的介面」。以開車來說：

- **露出的（簡單介面）**：油門、煞車、方向盤、排檔——你只要會操作這幾樣，就能把車開走。
- **隱藏的（複雜細節）**：引擎內燃機怎麼點火爆炸做功、變速箱怎麼換齒比、燃油怎麼噴射、電子控制單元怎麼運算。

「駕駛」這個介面把整台車的複雜機械都抽象掉了，所以你「會開車不代表你會造引擎」——這正是抽象的重點：**讓你不必懂全部，就能正確使用**。

**2. 為什麼抽象是對付複雜度的唯一辦法？舉一個本課的例子**

因為**人腦無法一次掌握巨大的複雜度**。一台電腦從電晶體、邏輯閘、機器碼、作業系統到高階語言，細節多到沒有任何人能同時理解每一層。抽象把複雜切成「一層層」，每一層只需要理解「下一層提供什麼介面」，而不用懂「下一層怎麼做到的」，人才有辦法在有限的腦容量下完成工作。

本課的例子（任舉一即可）：

- **作業系統（cs-5-1）** 把硬體抽象成「系統呼叫」——你存檔只要呼叫 API，不用懂硬碟磁頭怎麼移動。
- **高階語言（cs-4-1）** 把機器碼抽象掉——你寫 `a + b`，不用管暫存器與組合語言。
- **網路分層（cs-6-2）** 每層抽象下一層——你用 HTTP 傳資料，不用懂封包怎麼路由。

**3. 思考題：既然有抽象，為什麼還要懂底層？**

因為**抽象會「漏水」**——當系統出 bug、效能異常、或行為不如預期時，問題往往就藏在被隱藏的下一層。這時候你必須能「掀開抽象、往下一層查」：

- **除錯時**：上層的錯誤訊息可能只是表象，真正原因在底層（例如記憶體配置、系統呼叫失敗、網路封包遺失）。不懂底層就只能瞎猜。
- **做效能／取捨判斷時**：懂底層才知道某個寫法為什麼慢、某個抽象的代價在哪，才能做出好的設計決策。
- **選對抽象時**：知道下面怎麼運作，才分得出「好的抽象」與「洩漏細節、藏太多」的壞抽象。

這正呼應 cs-0-1「為什麼要學底層」：抽象讓你平常不必懂底層就能工作，但一旦抽象出問題，懂底層就是你能不能解決問題的分水嶺。
