...
- **命令式（imperative）**：一步一步明確告訴電腦「怎麼做」，親手管理狀態的變化（像寫食譜步驟）。
- **物件導向（Object-Oriented）**：把「資料」和「操作這些資料的行為」封裝成一個個物件，用物件來組織程式。
- **函式式（functional）**：用函式的組合來表達運算，盡量不改變狀態、偏好不可變資料（把資料流過一連串函式轉換）。

**2. `impl` 方法與迭代器 `map/filter` 各屬於哪種典範？**

- **`impl` 方法**（rust-3-2）→ **物件導向**風格。它把資料（struct 的欄位）和行為（方法）綁在一起、封裝成物件，外界透過方法來互動。
- **迭代器 `map`／`filter`**（rust-6-4）→ **函式式**風格。它用函式組合（把資料流過一連串轉換）來描述「要什麼」，而不是手動寫迴圈管理變數，並且偏好不可變、產生新資料而非改動原資料。

（補充：這也說明 Rust 是「多典範」語言——同一個語言裡兩種風格都能用。）

**3. 思考題：為什麼現代語言要「多典範」？給工程師什麼好處？**

因為**沒有一種典範對所有問題都是最好的**。不同問題的形狀不一樣：

- 描述一連串明確步驟、要精細控制流程或效能時，**命令式**最直接。
- 要把大系統拆成一個個有清楚職責、能封裝與重用的模組時，**物件導向**很合適。
- 要對資料做一連串轉換、又想避免「到處變動的狀態」導致的並行 bug 時，**函式式**最好推理。

多典範語言讓工程師**能按問題選最合適的風格，甚至在同一支程式裡混用**——例如 Rust 用 `struct + impl` 組織（物件導向味）、用迭代器 `map/filter` + 不可變處理資料（函式式味）、用迴圈跑流程（命令式味）。

好處是：**工具箱更豐富**。你不必為了換風格就換語言，而是「哪種好用就用哪種」，寫出更清楚、更貼合問題的程式。懂多種典範，本身就讓你成為更全面的工程師。
