...
- **Chrome 瀏覽器** → 應用程式（Application）。它是「為使用者完成特定任務（上網）」的軟體，跑在作業系統之上。
- **Windows** → 系統軟體（System Software）中的作業系統。它管理資源、提供應用程式運作的環境。
- **印表機驅動程式** → 系統軟體（System Software）中的驅動程式。它是 OS 與特定印表機之間的翻譯官。
- **主機板 BIOS** → 韌體（Firmware）。它燒在主機板硬體裡，開機時最先跑，很貼近硬體。

（由上到下排：Chrome ＞ Windows／印表機驅動 ＞ BIOS ＞ 硬體。）

**2. 用自己的話解釋「驅動程式」的角色，以及為什麼裝新硬體常要裝驅動**

驅動程式是「特定硬體的翻譯官」。世界上有無數種硬體（不同廠牌的印表機、顯卡、滑鼠…），作業系統不可能內建「怎麼操作每一種」的知識。所以每個硬體裝置配一個驅動程式：

- 它**知道怎麼跟這個特定裝置溝通**（懂這台裝置的私有指令）。
- 它同時**向作業系統提供一組標準介面**。

於是 OS 只要會用「標準介面」，剩下的「翻成這台裝置聽得懂的話」就交給驅動。

**為什麼裝新硬體常要裝驅動**：因為 OS 原本沒有「和這個新裝置溝通的翻譯官」。裝驅動，就是幫作業系統補上這位翻譯官，OS 才知道怎麼正確操作這個新裝置。

**3. 思考題：應用程式為什麼不直接操作硬體，而要透過系統軟體？**

主要有三個原因（呼應 cs-5-1）：

- **複雜**：直接操作硬體要懂每種裝置的底層細節（暫存器、時序、私有指令）。若每個 App 都自己來，程式會爆炸性複雜。透過 OS／驅動提供的統一介面，App 只要呼叫標準功能即可。
- **安全**：如果 App 能隨意直接碰硬體與記憶體，一個惡意或有 bug 的程式就能搞垮整台機器、竊取別人的資料。OS 當「守門員」，統一管制存取，才能保護系統與其他程式。
- **共享**：一台電腦上有很多程式同時要用 CPU、記憶體、硬碟、印表機。必須有 OS 居中協調、排隊、分配資源，大家才不會互相打架。若人人直搶硬體，就會亂成一團。

一句話：**讓 OS 當中間人，換來的是「更簡單、更安全、能共享」**——這也正是「軟體分層」與抽象的價值。
