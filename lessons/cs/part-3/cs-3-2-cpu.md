...
**1. CPU 內部三個關鍵部件，各自的工作**

- **控制單元（CU）**：CPU 的總指揮，它自己不做運算，而是負責「讀取指令、解碼、指揮其他部件去執行」。
- **算術邏輯單元（ALU）**：真正動手做運算的部件，負責算術（加減乘除）和邏輯（AND、OR、NOT、比較大小）。
- **暫存器（Registers）**：CPU 內部超快、超小的儲存，負責就近暫放「此刻正在運算的那幾個值」。

**2. 為什麼要有暫存器，而不是每次都用記憶體？**

因為**速度差太多**。暫存器就在 ALU 旁邊，是所有儲存裡最快的；而記憶體相對慢很多。如果 CPU 每做一步運算都跑去（相對很慢的）記憶體拿資料、放結果，速度會被記憶體拖垮，快速的 ALU 大半時間都在乾等。所以 CPU 把「正在算的那幾個值」先搬進暫存器，就近高速存取，算完再寫回記憶體。這也是為什麼暫存器位在記憶體階層（cs-3-4）的最頂端——最快、最小、最貴。

**3. 「3 GHz」是什麼意思？為什麼 GHz 高不一定比較快？**

3 GHz 指的是 CPU 的**時脈頻率**每秒跳 30 億次（G = 10 億，Hz = 每秒一次）。時脈像節拍器，每一拍 CPU 推進一步，所以頻率越高，通常每秒能推進越多步。

但 GHz 高不一定代表比較快，因為整體效能還要看：

- **每一拍能做多少事**：不同架構設計下，同一個時脈週期能完成的工作量不一樣（IPC，每週期指令數）。
- **有幾個核心**：多核心（cs-5-3）能同時處理多件事，這跟單核時脈高低是兩回事。
- 還有快取大小、記憶體頻寬等其他因素。

所以時脈只是眾多效能指標的其中一個，不能只看 GHz 就斷定誰快誰慢。
