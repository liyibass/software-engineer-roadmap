...
**1. 依速度由快到慢排序，以及容量與價格趨勢**

速度由快到慢：**暫存器 → 快取 → RAM → 硬碟**。

容量與價格的趨勢正好**跟速度相反**：越快的層，容量越小、每單位越貴；越慢的層，容量越大、越便宜。也就是「越快 = 越貴 = 越小」是一條鐵則。所以暫存器最快但只有幾十個值；硬碟最慢卻可以做到幾 TB 又便宜。

**2. 「局部性」是什麼？為什麼讓「分層」有效？**

局部性指的是「程式存取資料有規律、會集中在某些地方」，分兩種：

- **時間局部性**：剛用過的東西，很可能馬上又要用（例如迴圈裡反覆用到的變數）。
- **空間局部性**：用了某個東西，它附近的東西也很可能接著用（例如陣列的相鄰元素）。

為什麼讓分層有效？因為有局部性，代表「接下來要用的資料」大多集中在「最近用過或附近」的一小撮。只要把這一小撮放進快的層，大部分存取就能在快層命中、享受高速度，只有偶爾才需要去慢層拿。這就是「用小容量的快記憶體，撐起大部分速度需求」的祕密——如果存取是完全隨機、沒有局部性，分層就沒什麼用了。

**3. 「CPU 快取」和「瀏覽器快取」核心思想一樣嗎？**

**一樣。** 共同點是同一句話：**「把常用的東西，放在更近、更快的地方，避免每次都去慢的地方拿。」**

- CPU 快取：把常用資料放在 CPU 旁邊，避免每次都跑去（慢的）主記憶體。
- 瀏覽器快取：把載過的網頁資源（圖片、CSS…）存在本機，避免每次都重新從（慢的、遠的）伺服器下載。

兩者的媒介不同（一個是硬體晶片、一個是本機硬碟），但背後的策略完全相同。這個思想還貫穿 CDN、Redis 等——全都是「常用的東西放近一點、快一點」。
