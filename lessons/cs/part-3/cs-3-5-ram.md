...
**1. 用「倉庫 vs 工作桌」比喻硬碟和 RAM 的分工**

- **硬碟像「倉庫」**：東西平常都收在裡面，容量很大、能長期保存，但拿取比較慢。
- **RAM 像「你的工作桌」**：把「正在用的東西」攤開在桌上，方便快速取用，但桌面有限、不能長期堆放。

要做一件事的流程是：從倉庫（硬碟）把需要的東西搬到桌上（RAM），在桌上快速操作；做完想長期保存，再收回倉庫。一句話：**硬碟負責「記得住」，RAM 負責「跑得快」**。

**2. 「揮發性」是什麼？怎麼解釋「沒存檔就當機會不見」？**

揮發性（volatile）指的是：**一旦斷電，RAM 裡的內容就全部消失**。RAM 像白板上的字，「電源」就是讓字留著的力量，一停電白板瞬間被擦乾淨。

這就解釋了「沒存檔就當機／斷電，東西會不見」：你打的文件在編輯過程中是放在 RAM 裡的，還沒寫進硬碟。RAM 是揮發的，一當機斷電就清空，所以還沒存檔的內容就跟著消失了。而「存檔」這個動作，本質就是把資料從「揮發的 RAM」寫到「不揮發的硬碟」，這樣斷電也還在。

**3. 為什麼「RAM 越大，能同時開越多程式、跑大軟體越順」？**

因為每個正在執行的程式、以及它們執行中的資料，都要放在 RAM（工作桌）上。**工作桌越大，能同時攤開的東西就越多**——也就是能同時載入、執行更多程式，或載入更大型的軟體。

反過來，如果 RAM 太小、桌面不夠攤，作業系統就得把暫時用不到的部分先挪回（很慢的）硬碟，需要時再搬回來（這叫虛擬記憶體／置換，cs-5-4）。這種「硬碟↔RAM 之間反覆搬運」很慢，就會造成載入變慢、卡頓。所以 RAM 越大，越不需要頻繁動用慢速硬碟，整體就越順。
