...
- **取指（Fetch）**：從記憶體拿下一條指令。
- **解碼（Decode）**：搞懂這條指令要做什麼。
- **執行（Execute）**：實際做它（運算、存取記憶體等）。

（有時把「把結果寫回記憶體」算成第四步 Write-back。）做完後回到第 1 步，拿下一條指令，周而復始。

**2. 程式計數器（PC）的作用與變化**

程式計數器是一個特殊的暫存器，它永遠記著「**下一條要執行的指令在記憶體的哪個位址**」。CPU 靠它才知道要去哪裡取下一條指令。

每執行完一條指令，程式計數器通常會**自動 +1**（指向下一條指令的位址），於是指令就被「一條接一條」依序執行下去。比喻上它像你讀書時的書籤，每讀完一頁就往後移一頁。

**3. 「迴圈」在指令週期層面怎麼做到？**

關鍵在於：**程式計數器不一定只會 +1，有些指令會「直接改寫程式計數器的值」**。

如果 PC 只會一直 +1，程式就只能從頭到尾直線執行。但迴圈需要「重複執行同一段」，所以編譯後會用一條**跳躍指令**，把程式計數器「**跳回**」迴圈開頭的位址。這樣 CPU 下一次取指就會回到迴圈的第一條指令，重新把這段跑一遍；配合一個條件判斷來決定要不要再跳，就形成了「重複執行、直到條件不成立才離開」的迴圈。

一句話：迴圈的本質，就是「在適當時機把記住下一步在哪的程式計數器，改寫回迴圈開頭」。你寫的 `if`、`for`、函式呼叫，底層都是這種「改寫程式計數器」的把戲。
