...
**1. 用「翻譯整本書 vs 即時口譯」的比喻解釋編譯和直譯：**

編譯就像「把整本外文書事先完整翻成中文、印成一本新書」——翻譯的工夫在執行**之前**一次做完，之後大家直接讀翻好的中文版（跑得快），但要先等它翻完整本才有東西可讀。對應到程式：編譯器在執行前把整份原始碼一次翻成機器碼執行檔，之後 CPU 直接跑這個執行檔。

直譯就像「請一位口譯員，你讀一句、他當場翻一句」——不用事先翻，馬上就能開始（很靈活），但每讀一次都要即時翻一次（比較慢）。對應到程式：直譯器在執行時一行一行讀原始碼、即時翻譯並馬上執行，沒有「事先產生執行檔」這一步。

**2. 為什麼編譯較快、直譯較靈活？各一個原因：**

- 編譯較快：翻譯的工作在執行**前**就一次做完了，執行時 CPU 直接跑已經翻好的機器碼，不用邊跑邊翻，省掉了執行期的翻譯成本。
- 直譯較靈活：不需要「先編譯」這個步驟，改一行原始碼馬上就能跑（即寫即跑）；而且同一份原始碼只要對方裝了直譯器，在不同平台都能直接跑，不用為每個平台各編一次。

**3. 分類：**

- Rust：編譯（`cargo build` 產生獨立執行檔）
- C：編譯
- Python：直譯
- JavaScript：混合——傳統上算直譯，但現代引擎用 JIT（即時編譯），執行時把熱點程式碼動態編譯成機器碼加速，所以嚴格說是混合。
- Java：混合——先編譯成位元組碼，再由虛擬機（JVM）執行（[cs-4-4] 詳講）。
