...
你寫的程式碼像「組裝家具時你自己動手做的那部分」，函式庫像「現成的零件包」（別人做好的抽屜滑軌、螺絲、把手）。編譯後，你的部分裡會留著一些「待接的空缺」——例如你寫了「這裡要呼叫 `print`」，但 `print` 的真正實作在零件包（函式庫）裡，你手上只有一個記號。

連結（linking）就是把你的部分和這些現成零件「鎖在一起」的動作：連結器（linker）逐一把每個空缺，對應到函式庫裡真正的實作，全部接起來，最後組成一件完整、可以用的成品——也就是一個能跑的執行檔。

**2. 靜態連結 vs 動態連結的優缺點？「缺少 .dll」和哪種有關？**

- 靜態連結：編譯時把函式庫的程式碼「複製」進你的執行檔。優點是自給自足，丟到別台機器就能跑，不依賴外部函式庫；缺點是執行檔比較大，而且多個程式各自帶一份相同函式庫，浪費空間。
- 動態連結：執行檔裡不含函式庫程式碼，只記「我需要某個函式庫」，等到執行時才去系統裡載入那份共享函式庫。優點是執行檔小、多個程式能共用同一份函式庫（省空間）；缺點是依賴目標機器有裝那個函式庫，沒有就跑不起來。

「缺少 XXX.dll 無法執行」的錯誤和**動態連結**有關——`.dll`（Windows）、`.so`（Linux）就是這種共享函式庫，這個錯誤代表程式在執行時找不到它動態連結所需要的函式庫。

**3. 為什麼 Rust 的「自帶一切執行檔」（偏靜態連結）特別好部署？**

因為靜態連結把用到的函式庫都複製進了執行檔本身，產物是一個「什麼都自帶、自給自足」的獨立檔案。部署時你只要把這一個執行檔丟到目標機器就能跑，**不必事先在那台機器上裝好一堆對應版本的函式庫**，也就不會遇到「缺少某個 .so/.dll」「函式庫版本對不上」這類環境問題。

這正呼應 [rust-9-6] 說的「Rust 編譯出一個獨立執行檔，丟上去就能跑」——省掉了「先把環境和依賴裝對」這個部署上最容易出錯、最花時間的環節，所以特別省事、可靠。
