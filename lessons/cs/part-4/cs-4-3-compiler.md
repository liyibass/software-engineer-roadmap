...
1. 詞法分析（lexical analysis）：把一長串字元切成一個個有意義的「詞元（token）」，像把句子斷成單字（`let x = 5;` → `[let][x][=][5][;]`）。
2. 語法分析（parsing）：檢查這些 token 的排列符不符合語言的文法，並組成一棵語法樹表達結構（少分號、括號不對稱就在這關報「syntax error」）。
3. 語意分析（semantic analysis）：檢查「意思」對不對，尤其是型別檢查、有沒有用到沒宣告的變數（文法對、意思錯的問題在這關抓）。
4. 產生程式碼（code generation）：通過前面所有檢查後，才真正翻成機器碼，並可能順便做最佳化。

**2. 「let x: i32 = "hello";」會在哪個階段被抓出來？為什麼不是語法分析？**

會在**語意分析（第三關）**被抓出來。因為這行的**文法完全正確**——它確實符合「`let 名稱: 型別 = 運算式;`」這個格式，token 的排列沒問題，所以語法分析（第二關，只管「排列合不合文法」）不會有意見、順利放行。問題出在更深一層的「意思」：把一個字串 `"hello"` 塞給宣告為整數 `i32` 的變數，型別對不上。這種「文法對、意思錯」的型別不符，正是語意分析負責檢查的範圍，所以要到第三關才被攔下來。

**3. 為什麼強型別語言的語意分析嚴格能提早發現 bug？**

因為它把「型別對不對」這類檢查，從「執行時才爆」提前到「編譯時就擋」。強型別語言（Rust、TypeScript）在語意分析這關對型別要求很嚴——例如你不小心把字串當數字用、傳錯參數型別、拿一個可能不存在的值去運算，這些在編譯階段（程式還沒真正跑）就會直接報錯，你當下就得修。

相對地，弱型別或動態型別語言常常要等程式「執行到那一行」才發現不對，而那行可能藏在某個很少走到的分支裡，甚至上線後才被使用者踩到。所以「編譯期嚴格檢查」等於在最便宜、最早的時機幫你把一整類 bug 攔在門外——這正是 [rust-0-1] 說的「編譯期抓 bug」的價值。
