...
**1. 為什麼 CPU 只懂機器碼，人卻不直接寫機器碼？**

CPU 是一堆電晶體開關組成的硬體（cs-2-5），它天生只能對「有電流／沒電流」也就是 1 和 0 做反應。所以它唯一能直接執行的，就是一串串照特定格式排好的 0 和 1——這就是機器碼，是 CPU 的「母語」。

但人不直接寫機器碼，是因為對人來說它幾乎不可能用：全是 0 和 1，看不出哪段在做什麼、記不住、極容易多打或漏打一位就整個壞掉；一個小程式就是好幾頁的 0/1；而且不同 CPU 的機器碼格式還不一樣，換台機器就要重寫。所以人選擇寫「好讀好寫」的語言，再交給工具翻譯成機器碼——把「難懂但機器要」的部分交給工具，人專心表達想法。

**2. 組合語言和機器碼是什麼關係？**

組合語言是機器碼的「助記符號（好記的符號）」版本，兩者**幾乎一對一對應**：一條機器碼指令（例如 `10110000 01100001`）對應一句組合語言（例如 `MOV AL, 97`）。差別只在「給誰看」——機器碼給 CPU 執行，組合語言用人看得懂的短英文單字（`MOV`、`ADD`）代替那串 0/1。把組合語言翻成機器碼的工具叫「組譯器（assembler）」。所以它們表達的是同一件事，只是一個是人的可讀版、一個是 CPU 的執行版。

**3. 由「最貼近人」到「最貼近硬體」排序：**

高階語言（Rust/Python）→ 組合語言 → 機器碼 → 電晶體。

越往右越貼近硬體、越難讀；越往左越貼近人、越好寫。高階語言離人最近（一行 `5 + 3` 很直覺），電晶體是最底層的物理開關，離人最遠。
