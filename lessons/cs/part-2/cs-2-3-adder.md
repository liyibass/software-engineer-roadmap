...
**第 1 題：「1 + 1」在半加器裡的 sum 和 carry**

半加器的公式是 `sum = A XOR B`、`carry = A AND B`。代入 A = 1、B = 1：

```
sum   = 1 XOR 1 = 0   （兩個一樣 → XOR 輸出 0）
carry = 1 AND 1 = 1   （兩個都 1 → AND 輸出 1）
```

所以 sum = 0、carry = 1。合起來讀就是「carry 1、sum 0」＝二進位的 `10`，正好等於十進位的 2——`1 + 1 = 2`，只是二進位寫成 `10`。真值表最後一行說的就是這件事。

**第 2 題：半加器和全加器的差別**

差在**有沒有考慮「來自下一位的進位」**：

- **半加器**只吃兩個輸入 A、B，算出這一位的 sum 和 carry。它不知道「下面那一位有沒有進位上來」。
- **全加器**吃三個輸入：A、B，還多了一個「進位輸入（carry-in）」，也就是下一位傳上來的進位。

多位數相加時（像 18 + 15，個位進位要加到十位），每一位都得接收下面傳上來的進位，所以真正串起來做多位元加法要用全加器。全加器其實就是**用兩個半加器加一個 OR 閘**組出來的——一樣是邏輯閘的組合，只是多處理了那個進位輸入。

**第 3 題：為什麼有了二補數，同一個加法器就能做減法？**

因為二補數的設計，讓「**減一個數**」等於「**加它的相反數（負數）**」。也就是：

```
A - B  =  A + (-B)
```

在二補數表示法裡，`-B` 就是把 B「每位元反相後加 1」得到的。一旦把 `-B` 算出來，`A - B` 就變成一個純粹的加法 `A + (-B)`——完全用現成的加法器電路就能算，不需要再另外設計一套減法電路。

這正是二補數最漂亮的地方：它把減法「轉化成」加法，讓一個加法器同時包辦加減，省下大量硬體。
