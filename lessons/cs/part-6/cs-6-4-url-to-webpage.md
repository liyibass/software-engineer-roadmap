...
**1. 自己寫出「輸入網址到看到網頁」的流程**

一個完整、正確的步驟版本：

1. 你在瀏覽器輸入 `example.com` 按 Enter。
2. **DNS 查詢**：把網址 `example.com` 翻譯成 IP 位址（像查電話簿）。
3. **建立 TCP 連線**：拿到 IP 後，和伺服器用 TCP「握手」，建立一條可靠的連線。（若是 HTTPS，這裡還會多一步 TLS 加密協商。）
4. **送出 HTTP 請求**：瀏覽器用 HTTP 說「請給我這個網頁」（client 向 server 發請求）。
5. **伺服器回應**：伺服器處理請求（可能查資料庫），把網頁內容（HTML/CSS/JS）打包成 HTTP 回應，切成封包、經路由傳回、由 TCP 確保完整送達。
6. **瀏覽器渲染**：瀏覽器解析 HTML/CSS/JavaScript，計算版面、畫出畫面，過程中可能再請求圖片等其他資源。

對照檢查重點：有沒有把 **DNS → TCP → HTTP → 回應 → 渲染** 這條鏈講齊，且順序正確。

**2. DNS、TCP、HTTP 各出現在哪一步、負責什麼**

- **DNS**：在**步驟②**。負責「翻譯」——把人看的網址 `example.com` 解析成網路實際用來找路的 IP 位址。
- **TCP**：在**步驟③（建立連線）與步驟⑤（回傳資料時）**。負責「可靠傳輸」——先握手建立可靠連線，之後確保回來的封包都到齊、順序正確、掉的重傳。
- **HTTP**：在**步驟④與⑤**。是應用層協定，負責「請求與回應的內容約定」——瀏覽器用它送出「我要這個網頁」的請求，伺服器用它回傳網頁內容。

一句話：**DNS 找地址、TCP 保證可靠送達、HTTP 約定要什麼與回什麼**。

**3. 思考題：這個流程怎麼用到 Part 1 與 Part 5？**

- **Part 1（資料表示）**：你打的網址文字、以及回傳的 HTML/CSS/JS，在底層全都要**編碼成 0 和 1**（例如文字用字元編碼）才能在網路上傳輸、在記憶體裡存放。沒有「資料如何用位元表示」這一層，這些內容根本無法被傳送或處理。
- **Part 5（作業系統）**：整個過程由**作業系統在背後協調**——它管理記憶體讓瀏覽器存放資料、操作網路卡實際收發封包、並用分時排程讓瀏覽器和其他程式同時跑。你能一邊開網頁一邊聽音樂，就是 OS 在協調硬體與多個程式。
