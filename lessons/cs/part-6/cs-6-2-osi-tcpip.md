...
**1. 用寄國際包裹的分工比喻解釋為什麼要分層**

網路要處理超多事：實體訊號怎麼傳、封包怎麼定位、資料怎麼確保不丟、應用程式怎麼理解內容……全部混在一起設計會是一團亂麻。所以把它拆成好幾層，每層只管一件事，就像寄國際包裹的分工：

- **你**：只管「寫信內容 + 收件人」，不用懂後面怎麼運送。
- **郵局**：負責「貼郵資、分類」。
- **貨運**：負責「裝上飛機/卡車、實際運送」。

每一層只管自己那塊，彼此靠約定好的介面配合，整體就運作了。你不用懂飛機怎麼飛，貨運也不用懂你信裡寫什麼——這樣每一塊都好設計、好除錯，也能各自替換。

**2. TCP/IP 四層，以及 TCP、IP 各屬哪層、負責什麼**

由上到下四層：

- **應用層**：應用程式的協定，例如 HTTP、DNS、Email。
- **傳輸層**：端對端傳輸、可靠性——**TCP 屬於這一層**，負責「可靠傳輸」（確保資料送達、順序正確、掉了重傳）。
- **網路層**：跨網路的定址與路由——**IP 屬於這一層**，負責「定址與找路」（決定資料送去哪個位址、走哪條路）。
- **連結層**：實體網路傳輸，例如 Wi-Fi、乙太網路。

一句話記：**TCP 管「可靠送到」（傳輸層），IP 管「找到對方」（網路層）**，兩者是網際網路的兩大支柱。

**3. 思考題：換成 Wi-Fi 上層不用改，體現哪個好處？**

這體現的是**「可替換」**（背後是「封裝／各層只透過介面互動」）的好處。

有線換成 Wi-Fi，改變的只是**最底下的連結層**。因為上層（HTTP、你的網頁和 App）只依賴傳輸層提供的服務，並不直接碰連結層的細節，所以底層怎麼換，上層完全感覺不到、也不用改一行。這正是分層「把複雜度切開、每層獨立可替換」的價值——換硬體不牽動軟體。
