...
**第 1 題：1 byte 等於幾個 bit？2 bytes 能表示幾種值？**

- 1 byte ＝ **8 bits**。
- 2 bytes ＝ 16 個 bit，能表示 2¹⁶ ＝ **65536 種**不同的值。（每多一個 bit 組合就翻倍，16 個 bit 就是 2 的 16 次方。）

**第 2 題：估算手機能存幾張照片**

這題答案依你的手機而定，示範算法（用「一張照片約 3 MB」、且 1 GB ≈ 1000 MB）：

- 若手機是 **128 GB**：128 GB ≈ 128000 MB，÷ 3 ≈ **約 4 萬 3 千張**。
- 若手機是 **256 GB**：256 GB ≈ 256000 MB，÷ 3 ≈ **約 8 萬 5 千張**。

（實際會更少一些，因為系統和 App 也佔空間——見下一題。）

**第 3 題：256 GB 手機為什麼只有 240 幾 GB 可用？兩個原因**

1. **1000 vs 1024 的定義差異**：廠商標示容量用「1 GB ＝ 1,000,000,000 bytes」（1000 那套）算，但手機系統常用「1024 那套」（1 GiB ＝ 1024³ bytes）來顯示。同樣的位元組數，除以比較大的 1024³，算出來的數字就變小——256「GB」用 1024 那套換算大約只剩 238「GiB」。沒人騙人，只是兩套定義。
2. **系統和預裝軟體先佔掉一部分**：手機出廠就內建作業系統、預裝 App、還原分割區等，這些本來就佔掉好幾 GB。所以「可用空間」一定小於「標示容量」。

（這兩個原因合起來，就是 256 GB 手機實際可用只有 240 幾 GB 的由來。）
