...
**第 1 題：什麼是「取樣」？它怎麼把連續的聲音變成一串數字？**

取樣（sampling）就是**把連續的東西，切成很多「一小格一小格」，每格記一個數字**。真實世界的聲音是連續的波（氣壓隨時間平滑變化），電腦裝不下「連續」，只能存分開的數字。

做法是：**每隔極短的時間，量一次「現在波的高度（音量）」，記成一個數字**。一秒鐘量很多次（例如 CD 音質每秒量 44100 次），把這些數字依時間順序串起來，就用「一串數字」逼近了原本連續的聲波。格子（量測的時間間隔）切得夠密，人耳就聽不出它其實是一格一格的。

**第 2 題：圖片放很大時看到的一格一格小方塊是什麼？**

那是**像素（pixel）**。一張數位圖片就是由超多小方格（像素）組成的，每個像素記錄那一點的顏色（例如 RGB 三個數字）。平常看不出來，是因為像素很小很密；放到很大時，單一像素被撐大，就看到一格一格的方塊了。

**第 3 題：JPEG vs PNG**

- **哪個檔案通常比較小？** 拍照（照片這種色彩豐富、漸層多的內容）存成 **JPEG** 通常小很多。因為 JPEG 是**有損壓縮**，會丟掉人眼難察覺的細節換取更小的檔案。
- **哪個能保留完整原始資料？** **PNG**。它是**無損壓縮**，縮小後能完全還原原始資料，一個像素都不差。
- **各適合什麼場合？**
  - JPEG 適合**照片、相片牆、網頁大圖**——這類內容容忍一點點細節損失，換來小檔案、載入快。
  - PNG 適合**需要精確、有透明背景、或色塊分明的圖**——例如 logo、圖示、螢幕截圖、有文字的圖，這些用有損壓縮容易在邊緣產生雜訊，而且常需要透明背景（JPEG 不支援透明）。
