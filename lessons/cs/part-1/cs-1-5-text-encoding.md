...
**第 1 題：Unicode 和 UTF-8 的分工**

- **Unicode 負責「編號」**：它是一張「字元 ↔ 編號」的對照表，給世界上每一個字元分配一個獨一無二的編號（叫碼點 code point，通常寫成 U+ 開頭的十六進位），例如 '你' ＝ U+4F60。它只管「哪個字對應哪個號碼」，不管這個號碼實際怎麼存。
- **UTF-8 負責「怎麼存」**：它規定「把 Unicode 的編號實際編成位元組」的方法，而且長度可變——常用英文只用 1 位元組、中文用 3 位元組、emoji 用 4 位元組。

一句話：**Unicode 給編號，UTF-8 決定編號怎麼變成位元組。** 兩者分工。

**第 2 題：查中文字的 Unicode 編號**

方法是搜尋「某字 unicode」，或用線上工具/程式查詢。示範幾個：

- 「好」＝ U+597D
- 「你」＝ U+4F60
- 「李」＝ U+674E

（你查自己名字裡的字，會得到一個類似 U+XXXX 的十六進位編號，這就是它在 Unicode 表裡的碼點。）

**第 3 題：為什麼「英文只用 1 位元組」很聰明？**

因為這樣 UTF-8 就**和舊的 ASCII 完全相容**。ASCII 本來就用 1 個位元組表示英文字元，而 UTF-8 刻意讓英文字元的編碼和 ASCII 一模一樣。好處是：

- 幾十年來全世界累積的大量純英文文件、程式碼、設定檔（都是 ASCII），不用做任何轉換，直接就是合法的 UTF-8，舊系統和新系統都讀得懂。
- 英文內容不會因為改用 UTF-8 而變大（還是 1 位元組），省空間也省頻寬。

正是這個「無痛相容舊世界」的設計，讓 UTF-8 能順利被大家接受，最後成為網路世界的事實標準（絕大多數網頁都用 UTF-8）。
