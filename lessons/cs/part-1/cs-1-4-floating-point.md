...
不管用哪個語言（JavaScript 直接在瀏覽器 console 打、Python 開 REPL、Rust 用 `println!`…），你都會看到：

```
0.30000000000000004
```

而不是剛好的 `0.3`。多出來的那串 `...4` 就是浮點數誤差現形。再試 `0.1 + 0.2 == 0.3`，會得到 `false`——因為左邊不剛好等於 0.3。

**第 2 題：為什麼 `0.1` 存不準？**

類比十進位：十進位沒辦法用有限位數精確寫出 1/3，只能寫成 0.3333… 無限循環，寫到某位就得截斷，於是有一點誤差。

二進位也一樣，只是「哪些數字會循環」不同。`0.1` 換算成二進位是 `0.0001100110011…`（1100 無限循環）。電腦的浮點數只有固定的位元數（例如 f64 的有效數字是 52 個 bit），裝不下無限循環，只能存一個「非常接近 0.1、但不完全等於 0.1」的近似值。存進去的當下就帶了一點誤差，`0.1 + 0.2` 相加後誤差就顯現出來了。

**第 3 題：記帳 App 該用浮點數存金額嗎？**

**不該。** 金額最忌諱誤差——一分一毛算錯、對帳兜不攏都是大事，而浮點數天生存不準（像上面 0.1 的問題）。

實務上常見兩種做法：

1. **用整數存「最小單位」**：把幣值換算到最小單位（台幣的分、或直接用「分」為單位），用整數存。「123.45 元」就存成整數 `12345`（單位是分），顯示時再除以 100。整數運算精確無誤差，這是最常見的做法。
2. **用專門的十進位/定點數型別**：例如資料庫的 `DECIMAL`、或語言提供的 Decimal/BigDecimal 型別，它們專為精確的十進位運算設計。
