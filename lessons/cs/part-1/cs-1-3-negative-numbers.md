...
所以 −3 ＝ **1111 1101**。（快速檢查：開頭是 1，確實是負數。）
...
結果是 **2**，正確！這證明了 CPU 只要會做加法，把減數轉成二補數就能做減法。

**第 3 題：為什麼符號位元有兩個零，二補數沒有？**

「符號位元」做法是「最左邊那個 bit 表示正負，其餘位元表示數值大小」。當數值大小是 0 時，其餘位元全是 0，而符號位可以是 0 也可以是 1：

- `0 0000000` ＝ +0
- `1 0000000` ＝ −0

同一個「零」有了兩種表示，很尷尬（比較大小、判斷相等都要特別處理）。

二補數則不同：它不是「符號位＋數值」的拼裝，而是一套完整的編碼。零只有一種寫法 `0000 0000`。你可以親手驗證——如果對 0 套用「反轉再加一」去求 −0：反轉 `0000 0000` 得到 `1111 1111`，加 1 變成 `1 0000 0000`，丟掉溢出位後又回到 `0000 0000`。也就是 −0 算出來還是 0，自動合而為一，不會多出第二個零。
