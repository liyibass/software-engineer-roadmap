...
**第 1 題：把一台裝置的零件分類到五大單元。**

以一台**筆記型電腦**為例（你的手機、平板同理）：

| 五大單元 | 這台筆電上的零件 |
|---------|-----------------|
| 輸入單元 | 鍵盤、觸控板、鏡頭、麥克風、（收資料時的）網路卡 |
| 輸出單元 | 螢幕、喇叭、（送資料時的）網路卡 |
| 記憶單元 | RAM（執行時暫存）、SSD／硬碟（長期儲存） |
| 控制單元 | CPU 裡的控制單元（CU） |
| 算術邏輯單元 | CPU 裡的 ALU |

注意兩件事：一，**網路卡同時是輸入也是輸出**（收／送資料）；二，**控制單元和 ALU 都藏在 CPU 這一顆晶片裡**——這就是為什麼說「CPU 是電腦的大腦」。

**第 2 題：用廚房比喻解釋控制單元和 ALU 的分工。**

想像一間廚房：**控制單元就像主廚**，他自己不太動手炒菜，但他看著食譜（指令）、決定「下一步該做什麼、叫誰去做」，負責整個流程的指揮調度。**算術邏輯單元（ALU）就像實際動手的廚師**，主廚一聲令下，他就真的去切菜、烹煮——也就是實際做加減乘除（算術）和比較大小、判斷真假（邏輯）。一個「動嘴指揮」、一個「動手做」，兩個合作，菜（運算結果）才做得出來。

**第 3 題：用五大單元接力描述「按下 7 × 8」。**

1. **輸入單元**：你從螢幕（觸控）或鍵盤按下「7」「×」「8」「＝」。
2. **記憶單元**：把數字 `7`、`8` 和「相乘」這個指令暫存起來。
3. **控制單元**：讀到「相乘」指令、解讀它，指揮 ALU 去執行乘法。
4. **算術邏輯單元（ALU）**：實際把 7 和 8 相乘，算出 `56`。
5. **記憶單元**：把結果 `56` 暫存起來。
6. **輸出單元**：把 `56` 顯示在螢幕上。

你覺得「一按就跳出答案」的瞬間，其實是這五個單元一棒接一棒配合的結果。更精細的「讀取指令 → 解碼 → 執行」流程，Part 3-3「指令週期」會再深入。
