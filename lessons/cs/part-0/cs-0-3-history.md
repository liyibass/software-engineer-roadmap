...
**第 1 題：用「核心元件」說出四個世代的順序。**

**真空管 → 電晶體 → 積體電路（IC）→ 微處理器**。

一句話串起來：世代是用「拿什麼元件來做運算與記憶」來劃分的，而這條路的主軸就是「**元件越做越小、越快、越省電**」——從一根根像燈泡的真空管（大如房間），到指甲大的晶片上塞下數十億個電晶體。

**第 2 題：查一個人物，寫下貢獻。**

這題是開放題，選一個你有興趣的即可。示範三個各兩三句：

- **圖靈（Alan Turing）**：提出「圖靈機」這個抽象模型，替「什麼叫做計算、哪些問題電腦算得出來」打下理論基礎；二戰時參與破解德軍的 Enigma 密碼。被譽為電腦科學與人工智慧之父。
- **馮紐曼（John von Neumann）**：提出「馮紐曼架構」——把「程式（指令）也當成資料，和資料一起存在同一塊記憶體」。這個設計沿用至今，幾乎所有現代電腦都是這個架構。
- **愛達·勒芙蕾絲（Ada Lovelace）**：19 世紀為巴貝奇的「分析機」寫下被認為是史上第一個演算法，因此常被尊稱為第一位程式設計師；她還預見了電腦不只能算數字、還能處理更一般的符號。

**第 3 題：找一個「歷史遺留」的設計。**

示範答案（QWERTY 鍵盤）：QWERTY 這個看似亂排的字母順序，源自 19 世紀的**機械打字機**時代。當時的說法是：把常一起出現的字母分開放，可以**降低相鄰打字桿卡在一起的機率**；打字機被淘汰後，大家早已習慣這個排列，於是它就「留」到了電腦鍵盤上。雖然後來有人設計出理論上更快的 Dvorak 排列，但積習難改，QWERTY 至今仍是主流。

其他好例子：檔案的**副檔名**（`.txt`、`.jpg`）是早期作業系統用來標記檔案類型的慣例；一年被分成的某些奇怪規則、終端機的 80 字元寬……很多「奇怪的設計」查下去都會發現一段有趣的來龍去脈。
