...
**1. 用「大樓管理員」比喻解釋 OS 的角色**

把硬體（CPU、記憶體、硬碟、網路卡）想成一棟大樓裡的各種設施——電梯機房、配電盤、水塔；把你的程式想成入住的房客。房客不會、也不該自己跑去操作配電盤或水塔（太複雜又危險），他們只要跟「大樓管理員」說「我要用電、要用水、要上網」，管理員就幫忙協調。作業系統就是這個管理員：它站在硬體和程式中間，房客（程式）不用直接面對複雜的硬體，只要提出需求，OS 幫你搞定底層細節，還負責讓所有房客公平地共用大樓資源、不互相打架。

**2. 「程式不直接操作硬體」的三個理由，各舉一例**

- **太複雜**：每種硬碟、網卡、顯卡的操作方式都不一樣。如果你的程式要自己會操作「所有廠牌的硬碟」，光是存一個檔案就得寫一堆針對不同硬體的程式碼。有了 OS，你只要說「幫我存這個檔案」，底下是 SSD 還是 HDD、哪個廠牌，你都不用管。
- **不安全**：如果任何程式都能直接亂改硬體和記憶體，一個有 bug 或惡意的程式就能搞垮整台機器、偷別人的資料。例如某個程式若能隨意讀寫別人的記憶體，就能偷走你在瀏覽器輸入的密碼。OS 當守門員，管控誰能做什麼。
- **要共享**：一台電腦同時跑很多程式，卻只有一顆 CPU、一份記憶體。如果沒人協調「誰先用 CPU、記憶體怎麼分」，大家就會搶成一團。例如你一邊聽音樂一邊打字，就是 OS 在背後協調兩者共用 CPU。

**3. 核心模式 vs 使用者模式，為什麼對安全重要**

- **核心模式（kernel mode）**：核心（kernel）執行的模式，有完整權限，能直接碰硬體、做任何敏感操作。
- **使用者模式（user mode）**：你的程式執行的模式，權限受限，想碰硬體（讀檔、上網）必須透過核心提供的「系統呼叫（system call）」拜託核心代勞。

為什麼對安全重要？因為這個分級把你的程式關在一個「權限受限的沙盒」裡。程式想做任何敏感操作，都得先向核心提出請求，核心會**先審核**（你有沒有權限？這操作合法嗎？）才代為執行。這樣一來，就算某個程式壞掉或懷有惡意，它也無法繞過核心直接破壞系統或偷別人的資料——所有危險操作都被核心這道關卡把守著。
