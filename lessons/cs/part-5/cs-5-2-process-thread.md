...
- **程式 vs 行程 = 食譜 vs 做菜**：程式（program）是躺在硬碟上的執行檔，靜態、死的，就像一本食譜——上面寫好步驟，但它自己不會動。行程（process）是那個程式「被執行起來」的狀態，動態、活的，就像「廚師正照著食譜在做菜」這個進行中的活動。同一本食譜（程式）可以同時有好幾個廚師各做各的（多個行程）——像你開三個記事本視窗，就是「一個程式、三個行程」。
- **行程 vs 執行緒 = 工廠 vs 工人**：行程像一間工廠，有廠房、原料、設備（也就是它專屬的記憶體與資源）。執行緒（thread）像工廠裡的工人——一間工廠可以請多個工人，同時做不同的活，而他們共享同一間工廠的廠房和原料（同一個行程的記憶體）。

**2. 行程隔離 vs 執行緒共享的好處與風險**

- **行程之間記憶體隔離**：
  - 好處是**穩定與安全**——A 行程不能讀寫 B 行程的記憶體，所以一個程式崩潰不會拖垮別的。例如瀏覽器把每個分頁做成獨立行程，一個分頁掛了不影響其他分頁。
  - 代價（風險）是行程之間要溝通比較麻煩，得靠專門的跨行程機制，而且建立行程的成本較重。
- **執行緒之間記憶體共享**：
  - 好處是**高效合作**——同行程的執行緒直接共用記憶體，溝通容易、建立成本輕，能一起處理同一份資料。
  - 風險是**隔離性弱**——只要一個執行緒把共享資料搞壞，同行程的其他執行緒全遭殃；而且多個執行緒同時亂改共享資料，會產生資料競爭（data race）這種難抓的 bug。

**3. 思考題：為什麼「執行緒共享記憶體」既是優勢又是危險**

因為「共享」這件事本身是一把雙面刃。優勢面：多個執行緒共用同一份記憶體，就能高效地一起處理一份資料、彼此溝通不用繞遠路，這正是並行能加速的關鍵。危險面：正因為大家都能碰同一份資料，一旦**多個執行緒同時修改**它，而沒有任何協調，就會發生資料競爭——結果取決於「誰剛好先寫」，變得不可預測、還會產生隨機出錯的 bug。

Rust 課程之所以花大力氣講「所有權」和 `Arc`/`Mutex`，就是要在享受「共享」的好處的同時，用規則管住「同時亂改」的危險：`Arc` 讓多個執行緒安全地共享同一份資料的擁有權，`Mutex` 則保證同一時間只有一個執行緒能真正動它。換句話說，這一章的「執行緒共享記憶體」正是 Rust 那套並行規則要解決的底層問題。
