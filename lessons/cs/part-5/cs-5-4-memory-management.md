...
**1. 虛擬記憶體給程式什麼「幻覺」？有什麼好處？**

虛擬記憶體讓每個程式都以為自己「獨佔一整片、從 0 開始、連續的記憶體」。這是假象——實際上程式的資料零散分布在實體 RAM 的各個角落，甚至有一部分被搬到硬碟上，是 OS 在背後偷偷把程式看到的「虛擬位址」對應到真正的「實體位址」。

好處有三：

- **簡單**：程式不用管「我被分到哪塊、記憶體夠不夠連續」，它眼中永遠是一片乾淨、連續的空間，寫程式輕鬆很多。
- **隔離（安全）**：A 的虛擬位址和 B 的虛擬位址就算數字一樣，也對應到不同的實體位置，A 根本碰不到 B 的記憶體。這正是 [cs-5-1] 說的「隔離安全」的具體實現。
- **彈性**：OS 能自由地搬動、重新分配實體記憶體（甚至搬到硬碟再搬回來），程式完全無感。

**2. 為什麼 A 和 B「同樣的虛擬位址」不會互相干擾？**

因為虛擬位址只是「程式眼中的地址」，真正去哪裡讀寫，得經過 OS 的頁表（page table）轉換成實體位址。A 和 B 各有各的頁表，所以：

- A 的虛擬位址 100 → 經 A 的頁表轉換 → 例如實體位址 5000
- B 的虛擬位址 100 → 經 B 的頁表轉換 → 例如實體位址 9000

同樣是「位址 100」，卻對應到完全不同的實體位置。A 寫它的 100 動到的是實體 5000，B 寫它的 100 動到的是實體 9000，兩者永遠踩不到對方。OS（配合 CPU 裡的 MMU 硬體）保證每個程式的虛擬位址只會映射到分配給它自己的實體記憶體，這就是隔離的來源。

**3. 思考題：為什麼 swap 用多了電腦會超卡？**

swap 的做法是：實體 RAM 不夠用時，把暫時用不到的記憶體內容搬到硬碟上，騰出 RAM 給急需的程式；需要時再搬回來。它讓「所有程式要的記憶體總和 > 實體 RAM」也能運作——記憶體看似變大了。

但關鍵在速度差：硬碟（尤其 HDD）比 RAM 慢好幾個數量級（回憶 cs-3-4 的記憶體階層）。偶爾搬一次還好，但當記憶體嚴重不足時，程式頻繁存取那些被搬到硬碟上的資料，OS 就得不停地把資料在 RAM 和硬碟之間搬來搬去——這叫 **thrashing（抖動）**。這時 CPU 大部分時間都在等超慢的硬碟搬資料，幾乎做不了正事，電腦就會慢到爆。這正是「記憶體不足時電腦狂卡」的真相，也是為什麼「加 RAM」對常常記憶體吃緊的電腦特別有感——減少了拿慢速硬碟當記憶體用的窘境。
