...
一顆 CPU 核心任何時刻只能做一件事，就像一個廚師一次只能動一雙手。但這個廚師不會傻傻地把 A 桌的菜全做完才服務 B 桌，而是：切 A 桌的菜 30 秒 → 換去炒 B 桌 → 換去 C 桌擺盤 → 回 A 桌繼續……他在各桌之間快速輪流，每桌只做一小段就換下一桌。因為切換得夠快，每桌客人都「感覺廚師一直在服務我」，其實廚師從頭到尾都只服務一桌。這就是分時（time-sharing）：CPU 飛快地在各程式之間切換，製造出「同時在跑」的錯覺。

**2. 並行 vs 平行的關鍵差別，單核心能達成哪一個**

- **並行（concurrency）**：「有能力處理多件事」，靠單核心快速切換做到「看起來同時」，本質是**輪流**。
- **平行（parallelism）**：「真的同時在執行」，需要**多個核心**，物理上一起跑。

單核心只能達成**並行**——它一次只能執行一條指令，所謂「同時」是快速切換造成的錯覺。要真正的平行，得有多個核心，讓不同工作在不同核心上物理性地一起跑。一個比喻：並行像「一個人輪流陪兩個小孩玩，切很快讓兩個都覺得有人陪」；平行像「兩個大人各陪一個小孩，真的同時」。

**3. 思考題：上下文切換是什麼？為什麼開很多程式會變慢？**

**上下文切換（context switch）**：每次 CPU 從工作 A 換到工作 B，要先把 A 的「現場」（暫存器、程式計數器等狀態）儲存起來，再把 B 之前存的現場載入，讓 B 從上次中斷的地方繼續。這個「存檔／讀檔」的動作本身不做任何有用的計算，是一種純開銷。

為什麼開很多程式會變慢？有兩層原因：其一，程式越多，排程器要輪流照顧的對象越多，切換次數就越頻繁，而每次切換都要付出上下文切換的開銷——切換太頻繁時，CPU 的時間都耗在「存檔讀檔」上，真正做正事的時間反而變少。其二（延伸到下一章）記憶體也會吃緊，可能被迫用慢速硬碟當記憶體（swap），進一步拖慢。所以「開太多程式會變慢」，切換開銷是原因之一。
