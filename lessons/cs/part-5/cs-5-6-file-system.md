...
**1. 檔案系統把「一堆位元組」變成什麼好用的東西？**

硬碟本質上只是一大片能存 0 和 1 的空間，如果直接面對它，你看到的只是「第幾號到第幾號位元組存了什麼」——一片無名的位元組海洋，根本無從找起。檔案系統（file system）是 OS 提供的一層組織，把這片混沌整理成你熟悉的**檔案（file）** 和**資料夾（folder/directory）**：一個個有名字、有結構的東西，像「報告.docx」「照片資料夾」。這是 [cs-5-1] 說的「抽象」的好例子——你存檔案時完全不用管它實際存在硬碟哪幾個位置，只要用「檔案」「資料夾」這些好懂的概念就好。

**2. 一個檔案路徑，標出資料夾／檔名／副檔名**
...
- `home/`、`alice/`、`照片/` 是**資料夾**（一層層的目錄，最前面的 `/` 是根目錄）
- `貓.jpg` 是**檔名**
- 檔名裡最後那個 `.jpg` 是**副檔名**，提示這是一張圖片，讓 OS 知道該用看圖程式打開

（這是「絕對路徑」，從根目錄 `/` 開始一路指到檔案；若從目前所在資料夾開始寫，則是「相對路徑」。）

**3. 思考題：為什麼刪除後有時還能救回？怎麼徹底銷毀？**

當你「刪除」一個檔案時，檔案系統通常並沒有真的去把硬碟上那些 0 和 1 抹掉，它只是**把該檔案的記錄標記為「這塊空間可以再用了」**。實際的內容當下還原封不動地留在硬碟上，只是你在檔案總管裡看不到它了。因此只要那塊空間還沒被新資料覆蓋，救援軟體就能把原本的內容找回來、重建成檔案——這就是「刪掉的檔案有時能救回」的原因。

正因如此，要**徹底銷毀**機密資料，不能只按刪除（那只是拿掉了記錄）。正確做法是**覆寫（overwrite）** 那塊空間——用無意義的資料（例如全 0、或隨機位元）把原本的內容實際蓋掉，這樣即使有人去讀那塊硬碟空間，也只讀到垃圾，救不回原始內容。（相關資安考量可參考課外讀物 E-10。）
