...
- **沒 Redis**：每個請求都直接查資料庫，1000 個人 = **查 1000 次**。而且這 1000 次查的還是「同一篇文章」，等於重複做了 999 次白工。
- **有 Redis（Cache-Aside + TTL）**：只有**第 1 個**請求會 miss、去查資料庫（順手把結果存進 Redis），後面 999 個請求全部從 Redis 命中，**查資料庫只有 1 次**。

差別：資料庫的負擔從 1000 次驟降到 1 次（相差 1000 倍）。這就是本章講的核心——把「重複的查詢」擋在 Redis 這層，讓資料庫只處理「真正不重複」的查詢。對資料庫來說，這代表壓力大減、不容易被熱門內容打爆，同時使用者也因為從記憶體拿（~1ms）而更快。
...
1. **首頁熱門商品列表 → 值得快取 ✅**。很多人重複看同一份列表（命中率高），而且不需要秒級即時準確——設個短 TTL（例如幾十秒）就能大幅減壓，是快取的完美場景。
2. **即時帳戶餘額 → 不該快取 ❌**（或要非常小心）。餘額必須即時準確，快取一舊就可能顯示錯的數字，屬於本章表格裡「必須即時準確」那類，不適合。
3. **2 秒的報表結果 → 非常值得快取 ✅**。這是「算起來很貴」的典型——算一次要 2 秒，快取起來後續都省下這 2 秒。只要能容忍報表資料稍微過時（設合理 TTL），效益極大。
4. **每個請求都不同的隨機推薦 → 不值得 ❌**。既然每個請求結果都不同，就不存在「重複」，命中率趨近於 0，快取了也幾乎不會被再次用到，反而白佔記憶體。

判斷主軸都回到 cache-1-2 的三條件：**會不會重複讀（命中率）＋ 能不能容忍稍微過時**。
...
- **複雜運算的結果**：例如「計算某使用者的推薦清單」很耗 CPU，算一次就快取起來重複用。
- **外部 API 的回應**：呼叫第三方 API 又慢、又可能按次收費，在合理時間內快取它的回應，能省時間也省錢。
- **組裝好的頁面片段**：把已經渲染好的內容快取起來，省去每次重複組裝的成本。

**共同特徵**：這些東西都是「**拿起來慢（或貴）、而且會被重複要**」。只要符合這個特徵，不管來源是資料庫、CPU 運算還是外部服務，都可以考慮快取——這正是快取最核心的精神（cache-1-1）。
