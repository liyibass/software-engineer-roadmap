...
- **過期（Expiration）＝主動、因「時間到」**：你給每個 key 設 TTL，時間一到就自動失效消失。這是**你主動控制**的（你決定它活多久）。
- **淘汰（Eviction）＝被動、因「沒空間」**：記憶體滿了、又要存新東西，快取被迫依淘汰策略（如 LRU）「踢掉一些 key 騰空間」。這是**被動發生**的，即使 key 還沒到 TTL 也可能被踢。

所以一個 key 消失的**兩種原因**：(1) **到了 TTL 自動過期**，(2) **記憶體滿了被淘汰**。這也是為什麼你的應用必須接受「快取裡的東西隨時可能不在」，miss 了就重新從 DB 載入（Cache-Aside）。
...
1. 兩者看的維度不同：
   - **LRU（Least Recently Used，最近最少使用）**：看「**最近**有沒有被存取」（**時間**維度），踢掉**最久沒被碰**的那個。
   - **LFU（Least Frequently Used，最少使用）**：看「**總共**被用了幾次」（**頻率**維度），踢掉**累計使用次數最少**的那個。
2. LRU 的假設合理，是因為程式存取有「**時間局部性**」——**最近常被用的資料，未來很可能也常被用；最久沒碰的，先踢掉通常影響不大**。這跟 CPU / OS 快取用的邏輯一致（cache-2-2），對「有時效性熱點」的資料特別準（最近熱的往往就是接下來還熱的）。補充：LFU 則在「長期穩定熱點 vs 偶發爆紅」的區分上更好——一個偶爾爆紅但平常沒人用的，LFU 不會傻傻留著它。
...
**為什麼危險**：如果大量 key 都設「一模一樣的 TTL」（例如全部 60 秒），它們就會在**同一個瞬間一起過期** → 那一刻所有請求同時 miss → 海量請求瞬間一起湧向資料庫 → 資料庫被打爆，這就是**快取雪崩**（cache-6-2）。

**怎麼用隨機抖動避免**：給每個 key 的 TTL 加上一點隨機值，讓過期時間**錯開**。例如：

```
基礎TTL = 300秒
抖動 = 隨機(0, 60)秒
redis.set("product:" + id, 資料, EX = 基礎TTL + 抖動)
// → 各 key 在 300~360 秒之間分散過期，不會擠在同一秒一起失效
```

這樣過期壓力被「攤平」在一段時間內，資料庫每一刻只需承受少量回源，就不會發生雪崩。這是很重要也很常用的實務技巧。
