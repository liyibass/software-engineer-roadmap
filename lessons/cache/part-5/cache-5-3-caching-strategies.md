...
**為什麼分兩類**：快取要處理兩種截然不同的情境。

- **讀的時候**要問的是：「要資料時，快取和資料庫怎麼配合？」（先問快取、miss 才查 DB）。
- **寫的時候**要問的是：「資料更新了，快取裡的舊副本怎麼辦？」——這是讀策略不會碰到的問題（資料一改，快取就可能變成過時的錯資料）。

因為「讀」和「寫」面對的問題不同，自然發展出兩套策略：

- **讀策略**：Cache-Aside（應用自己管）、Read-Through（快取層自己管）。
- **寫策略**：Write-Through（同步寫快取和 DB）、Write-Behind（先寫快取、稍後非同步寫 DB）。
...
1. **Write-Behind 寫得快，Write-Through 保證一致。**
   - **Write-Through** 每次寫都要等「快取 + 資料庫」兩邊都完成才算數，所以**慢**，但好處是寫完的瞬間快取就跟資料庫一模一樣，永遠**一致**。
   - **Write-Behind** 只寫快取就馬上回應（資料庫稍後非同步、批次補寫），所以**快**，但快取和資料庫之間會有一小段時間不一致。
2. **資料遺失風險的來源**：Write-Behind 是「先只寫快取，資料庫稍後才補寫」。如果在「還沒把資料寫進資料庫」的那個空窗期，**快取節點就掛了**（斷電、當機），那些只存在快取、還沒落地到資料庫的資料就**永遠消失了**。這呼應 cache-2-4「快取是可丟的、重要資料要放 DB」——所以 Write-Behind 只適合能容忍極小機率遺失的資料（如點擊統計），一般業務資料要小心用。
...
1. **一般商品資料讀取 → Cache-Aside（讀）**。這是最常見、最靈活的預設選擇：先問快取、miss 才查 DB、查到存回快取。實務上再搭配「寫時刪快取」處理更新（cache-6-5）。
2. **記錄文章被點擊幾次 → Write-Behind（寫）**。點擊量海量、又能容忍丟掉幾秒的資料（少算幾次點擊沒差），正好適合「先 `incr` 寫快取、超快回應，背景再批次寫進 DB」，還能合併大量寫入替資料庫減壓。
3. **銀行轉帳後的餘額 → Write-Through（寫）**。餘額寫完馬上會被讀、而且**絕對不能錯或遺失**，需要「快取和資料庫同步一起更新、保證一致」。這裡寧可寫慢一點也要正確——絕不能用有遺失風險的 Write-Behind。

選策略的主軸：**看重「速度」還是「一致性 / 不能遺失」**，以及「能不能容忍不一致」。
