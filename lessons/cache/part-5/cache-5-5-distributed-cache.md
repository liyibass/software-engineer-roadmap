...
問題出在公式裡的「節點數」被當成除數。加一個節點，除數就從 3 變成 4，於是**幾乎每個 key 算出來的節點都跟著變了**：

```
原本（3 節點）：hash("product:123") % 3 = 1  → 資料在節點 1
加到（4 節點）：hash("product:123") % 4 = 3  → 現在卻去節點 3 找
→ 但資料還在節點 1，去節點 3 找 → 找不到（miss）！
```

因為約 3/4 的 key 歸屬都改變了，結果是**幾乎整個快取瞬間全部 miss**。**可怕之處**：這些 miss 的請求會在同一時間**全部湧向資料庫**，形成**快取雪崩**（cache-6-2），可能直接把資料庫打垮。也就是說——你只是想「加一台機器擴容」，卻反而引發了一場災難。這就是為什麼需要一致性雜湊。
...
一致性雜湊把**節點**和 **key** 都用 hash 對應到同一個「圓環（0 ~ 最大值）」上的位置。找家規則是：**從 key 的位置往「順時針」走，遇到的第一個節點就是它的家。**

關鍵在於：一個 key 的歸屬**只跟「它順時針最近的那個節點」有關**，跟其他節點無關。所以：

- **加一個新節點**：它落在環上某個位置，只會「接管」原本屬於它順時針後方那一小段的 key（這些 key 現在順時針先遇到的是新節點）。環上**其他絕大多數 key 的歸屬完全不變**——因為它們順時針最近的節點根本沒動。
- **減一個節點**：只有原本屬於它的那一小段 key，需要改由順時針的下一個節點接手，其他 key 一樣不受影響。

對比取模「除數一變、幾乎全部 key 重算」，一致性雜湊加減一個節點只影響環上「相鄰的一小段」，大約只有 `1/節點數` 的 key 要搬家 → 只有一小部分 miss，不會全部雪崩。（實務再用**虛擬節點**讓分布更均勻。）
...
1. **不需要自己從頭實作。** 實務上是「**用**」現成方案，而不是「造輪子」：
   - **Redis Cluster**（官方叢集方案）內建資料分片（用 hash slot，概念類似一致性雜湊），加節點時它會自動搬移一部分 slot。
   - 很多 Redis 客戶端函式庫、代理（如 Twemproxy）也幫你處理分片。
   - 雲端託管 Redis（aws ElastiCache）提供叢集模式。
2. **即使用現成方案，還是要懂原理**，因為：
   - 你才知道這些叢集方案到底在**解什麼問題**（為什麼要分片、為什麼取模不行）。
   - 你才能預期**擴縮容時會發生什麼**——例如「加節點時會有一小部分 key 搬家、短暫 miss」是正常的，而不是驚慌以為系統壞了；也才懂為什麼不能用最天真的取模方式自己亂分。
   - 出問題（例如某次擴容後 miss 暴增）時，懂原理才debug 得動、才知道怎麼調整。
