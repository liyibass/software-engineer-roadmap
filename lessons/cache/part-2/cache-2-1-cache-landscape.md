...
從「離使用者最近」到「最底層的原始資料」，一條完整的鏈大概長這樣（至少列出其中 4 層就算過關）：

```
使用者眼睛
  ① 瀏覽器快取（你自己的電腦/手機）
  ② CDN 快取（離你近的邊緣節點）
  ③ 應用層快取（後端旁邊的 Redis / 行程內）
  ④ 資料庫快取（DB 自己的記憶體，buffer pool）
  ⑤ 作業系統快取（page cache）
  ⑥ CPU / 硬體快取（L1/L2/L3）
原始資料（硬碟）
```

重點不是「背出六層的精確名稱」，而是抓到兩件事：

1. **快取是一整條鏈，不是一個點**——一筆資料從硬碟到眼前，沿途每一層都可能把它暫存起來。
2. **順序反映「離使用者的遠近」**——越上層離使用者越近，命中就越快、越能擋掉往下的請求。

只要你畫出「瀏覽器 → CDN → 應用層(Redis) → 資料庫 → 硬碟」這種由近到遠的分層，方向對了就對了。
...
**在「瀏覽器快取」命中，對使用者更快、對系統也更省。** 原因是這兩者在全景圖裡差了非常多層：

- **瀏覽器快取命中**：資料就在使用者自己的電腦上，**連網路請求都不用發**，幾乎 0ms 就顯示。而且因為請求根本沒送出去，CDN、後端、資料庫**通通不用出力**——系統零負擔。
- **資料庫快取命中**：雖然 DB 不用讀硬碟（命中它自己的記憶體），但這代表請求已經「一路穿過」瀏覽器（沒中）→ CDN（沒中）→ 應用層 Redis（沒中）→ 才落到資料庫。中間經過了完整的網路往返、後端處理，對使用者來說慢得多，沿途每一層也都被打擾了一次。

這就是本章「命中越早越好」的核心：**請求在越上層（離使用者越近）的快取命中，效益越大**——不只更快，還能把下層的壓力整個擋掉。
...
主要牽涉離使用者最近的兩層：**① 瀏覽器快取** 和 **② CDN 快取**。

原因是：你在伺服器上把前端檔案（JS/CSS）更新了，但使用者的**瀏覽器**可能還存著舊版、**CDN 邊緣節點**也可能還快取著舊版。請求根本沒到你更新過的伺服器，就在這兩層被舊資料「攔截」了，於是使用者看到的還是舊畫面。

這正說明了本章的重點——**同一個問題常常要跨多層一起解決**。這裡就得同時處理瀏覽器快取（例如檔名加 hash、設定 Cache-Control）和 CDN 快取（例如 purge / 失效），只搞定一層是不夠的。這個題目本書 Part 3、Part 4 會深入。
