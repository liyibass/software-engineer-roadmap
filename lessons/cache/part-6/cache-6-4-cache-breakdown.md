...
**擊穿 vs 雪崩的差別**：兩者都是「快取失效導致請求湧向 DB」，但差在「範圍」——

- **雪崩**：是「**大量、不同的** key」一起失效（一大片），例如一批 key 同時過期、或 Redis 整台掛掉，造成很多不同查詢同時打 DB。
- **擊穿**：是「**單一個熱點** key」失效。範圍只有一個 key，但因為這個 key 超熱門，在它過期的「那一瞬間」湧入的併發量極大，全部集中打在「同一筆」查詢上。

簡單說：雪崩是「很多點一起塌」，擊穿是「一個超熱的點在過期瞬間被打穿」。

**為什麼越熱門越容易擊穿**：擊穿的殺傷力來自「過期瞬間同時湧入的請求量」。一個 key 越熱門，代表它每秒被請求的次數越多（例如首頁熱門商品每秒 1 萬次）。當它 TTL 一到、過期的那一瞬間，這 1 萬個請求會同時發現 miss、同時穿過去查 DB 的同一筆資料。諷刺的是：本來只需要查 DB 一次就能回填快取，卻因為「大家同時 miss」變成瞬間查了上萬次一模一樣的東西。請求量越大（越熱門），過期瞬間的衝擊就越猛——所以越熱門的資料越容易、也越嚴重地被擊穿。
...
**為什麼只需要一個去查 DB**：過期瞬間湧入的上萬個請求，要的其實是「**同一筆**」資料（同一個熱點 key）。這上萬次查詢的結果會一模一樣，所以根本不需要每個請求都各自跑去查 DB——只要「一個」請求去查、把結果回填快取，其他人等它填好，直接從快取拿就行。讓上萬個相同請求都去查 DB，是在對同一筆資料做上萬次完全重複的無用功，白白壓垮 DB。

**互斥鎖怎麼達成**：用一把「鎖」讓這上萬個 miss 的請求去搶，只有一個能搶到：

1. 上萬個請求同時發現 key miss。
2. 它們去搶同一把鎖（例如 `redis.嘗試上鎖("lock:"+key)`）。
3. **只有搶到鎖的那一個**去查 DB、把結果回填快取、然後釋放鎖。
4. **其他沒搶到的**，稍等一下下（例如 50 毫秒）再重試——這時快取通常已被填好，直接命中快取拿到值。

效果：過期瞬間 DB 只被查「一次」（搶到鎖那個），而不是上萬次。這個「讓並發的相同請求實際只執行一次」的模式，也叫 **single-flight（單飛）**。代價是沒搶到鎖的請求多等一點延遲，但這遠比 DB 被打爆划算。
...
- **雪崩**：大量、不同的 key 在同一時間失效（同時過期或 Redis 掛），大批請求一起打 DB。
  **主要解法**：TTL 加隨機抖動（錯開過期）、多級快取、降級／限流兜底。
- **擊穿**：單一個超熱點 key 在過期的那一瞬間，被大量請求同時要，集中打在同一筆查詢上。
  **主要解法**：互斥鎖 / single-flight（只讓一個查 DB）、熱點永不過期 + 背景更新。
- **穿透**：一直查根本不存在的資料，永遠 miss、永遠打 DB（常被當攻擊）。
  **主要解法**：快取空值（把「不存在」也快取，設短 TTL）、布隆過濾器、參數驗證。

一句話記憶：**雪崩是「一大片塌了」，擊穿是「一個超熱的點破了」，穿透是「打一個本來就沒有的洞」。**
