...
根源是瀏覽器的兩個行為疊在一起：**「只認檔名」＋「強快取命中」**。

情境：使用者第一次載入 `app.js`，你給它設了快取（例如 `max-age=1 天`），瀏覽器就把這份舊版 `app.js` 存進本機、記住「一天內可用」。接著你修好 bug、部署了新版——**但檔名沒變，還是 `app.js`，只是內容換了**。

問題來了：使用者同一天再訪，瀏覽器要 `app.js` 時發現「這個檔名我快取過、還在 max-age 內」，於是**直接強快取命中、用本機那份舊的**，**根本沒去伺服器下載你的新版**。

核心：**瀏覽器只認「檔名」，不知道伺服器上同名檔案的內容已經換了**。只要檔名一樣又還沒過期，它就用舊的。所以使用者看到的還是有 bug 的舊版——不是你沒部署，而是他的瀏覽器壓根沒去下載新版。
...
**為什麼能解決坑**：既然問題根源是「瀏覽器只認檔名」，那就**讓檔名跟著內容變**。打包工具（Vite、webpack）會把檔案內容算成一段 hash 放進檔名——`app.a1b2c3.js`。內容一改，hash 就變，檔名就變成 `app.f9e8d7.js`。

於是使用者再訪時，頁面要載的是**新檔名** `app.f9e8d7.js`，瀏覽器一看「這個檔名我從沒快取過！」就**一定會去下載新版**——舊版的強快取再也命不中，因為根本沒人再要那個舊檔名了。這叫 **cache busting（快取破壞）**。

**為什麼能放心永久快取**：因為 hash 是從內容算出來的，所以「**某個特定檔名的內容永遠不會變**」——`app.a1b2c3.js` 這個名字對應的內容，永遠就是那份（內容一變就會變成別的檔名了）。既然這個檔名的內容不可能變，那快取它就絕不會拿到「錯的舊版」。所以可以放心設 `max-age=31536000, immutable`（一年 + 永不驗證），讓沒改到的資源持續享受永久強快取、超快。

這正是化解 cache-1-2「速度 vs 新鮮度」矛盾的完美解法：**既永久快取（快）、又不會拿到舊版（新鮮）**。
...
先建立關鍵角色認知：**`index.html` 是「指路牌」**——它裡面用 `<script src="/app.f9e8d7.js">` 這種引用，告訴瀏覽器「現在該去載哪些（帶 hash 的）檔案」。打包工具每次 build 都會自動更新這裡的引用，指向最新的 hash 檔名。

**1. 為什麼 index.html 不能永久快取（要 `no-cache`）**：
因為它是唯一一個「檔名固定不變（永遠叫 `index.html`）、但內容會變（引用的 hash 會更新）」的檔案。它正好是 hash 檔的反例——不能用 cache busting 換名字。所以它必須設 `no-cache`（或很短 max-age），確保瀏覽器**每次都去驗證、總是拿到最新版**（沒變回 304 省流量、變了拿到新指路牌）。只有拿到最新的指路牌，瀏覽器才知道要去載最新的 hash 檔。

**2. 為什麼 index.html 被快取成舊版就整套破功**：
因為指路牌是整個機制的起點。如果 `index.html` 被強快取成舊版，瀏覽器讀到的還是**舊的引用**——`<script src="/app.a1b2c3.js">`（舊 hash）。於是它去載的是**舊的 hash 檔**，而那個舊檔又正好在永久快取裡命中——結果使用者從頭到尾都在用舊版，你部署的新 `app.f9e8d7.js` 根本沒被任何人引用到。指路牌指錯了，後面全錯，所以整套破功。

這就是為什麼正確策略是「**兩種資源、兩種策略**」：hash 檔永久強快取（`immutable`）、index.html 不強快取（`no-cache`）。
