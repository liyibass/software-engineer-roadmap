...
協商快取省下的是「**重新下載一份『其實沒變』的檔案的流量**」。

「過期就重新下載整個檔案」的問題是：也許檔案根本沒變，卻要把一模一樣的內容整個重傳一次，很浪費。協商快取的做法是——過期後瀏覽器先帶著 `If-None-Match: "<ETag>"`（或 `If-Modified-Since`）去**問伺服器「我手上這份還能用嗎？」**，只有真的變了才下載新版。

**`304 Not Modified`** 就是伺服器回答「**沒變，你手上那份可以繼續用**」。它的關鍵價值是：**304 幾乎不帶任何內容**（只有標頭，可能幾百 bytes），不像重新下載要傳整個檔案（可能幾十 KB 甚至更大）。

舉例：一個 50KB 的 `style.css` 過期後沒變，用協商快取只花一個小小的 304 來回就確認「能繼續用」，**省下重傳 50KB**。所以省下的是「沒變的檔案的重傳流量」，代價只是一次很輕的網路來回。
...
一句話：**`ETag` 用「內容指紋」判斷變了沒，`Last-Modified` 用「最後修改時間」判斷。ETag 比較精確。**

- **`ETag`**：根據**內容**算出的一段指紋字串，內容變指紋才變。所以只要內容一樣，就算檔案被「碰過」也不會誤判成變了——精確度高。
- **`Last-Modified`**：看資源的**最後修改時間**。精度通常只到「秒」，而且「內容沒變但檔案被重新寫入／碰過」也會讓時間改變，因而可能**誤判成變了**，害瀏覽器白白重新下載——比較粗。

實務上兩者可以並存，**ETag 優先**。用 Nginx 這類伺服器時它們多半會自動處理，你主要記住「304 = 沒變、用快取、省流量」這個結果即可。
...
因為協商快取雖好，但它**「每次過期後都會發一個請求」去驗證**——即使結果永遠是 304。對「帶 hash 檔名、內容永不變」的資源來說，這個驗證請求是**純粹的浪費**：

- 用「短 max-age + ETag」→ 每過一小段時間就過期，瀏覽器就發一次協商請求去問「還能用嗎」。答案永遠是「沒變（304）」，因為這個檔名的內容根本不可能變（內容一變檔名就變成別的 hash 了）。雖然省了下載，但**每次都還要付一個網路來回的延遲**。
- 用「超長 max-age + `immutable`」→ 瀏覽器在超長的期限內**連問都不問**，直接落在強快取命中，完全不發請求，最快。`immutable` 更明確告訴瀏覽器「這資源永不變，別去驗證了」。

關鍵洞察：既然 hash 檔名保證「這個檔名的內容永遠不變」，那「去驗證它有沒有變」本身就是多餘的。所以：

- **會變的資源** → 短 max-age + 協商快取（ETag）：過期後問一下，沒變省下載。
- **永不變的資源（hash 檔名）** → 超長 max-age + `immutable`：根本不問，最省（cache-3-5）。
