...
**已完成**：兩台的 nginx / 目錄 / 防火牆設定（腳本化、冪等）、部署腳本、18 項驗證腳本、內網可瀏覽。

**未完成**：

- [ ] **Cloudflare Tunnel** — 需要 Cloudflare 帳號、決定網域、把 Route 53 的 NS 指過去。**在這之前對外連不到**
- [ ] **CI（GitHub self-hosted runner on `98`）** — 建議先手動部署跑一陣子。**把還沒驗證過的步驟寫成 YAML 等於在猜**
- [ ] `90` 上舊架構留下的 `/var/www/kinto` 還沒刪（已經沒人服務，但刪目錄不可逆，留給人工確認）
