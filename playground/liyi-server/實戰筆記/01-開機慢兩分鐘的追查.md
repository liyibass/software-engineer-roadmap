...
> 📌 **後續（2026-08-11）：這條邊已經消失了。**
>
> dnsmasq 因為[另一個原因](../決策紀錄/ADR-004-移除區網DNS改回IP直連.md)被移除，
> nginx 的啟動鏈也就跟著少了 `nss-lookup.target` 這一層——上面擔心的
> 「萬一 dnsmasq 啟動失敗，nginx 也會被卡住」不再是風險。
>
> 值得注意的是：**當初並不是為了開機時間才移除它的**，這只是附帶效果。
> 反過來說也成立——教訓六真正的用處不是「不要裝服務」，而是**知道自己的依賴圖長什麼樣**，
> 這樣不管是加還是減，你都預測得到會發生什麼。
