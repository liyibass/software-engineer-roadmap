...
> **發生時間**：2026-08-01 凌晨　**機器**：`90` (`liyi-jump`)、`98` (`liyi-linux-01`)
...
- 為什麼「同一個修改在兩台機器上會觸發不同機制」，以及怎麼避免被騙
...
修好 90 之後，順手查了 `98`（`liyi-linux-01`，跑 Grafana / Prometheus 的那台）。它也慢，而且更嚴重：

```
Startup finished in 659ms (kernel) + 3.442s (initrd) + 2min 4.143s (userspace)
```

`blame` 一看就知道是同一個病：

```
2min 221ms   systemd-networkd-wait-online.service
```
...
```
ExecStart=/lib/systemd/systemd-networkd-wait-online -i enp1s0f0:degraded
                                                       ^^^^^^^^ 只有這一張
```

90 的清單裡有**兩張**網卡（WiFi + 有線），至少 WiFi 是能就緒的。而 98 的清單裡**只有那張沒插線的有線網卡**，WiFi 根本不在裡面——所以它是 **100% 必定逾時**，沒有任何僥倖的可能。
...
然後照慣例去看產生的結果——**而這一步救了這次的判斷**：

```bash
cat /run/systemd/generator.late/systemd-networkd-wait-online.service.d/10-netplan.conf
```
...
**完全沒有 `ExecStart`。** netplan 改用了另一套機制。

原因是：98 的 WiFi 本來就標了 `optional`，所以把有線也標成 optional 之後，**一張需要等的介面都不剩**。netplan 這時不會產生「等一個空清單」的指令，而是加一個條件——指向一個它不會建立的符號連結。條件不成立，systemd 就**直接跳過整個服務**。
...
> 💡 **第七個教訓**：**同一個修改，在不同機器上可能觸發完全不同的機制。**
>
> 如果直接照抄 90 的結論、假設 98 也會產生 `-i wlp2s0b1`，就不會發現它其實走了另一條路。
> 更糟的是，如果 netplan 在這種情況下產生的是「沒有 `-i` 就等所有介面」，問題不但沒解決還會變糟——
> **而你會以為自己修好了。**
>
> 套用設定後去看「實際產生了什麼」，成本只有一行 `cat`，但它是「我以為」跟「我知道」的分界線。

⚠️ 這個修法對 98 有個取捨要記著：它現在**完全不等網路就緒**就啟動所有服務。這對它合適，因為服務都是 Docker 容器（`restart: unless-stopped` 會自己重試）。但若將來裝了「啟動時就必須有網路、否則直接失敗」的服務，就得把 WiFi 改回 `optional: false`。

### 意外的第二個發現：kdump

修好之後 98 是 19.229s，但 90 只有 10.358s。差在哪？

```
10.181s  kdump-tools.service      ← 98 有，90 沒有
```

`kdump` 的用途是**核心崩潰時把記憶體內容 dump 出來供分析**。它的代價有兩個，而第二個很容易被忽略：
...
比對兩台：90 **從未安裝**這個套件（`dpkg -l` 顯示 `un`），記憶體 7.7Gi 完整可用；98 是 `ii`，只剩 7.2Gi。兩台硬體相近、用途相似——這是**安裝時選項不同造成的偶然差異**，不是設計。
...
因為 `crashkernel=` 是**核心開機參數**，在 kernel 極早期就把記憶體切走了，跟服務有沒有跑無關。只停服務等於「保留了記憶體卻沒人用」——最差的組合。

而 `crashkernel=` 是套件自己裝的 GRUB 片段帶進來的：

```
/etc/default/grub.d/kdump-tools.cfg
```

所以 `apt purge` 會連它一起移除，不用手動編輯 GRUB。之後 `update-grub` 重新產生設定，**重開機才生效**。
...
**總計快 11.8 倍，userspace 快 18.9 倍，外加拿回 512MB 記憶體。**

最後 `blame` 的第一名只剩 `dev-rfkill.device`（4.756s）這種硬體裝置探測——**沒有任何服務在拖慢開機了**。這就是「已經沒得優化」的樣子。
...
| `grep -o 'crashkernel=[^ ]*' /proc/cmdline` | 看核心開機參數（服務停掉也不會影響它） |
