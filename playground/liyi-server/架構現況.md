...
graph TB
    ...
    R --- J
    R --- S
    U -->|"① DNS 查詢<br/>grafana.home.arpa"| J
    U -->|"② HTTP :80"| J
    J -->|"③ 反向代理<br/>:3000"| S
    S -->|"抓取指標 :9100"| J
```
...
| `dnsmasq` | `192.168.10.90:53` | ✅ | 區網 DNS，`.home.arpa` 權威 + 上游轉發 |
...
```
jump.home.arpa      → 192.168.10.90      機器本身
services.home.arpa  → 192.168.10.98      機器本身
grafana.home.arpa   → 192.168.10.90      服務（指向反向代理）
```
...
```
http://grafana.home.arpa/            → 192.168.10.98:3000
http://grafana.home.arpa/api/live/   → 同上，帶 WebSocket 升級標頭
http://192.168.10.90/grafana/        → 301 導向 grafana.home.arpa（舊網址相容）
...
- [ ] `dig +short @192.168.10.90 grafana.home.arpa` 回傳正確
- [ ] `curl -I http://grafana.home.arpa/` → 302
...
| 🔴 | **98 的 `wait-online` 只等沒插線的 `enp1s0f0`**，每次開機必定逾時 120 秒 | 修正檔已備於 98 的 `/tmp` |
| 🔴 | SSH 仍允許密碼登入（兩台）。90 的 `authorized_keys` 有一把來源不明的金鑰（註解是未修改的樣板 `"your.name@gemmis.io"`），需先驗證才能關閉密碼登入 | 待驗證 |
| 🟡 | 路由器 DHCP 尚未下發 `192.168.10.90` 當 DNS，客戶端還無法解析 `.home.arpa` | 待操作 |
...
> 之後若開機時間明顯變長，拿這組數字當比較基準。原始紀錄在 90 的 `~/boot-times.log`。
