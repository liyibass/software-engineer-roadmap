...
> **最後更新**：2026-08-11（移除區網 DNS，路由改回 IP 直連）
...
graph TB
    ...
    J["<b>90</b> liyi-jump<br/>192.168.10.90 (WiFi)<br/>nginx（唯一入口）<br/>node_exporter"]
    ...
    R --- J
    R --- S
    U -->|"① HTTP :80<br/>192.168.10.90"| J
    J -->|"② 反向代理<br/>:3000"| S
    S -->|"抓取指標 :9100"| J
```

使用者的一次存取只有兩步：瀏覽器直接連 `http://192.168.10.90/`，nginx 以 `default_server` 接住後轉發到 98 的 Grafana。
...
2026-08-11 移除區網 DNS 後，這套系統裡的 DNS 剩下兩套：
...
> ⚠️ 根路徑被 Grafana 佔走，原本的靜態導覽頁已移除。要再加第三個服務時，既沒有名稱
...
| 2026-08-11 | 移除 90 的 dnsmasq，區網 DNS 走入歷史（ADR-001 由 ADR-004 取代） | 同上 |
