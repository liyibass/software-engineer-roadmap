...
90 上有**兩個入口、兩種角色的服務**，另外多一條從外面進來的路：

| 網址 | 到哪 | 誰看得到 |
|---|---|---|
| `http://192.168.10.90/` | Grafana（`default_server` 接住，轉發到 98:3000） | 只有區網 |
| `http://192.168.10.90:8080/` | Kinto 官網（轉發到 98:8080 的 nginx origin） | 只有區網 |
| **`https://kinto.liyibass.com`** | 同上那條，經 Cloudflare Tunnel 進來 | **全世界** |

### 對外：Cloudflare Tunnel（2026-08-28 上線）

```
訪客 → Cloudflare 邊緣（TLS ＋ CDN）══Tunnel══→ 90 的 127.0.0.1:8080 → 98:8080
```

`cloudflared` 由 90 **主動對外撥出**長連線，所以**路由器一個入向 port 都不用開**，
家裡的公網 IP 也不會出現在 DNS 上（`dig kinto.liyibass.com` 回的是 Cloudflare 的 IP）。

⚠️⚠️ **`/etc/cloudflared/config.yml` 的 ingress catch-all 是唯一擋住 Grafana 上公網的東西。**

```yaml
ingress:
  - hostname: kinto.liyibass.com
    service: http://127.0.0.1:8080
  - service: http_status:404      # ⚠️ 不可刪、不可改成 http://…
```

90 的 `:80` 仍然是 Grafana 的 `default_server`、仍然通吃所有 Host——
**擋住外面的是這個檔，不是 nginx**。實測：拿不存在的主機名去打隧道會被
Cloudflare 擋在 530，根本到不了 `cloudflared`。

| 設定 | 值 |
|---|---|
| Always Use HTTPS | 開（`http://` → 301） |
| CDN 快取 | assets `HIT`；⚠️ **HTML 是 `DYNAMIC`**，免費方案預設不快取 HTML |
| 憑證檔 | `/etc/cloudflared/<UUID>.json`（600，**機密，不可進版控**） |
...
| `cloudflared` | —（主動外連） | ✅ | Cloudflare Tunnel，只通到 `127.0.0.1:8080` |
| `fail2ban` | — | ✅ | |
...
| `actions.runner.*` ×2 | —（主動外連） | GitHub self-hosted runner，`game-website` 與 `crash-games` 各一 |
| `fail2ban` | — | 2026-08-28 補裝（先前只有 90 有） |

⭐ **runner 與 origin 同一台，所以「部署」只是本機複製**——不需要 SSH 到 90、
不需要任何跨機金鑰。這是「門 vs 打雜」這個分法順手消掉的問題，不是刻意設計的。
...
| 2026-08-28 | 網域 DNS 從 Route 53 搬到 Cloudflare | [E-3-9](../../課外讀物/E-3-network/E-3-9-dns-delegation-migration.md) |
| 2026-08-28 | Cloudflare Tunnel 上線，`https://kinto.liyibass.com` 對外開放 | 同上 |
| 2026-08-28 | 98 裝兩個 GitHub self-hosted runner，兩個 repo push 即自動部署 | 04 |
| 2026-08-28 | 兩台套件更新，98 補上 fail2ban | — |
