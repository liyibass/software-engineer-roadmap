...
這張圖是目前的流量走向：**所有請求都先到 `http://192.168.10.90/`，再由它轉發到後端**。
90 自己不跑應用服務。目前只有一個後端，nginx 用 `default_server` 全數轉發；
分流規則等第二個服務出現再說（見 [ADR-004](決策紀錄/ADR-004-移除區網DNS改回IP直連.md)）。
...
| 02 | [一台機器上的三套 DNS](實戰筆記/02-三套DNS與自架區網解析.md) | systemd-resolved vs dnsmasq，以及一個會製造間歇性故障的設定陷阱 ⚠️ 這套 DNS 已於 2026-08-11 移除 | `infra-3-1`、`infra-3-4` |
...
- [x] ~~**路徑式 → 子網域路由** — dnsmasq + `.liyibass.internal`，見 ADR-001~~
      **2026-08-11 已回退**：拆掉 dnsmasq，改回 `http://192.168.10.90/` 直連。
